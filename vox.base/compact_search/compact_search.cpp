...
NeighborhoodSearch::NeighborhoodSearch(Real r, bool erase_empty_cells)
    : m_r2(r * r),
      ...
      m_initialized(false) {
    if (r <= 0.0) {
        std::cerr << "WARNING: Neighborhood search may not be initialized with a "
                     "zero or negative"
...
void NeighborhoodSearch::z_sort() {
    for (PointSet &d : m_point_sets) {
        ...
        std::sort(d.m_sort_table.begin(), d.m_sort_table.end(), [&](unsigned int a, unsigned int b) {
...
void NeighborhoodSearch::init() {
    ...
    for (unsigned int j = 0; j < m_point_sets.size(); ++j) {
        ...
        for (auto &l : d.m_locks) l.resize(d.n_points());
        for (unsigned int i = 0; i < d.n_points(); i++) {
            ...
            if (it == m_map.end()) {
                ...
                if (m_activation_table.is_searching_neighbors(j)) m_entries.back().n_searching_points++;
                temp_keys.push_back(key);
                m_map[key] = static_cast<unsigned int>(m_entries.size() - 1);
            } else {
                m_entries[it->second].add({j, i});
                if (m_activation_table.is_searching_neighbors(j)) m_entries[it->second].n_searching_points++;
            }
        }
    }
...
void NeighborhoodSearch::resize_point_set(unsigned int index, Real const *x, std::size_t size) {
    ...
    if (old_size > size) {
        ...
        for (unsigned int i = static_cast<unsigned int>(size); i < old_size; i++) {
            ...
            if (m_activation_table.is_searching_neighbors(index)) m_entries[it->second].n_searching_points--;
            if (m_erase_empty_cells) {
    ...
    for (unsigned int i = static_cast<unsigned int>(old_size); i < point_set.n_points(); i++) {
        ...
        if (it == m_map.end()) {
            ...
            if (m_activation_table.is_searching_neighbors(index)) m_entries.back().n_searching_points++;
            m_map[key] = static_cast<unsigned int>(m_entries.size() - 1);
        } else {
...
void NeighborhoodSearch::find_neighbors(unsigned int point_set_id,
                                        unsigned int point_index,
                                        std::vector<std::vector<unsigned int>> &neighbors) {
    query(point_set_id, point_index, neighbors);
}

void NeighborhoodSearch::find_neighbors(Real const *x, std::vector<std::vector<unsigned int>> &neighbors) {
    query(x, neighbors);
}

void NeighborhoodSearch::erase_empty_entries(std::vector<unsigned int> const &to_delete) {
    if (to_delete.empty()) return;

    // Indicated empty cells.
    m_entries.erase(std::remove_if(m_entries.begin(), m_entries.end(),
                                   [](HashEntry const &entry) { return entry.indices.empty(); }),
                    m_entries.end());
...
void NeighborhoodSearch::update_hash_table(std::vector<unsigned int> &to_delete) {
    ...
    for (unsigned int j = 0; j < m_point_sets.size(); ++j) {
        ...
        for (unsigned int i = 0; i < d.n_points(); ++i) {
            ...
            if (it == m_map.end()) {
                ...
                entry.add({j, i});
                if (m_activation_table.is_searching_neighbors(j)) entry.n_searching_points++;
            }

            unsigned int entry_index = m_map[d.m_old_keys[i]];
            m_entries[entry_index].erase({j, i});
            if (m_activation_table.is_searching_neighbors(j)) m_entries[entry_index].n_searching_points--;
            if (m_erase_empty_cells) {
    ...
    to_delete.erase(std::remove_if(to_delete.begin(), to_delete.end(),
                                   [&](unsigned int index) { return m_entries[index].n_indices() != 0; }),
                    to_delete.end());
...
void NeighborhoodSearch::query() {
    for (unsigned int i = 0; i < m_point_sets.size(); i++) {
        ...
        for (unsigned int j = 0; j < d.m_neighbors.size(); j++) {
            ...
            for (auto &n_ : n) {
                n_.clear();
                if (m_activation_table.is_active(i, j)) n_.reserve(INITIAL_NUMBER_OF_NEIGHBORS);
            }
        }
    }
...
void NeighborhoodSearch::query(unsigned int point_set_id,
                               unsigned int point_index,
                               std::vector<std::vector<unsigned int>> &neighbors) {
    ...
    for (unsigned int j = 0; j < m_point_sets.size(); j++) {
        auto &n = neighbors[j];
        n.clear();
        if (m_activation_table.is_active(point_set_id, j)) n.reserve(INITIAL_NUMBER_OF_NEIGHBORS);
    }
...
void NeighborhoodSearch::query(Real const *xa, std::vector<std::vector<unsigned int>> &neighbors) {
    neighbors.resize(m_point_sets.size());
    for (unsigned int j = 0; j < m_point_sets.size(); j++) {
        auto &n = neighbors[j];
        n.clear();
