...
namespace serialize {
namespace details {
template <class T> bool write(std::streambuf &buf, const T &val) {
...
template <class T> bool read(std::streambuf &buf, T &val) {
  static_assert(std::is_standard_layout<T>{}, "data is not standard layout");
  auto bytes = sizeof(T);
...
template <class T> bool write(std::streambuf &buf, T const &val) {
  using details::write;
