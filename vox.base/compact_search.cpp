...
NeighborhoodSearch::NeighborhoodSearch(Real r, bool erase_empty_cells)
    : m_r2(r * r), m_inv_cell_size(static_cast<Real>(1.0 / r)),
      m_erase_empty_cells(erase_empty_cells), m_initialized(false) {
  if (r <= 0.0) {
    std::cerr << "WARNING: Neighborhood search may not be initialized with a "
                 "zero or negative"
              << " search radius. This may lead to unexpected behavior."
              << std::endl;
  }
}
  ...
  for (PointSet &d : m_point_sets) {
    ...
    std::sort(d.m_sort_table.begin(), d.m_sort_table.end(),
              [&](unsigned int a, unsigned int b) {
                return z_value(cell_index(d.point(a))) <
                       z_value(cell_index(d.point(b)));
              });
  }
  m_initialized = false;
}
...
void NeighborhoodSearch::resize_point_set(unsigned int index, Real const *x,
                                          std::size_t size) {
  ...
  for (unsigned int i = static_cast<unsigned int>(old_size);
       i < point_set.n_points(); i++) {
...
void NeighborhoodSearch::find_neighbors(
    unsigned int point_set_id, unsigned int point_index,
    std::vector<std::vector<unsigned int>> &neighbors) {
  query(point_set_id, point_index, neighbors);
}

void NeighborhoodSearch::find_neighbors(
    Real const *x, std::vector<std::vector<unsigned int>> &neighbors) {
  query(x, neighbors);
}

void NeighborhoodSearch::erase_empty_entries(
    std::vector<unsigned int> const &to_delete) {
  if (to_delete.empty())
    return;
...

  {
    ...
    while (it != m_map.end()) {
      ...
      if (kvp.second <= to_delete.front() && kvp.second >= to_delete.back() &&
          std::binary_search(to_delete.rbegin(), to_delete.rend(),
                             kvp.second)) {
        it = m_map.erase(it);
      } else {
        ++it;
  ...
  std::transform(
      m_map.begin(), m_map.end(), kvps.begin(),
...
#endif
      (kvps.begin(), kvps.end(),
       [&](std::pair<HashKey const, unsigned int> *kvp_) {
         auto &kvp = *kvp_;
...
void NeighborhoodSearch::update_hash_table(
    std::vector<unsigned int> &to_delete) {
...
void NeighborhoodSearch::query() {
  ...
  std::vector<std::pair<HashKey const, unsigned int> const *> kvps(
      m_map.size());
  std::transform(
      m_map.begin(), m_map.end(), kvps.begin(),
...
#endif
      (kvps.begin(), kvps.end(),
       [&](std::pair<HashKey const, unsigned int> const *kvp_) {
         auto const &kvp = *kvp_;
         ...
         for (unsigned int a = 0; a < entry.n_indices(); ++a) {
           ...
           for (unsigned int b = a + 1; b < entry.n_indices(); ++b) {
             ...
             if (!m_activation_table.is_active(va.point_set_id,
                                               vb.point_set_id) &&
                 !m_activation_table.is_active(vb.point_set_id,
                                               va.point_set_id)) {
               continue;
             }
             ...
             if (l2 < m_r2) {
               if (m_activation_table.is_active(va.point_set_id,
                                                vb.point_set_id)) {
                 da.m_neighbors[vb.point_set_id][va.point_id].push_back(
                     vb.point_id);
               }
               if (m_activation_table.is_active(vb.point_set_id,
                                                va.point_set_id)) {
                 db.m_neighbors[va.point_set_id][vb.point_id].push_back(
                     va.point_id);
               }
             }
           }
         ...
         for (int dj = -1; dj <= 1; dj++)
           for (int dk = -1; dk <= 1; dk++)
             for (int dl = -1; dl <= 1; dl++) {
               ...
               auto it =
                   m_map.find({key.k[0] + dj, key.k[1] + dk, key.k[2] + dl});
               if (it == m_map.end())
                 continue;
  ...
  for (int dj = -1; dj <= 1; dj++) {
    for (int dk = -1; dk <= 1; dk++) {
      for (int dl = -1; dl <= 1; dl++) {
        ...
        auto it = m_map.find(
            {hash_key.k[0] + dj, hash_key.k[1] + dk, hash_key.k[2] + dl});
        if (it == m_map.end())
          continue;
...
void NeighborhoodSearch::query(
    Real const *xa, std::vector<std::vector<unsigned int>> &neighbors) {
  neighbors.resize(m_point_sets.size());
  for (unsigned int j = 0; j < m_point_sets.size(); j++) {
    auto &n = neighbors[j];
    n.clear();
