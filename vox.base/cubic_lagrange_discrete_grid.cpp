...
bool CubicLagrangeDiscreteGrid::determineShapeFunctions(
    unsigned int field_id, Eigen::Vector3d const &x,
    std::array<unsigned int, 32> &cell, Eigen::Vector3d &c0,
    Eigen::Matrix<double, 32, 1> &N, Eigen::Matrix<double, 32, 3> *dN) const {
...
double CubicLagrangeDiscreteGrid::interpolate(
    unsigned int field_id, Eigen::Vector3d const &xi,
    const std::array<unsigned int, 32> &cell, const Eigen::Vector3d &c0,
    const Eigen::Matrix<double, 32, 1> &N, Eigen::Vector3d *gradient,
...
double CubicLagrangeDiscreteGrid::interpolate(unsigned int field_id,
                                              Vector3d const &x,
...
void CubicLagrangeDiscreteGrid::reduceField(unsigned int field_id,
  ...
  for (auto l = 0u; l < coeffs.size(); ++l) {
    ...
    keep[l] =
        pred(xi, coeffs[l]) && coeffs[l] != std::numeric_limits<double>::max();
  }
...
void CubicLagrangeDiscreteGrid::forEachCell(
    ...
    std::function<void(unsigned int, AlignedBox3d const &, unsigned int)> const
        &cb) const {
  ...
  for (auto i = 0u; i < n; ++i) {
    ...
    domain.min() =
        m_domain.min() +
        Matrix<unsigned int, 3, 1>::Map(mi.data()).cast<double>().cwiseProduct(
            m_cell_size);
