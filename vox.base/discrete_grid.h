...
public:
  ...
  DiscreteGrid(Eigen::AlignedBox3d const &domain,
  ...
  double interpolate(Eigen::Vector3d const &xi,
                     Eigen::Vector3d *gradient = nullptr) const {
   ...
   */
  virtual bool
  determineShapeFunctions(unsigned int field_id, Eigen::Vector3d const &x,
   ...
   */
  virtual double
  interpolate(unsigned int field_id, Eigen::Vector3d const &xi,
              const std::array<unsigned int, 32> &cell,
              const Eigen::Vector3d &c0, const Eigen::Matrix<double, 32, 1> &N,
  ...
  [[nodiscard]] std::array<unsigned int, 3> const &resolution() const {
    return m_resolution;
  };
  [[nodiscard]] Eigen::Vector3d const &cellSize() const { return m_cell_size; }
  [[nodiscard]] Eigen::Vector3d const &invCellSize() const {
    return m_inv_cell_size;
  }
