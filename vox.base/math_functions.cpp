...
void MathFunctions::svdWithInversionHandling(const Matrix3r &A, Vector3r &sigma, Matrix3r &U, Matrix3r &VT) {
    ...
    if (chk > 0) {
        ...
        Vector3r sigmaInv(static_cast<Real>(1.0) / sigma[0], static_cast<Real>(1.0) / sigma[1],
...
void MathFunctions::getOrthogonalVectors(const Vector3r &vec, Vector3r &x, Vector3r &y) {
    ...
    if (fabs(v.dot(vec)) > 0.999) v = Vector3r(0, 1, 0);
...
void MathFunctions::APD_Newton(const Matrix3r &F, Quaternionr &q) {
    ...
    for (int it = 0; it < 1; it++) {
        ...
        const Real detH = static_cast<Real>(-1.0) * h02 * h02 * h11 + static_cast<Real>(2.0) * h01 * h02 * h12 -
        ...
        if (fabs(detH) < static_cast<Real>(1.0e-9)) omega = -gradient;

        // instead of clamping just use gradient descent. also works fine and does not require the norm
        if (omega.dot(gradient) > 0.0) omega = gradient * static_cast<Real>(-0.125);
        ...
        q = q * Quaternionr(w, vec.x(), vec.y(),
                            vec.z());  // no normalization needed because the Cayley map returs a unit quaternion
    }
}
