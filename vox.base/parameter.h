...
public:
  ...
  template <typename T> using SetFunc = std::function<void(T)>;
  template <typename T> using GetFunc = std::function<T()>;
  template <typename T> using SetVecFunc = std::function<void(T *)>;
  template <typename T> using GetVecFunc = std::function<T *()>;

  ParameterBase(std::string name, std::string label, const DataTypes type)
      : m_name(std::move(name)), m_label(std::move(label)), m_type(type),
        m_readOnly(false), m_visible(true) {}
   ...
   */
  template <typename T> bool checkType(T v) { return false; }
 ...
 * There are two ways to construct a generic parameter. Either the user provides
 * a pointer of a variable of the corresponding type or he provides a get and a
 * set function. Changing the generic parameter will then also change the
 * corresponding variable or class the corresponding set function.\n\n An
 * example how to generate generic parameters and how to use them can be found
 * in the ParameterTest and the corresponding TestParameterObject.
 */
...
public:
  ...
  /** Get/Set-Function constructor. Note that if the setValue function is the
   * nullptr, then the parameter is marked as read-only. */
  Parameter(const std::string &name, const std::string &label,
            ParameterBase::DataTypes type, GetFunc<T> getValue,
            SetFunc<T> setValue)
      : ParameterBase(name, label, type), m_getValue(getValue),
        m_setValue(setValue) {
