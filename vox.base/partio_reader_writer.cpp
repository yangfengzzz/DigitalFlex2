...
bool PartioReaderWriter::readParticles(const std::string &fileName,
    ...
    if (velIndex != 0xffffffff) {
        auto fSize = (unsigned int)velocities.size();
        velocities.resize(fSize + data->numParticles());
        data->attributeInfo(velIndex, attr);
        for (int i = 0; i < data->numParticles(); i++) {
            const auto *vel = data->data<float>(attr, i);
        ...
        auto fSize = (unsigned int)velocities.size();
        velocities.resize(fSize + data->numParticles());
        for (int i = 0; i < data->numParticles(); i++) velocities[i + fSize].setZero();
    }
...
bool PartioReaderWriter::readParticles(const std::string &fileName,
    ...
    if (radiusIndex != 0xffffffff) {
        ...
        const auto *radius = data->data<float>(attr, 0);
...
void PartioReaderWriter::writeParticles(const std::string &fileName,
    ...
    if (particleVelocities != nullptr) velAttr = particleData.addAttribute("velocity", Partio::VECTOR, 3);
    Partio::ParticleAttribute scaleAttr;
    if (particleRadius != 0.0) scaleAttr = particleData.addAttribute("pscale", Partio::FLOAT, 1);
    ...
    for (unsigned int i = 0; i < numParticles; i++) {
        ...
        auto *pos = particleData.dataWrite<float>(posAttr, index);
        ...
        if (particleVelocities != nullptr) {
            auto *vel = particleData.dataWrite<float>(velAttr, index);
        ...
        if (particleRadius != 0.0) {
            auto *scale = particleData.dataWrite<float>(scaleAttr, index);
