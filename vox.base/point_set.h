...
public:
   ...
   * @param point_set Point set index of other point set where neighbors have
   * been searched.
   ...
   */
  [[nodiscard]] std::size_t n_points() const { return m_n; }
  ...
  /**
   * Reorders an array according to a previously generated sort table by
   * invocation of the method "z_sort" of class "NeighborhoodSearch". Please
   * note that the method "z_sort" of class "Neighborhood search" has to be
   * called beforehand.
   */
  template <typename T> void sort_field(T *lst) const;
...
private:
  ...
  PointSet(Real const *x, std::size_t n, bool dynamic,
      ...
      : m_x(x), m_n(n), m_dynamic(dynamic), m_user_data(user_data),
        m_neighbors(n), m_keys(n, {std::numeric_limits<int>::lowest(),
  ...
  void resize(Real const *x, std::size_t n) {
    m_x = x;
    m_n = n;
    m_keys.resize(n, {std::numeric_limits<int>::lowest(),
                      std::numeric_limits<int>::lowest(),
                      std::numeric_limits<int>::lowest()});
    m_old_keys.resize(n, {std::numeric_limits<int>::lowest(),
                          std::numeric_limits<int>::lowest(),
                          std::numeric_limits<int>::lowest()});
    m_neighbors.resize(n);
  }

  [[nodiscard]] Real const *point(unsigned int i) const { return &m_x[3 * i]; }
...
template <typename T> void PointSet::sort_field(T *lst) const {
  if (m_sort_table.empty()) {
    std::cerr
        << "WARNING: No sort table was generated for the current point set. "
        << "First invoke the method 'z_sort' of the class 'NeighborhoodSearch.'"
        << std::endl;
    return;
  }
