...
public:
  ...
  TriangleMesh(double const *vertices, unsigned int const *faces,
  ...
  [[nodiscard]] FaceConstContainer faces() const {
    return FaceConstContainer(this);
  }
  [[nodiscard]] IncidentFaceContainer incident_faces(unsigned int v) const {
    return IncidentFaceContainer(v, this);
  }
  VertexContainer vertices() { return VertexContainer(this); }
  [[nodiscard]] VertexConstContainer vertices() const {
    return VertexConstContainer(this);
  }
  ...
  // Entity getters.
  [[nodiscard]] unsigned int const &faceVertex(unsigned int f,
  ...
  [[nodiscard]] Eigen::Vector3d const &vertex(unsigned int i) const {
    return m_vertices[i];
  }
  Eigen::Vector3d &vertex(unsigned int i) { return m_vertices[i]; }
  [[nodiscard]] std::array<unsigned int, 3> const &face(unsigned int i) const {
    return m_faces[i];
  }
  std::array<unsigned int, 3> &face(unsigned int i) { return m_faces[i]; }
  [[nodiscard]] Halfedge incident_halfedge(unsigned int v) const {
    return m_v2e[v];
  }

  // Data getters.
  [[nodiscard]] std::vector<Eigen::Vector3d> const &vertex_data() const {
    return m_vertices;
  }
  std::vector<Eigen::Vector3d> &vertex_data() { return m_vertices; }
  [[nodiscard]] std::vector<std::array<unsigned int, 3>> const &
  face_data() const {
    return m_faces;
  }
