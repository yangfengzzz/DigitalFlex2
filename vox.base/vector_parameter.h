 ...
 */
template <typename T> class VectorParameter : public ParameterBase {
protected:
...
public:
  VectorParameter(const std::string &name, const std::string &label,
  ...
  VectorParameter(const std::string &name, const std::string &label,
                  const unsigned int dim, ParameterBase::GetVecFunc<T> getValue,
                  ParameterBase::SetVecFunc<T> setValue)
      : ParameterBase(name, label, ParameterBase::INT32), m_dim(dim),
        m_getVecValue(getValue), m_setVecValue(setValue) {
    setType(getValue()[0]);
  }
