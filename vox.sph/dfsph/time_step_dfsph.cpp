...
int TimeStepDFSOLVER_ITERATIONS_V = -1;
int TimeStepDFMAX_ITERATIONS_V = -1;
int TimeStepDFMAX_ERROR_V = -1;
int TimeStepDFUSE_DIVERGENCE_SOLVER = -1;

TimeStepDFSPH::TimeStepDFSPH() : TimeStep(), m_simulationData() {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) {
        ...
        model->addField(
                {"advected density", FieldType::Scalar, [this, fluidModelIndex](const unsigned int i) -> Real * {
                     return &m_simulationData.getDensityAdv(fluidModelIndex, i);
                 }});
        model->addField({"kappa", FieldType::Scalar,
                         [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getKappa(fluidModelIndex, i);
                         },
                         true});
        model->addField({"kappa_v", FieldType::Scalar,
                         [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getKappaV(fluidModelIndex, i);
                         },
                         true});
    }
}
...
void TimeStepDFSPH::initParameters() {
    ...
    USE_DIVERGENCE_SOLVER =
            createBoolParameter("enableDivergenceSolver", "Enable divergence solver", &m_enableDivergenceSolver);
...
#pragma omp for schedule(static)
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nFluids; fluidModelIndex++) {
        ...
        //////////////////////////////////////////////////////////////////////////
        for (int i = 0; i < numParticles; i++) m_simulationData.getKappa(fluidModelIndex, i) *= h2;
    }
#endif
}
...
#pragma omp for schedule(static)
    ...
    while ((!chk || (m_iterationsV < 1)) && (m_iterationsV < maxIter)) {
        ...
        for (unsigned int i = 0; i < nFluids; i++) {
            ...
            const Real eta = (static_cast<Real>(1.0) / h) * maxError * static_cast<Real>(0.01) *
                             density0;  // maxError is given in percent
            chk = chk && (avg_density_err <= eta);
        }
...
#ifdef USE_WARMSTART_V
        for (int i = 0; i < numParticles; i++) m_simulationData.getKappaV(fluidModelIndex, i) *= h;
#endif
...
void TimeStepDFcomputeDFSPHFactor(const unsigned int fluidModelIndex) {
    //////////////////////////////////////////////////////////////////////////
    // Init parameters
    //////////////////////////////////////////////////////////////////////////
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors_avx_nox(compute_xj(fm_neighbor, pid); compute_Vj(fm_neighbor); compute_Vj_gradW();
                                           const Vector3f8 &gradC_j = V_gradW;
                                           sum_grad_p_k_avx += gradC_j.squaredNorm(); grad_p_i_avx += gradC_j;);

            //////////////////////////////////////////////////////////////////////////
            // Boundary
            //////////////////////////////////////////////////////////////////////////
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors_avx(const Scalarf8 V_avx =
                                                      convert_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j],
            ...
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(grad_p_i -= gradRho;);
            } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(const Vector3r grad_p_j = -Vj * sim->gradW(xi - xj); grad_p_i -= grad_p_j;);
            }
...
#ifdef USE_WARMSTART
void TimeStepDFwarmstartPressureSolve(const unsigned int fluidModelIndex) {
    const Real h = TimeManager::getCurrent()->getTimeStepSize();
    const Real h2 = h * h;
    const Real invH = static_cast<Real>(1.0) / h;
    const Real invH2 = static_cast<Real>(1.0) / h2;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            // m_simulationData.getKappa(fluidModelIndex, i) = max(m_simulationData.getKappa(fluidModelIndex, i)*invH2,
            // -static_cast<Real>(0.5) * density0*density0);
            computeDensityAdv(fluidModelIndex, i, numParticles, h, density0);
            if (m_simulationData.getDensityAdv(fluidModelIndex, i) > 1.0)
                m_simulationData.getKappa(fluidModelIndex, i) =
                        static_cast<Real>(0.5) *
                        max(m_simulationData.getKappa(fluidModelIndex, i), static_cast<Real>(-0.00025)) * invH2;
            else
                m_simulationData.getKappa(fluidModelIndex, i) = 0.0;
        }
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            // if (m_simulationData.getDensityAdv(fluidModelIndex, i) > density0)
            {
                ...
                forall_fluid_neighbors_avx_nox(
                        compute_xj(fm_neighbor, pid); compute_Vj(fm_neighbor); compute_Vj_gradW();
                        const Scalarf8 densityFrac_avx(fm_neighbor->getDensity0() / density0);
                        const Scalarf8 kj_avx = convert_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j],
                                                             &m_simulationData.getKappa(pid, 0), count);
                ...
                if (fabs(ki) > m_eps) {
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                        ...
                        forall_boundary_neighbors_avx(
                                ...
                                // Directly update velocities instead of storing pressure accelerations
                                const Vector3f8 velChange = -CubicKernel_AVX::gradW(xj_avx - xi_avx) *
                                                            (Vj_avx * Scalarf8(1.0f * h) *
                                                             ki_avx);  // ki, kj already contain inverse density
                                delta_vi += velChange;
                ...
                if (fabs(ki) > m_eps) {
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                        forall_density_maps(
                                ...
                                vi += velChange; bm_neighbor->addForce(xj, -model->getMass(i) * velChange * invH););
                    } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                        forall_volume_maps(
                                const Vector3r velChange =
                                        h * ki * Vj * sim->gradW(xi - xj);  // kj already contains inverse density
                                vi += velChange; bm_neighbor->addForce(xj, -model->getMass(i) * velChange * invH););
                    }
                }
            }
...
void TimeStepDFpressureSolveIteration(const unsigned int fluidModelIndex, Real &avg_density_err) {
    Simulation *sim = Simulation::getCurrent();
    FluidModel *model = sim->getFluidModel(fluidModelIndex);
    const Real density0 = model->getDensity0();
    const int numParticles = (int)model->numActiveParticles();
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            if (model->getParticleState(i) != ParticleState::Active) continue;
            ...
            forall_fluid_neighbors_avx_nox(
                    compute_xj(fm_neighbor, pid); compute_Vj(fm_neighbor); compute_Vj_gradW();
                    const Scalarf8 densityFrac_avx(fm_neighbor->getDensity0() / density0);
                    const Scalarf8 densityAdvj_avx = convert_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j],
                                                                  &m_simulationData.getDensityAdv(pid, 0), count);
                    const Scalarf8 factorj_avx = convert_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j],
                                                              &m_simulationData.getFactor(pid, 0), count);
            ...
            if (fabs(ki) > m_eps) {
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    ...
                    forall_boundary_neighbors_avx(
                            ...
                            // Directly update velocities instead of storing pressure accelerations
                            const Vector3f8 velChange =
                                    -CubicKernel_AVX::gradW(xj_avx - xi_avx) *
                                    (Vj_avx * Scalarf8(1.0f * h) * ki_avx);  // ki, kj already contain inverse density
                            delta_vi += velChange;
...
#ifdef USE_WARMSTART_V
void TimeStepDFwarmstartDivergenceSolve(const unsigned int fluidModelIndex) {
    const Real h = TimeManager::getCurrent()->getTimeStepSize();
    const Real invH = static_cast<Real>(1.0) / h;
    Simulation *sim = Simulation::getCurrent();
    FluidModel *model = sim->getFluidModel(fluidModelIndex);
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            if (m_simulationData.getDensityAdv(fluidModelIndex, i) > 0.0)
                m_simulationData.getKappaV(fluidModelIndex, i) =
                        static_cast<Real>(0.5) *
                        max(m_simulationData.getKappaV(fluidModelIndex, i), static_cast<Real>(-0.5)) * invH;
            else
                m_simulationData.getKappaV(fluidModelIndex, i) = 0.0;
        }
        ...
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // if (m_simulationData.getDensityAdv(fluidModelIndex, i) > 0.0)
            {
                ...
                forall_fluid_neighbors_avx_nox(
                        ...
                        const Scalarf8 densityFrac_avx(fm_neighbor->getDensity0() / density0);
                        const Scalarf8 kj_avx = convert_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j],
                                                             &m_simulationData.getKappaV(pid, 0), count);
                        ; const Scalarf8 kSum_avx = ki_avx + densityFrac_avx * kj_avx;
                ...
                if (fabs(ki) > m_eps) {
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                        ...
                        forall_boundary_neighbors_avx(
                                ...
                                const Vector3f8 velChange =
                                        -CubicKernel_AVX::gradW(xj_avx - xi_avx) *
                                        (Vj_avx * h_avx * ki_avx);  // ki, kj already contain inverse density
                                delta_vi += velChange;
...
void TimeStepDFdivergenceSolveIteration(const unsigned int fluidModelIndex, Real &avg_density_err) {
    Simulation *sim = Simulation::getCurrent();
    FluidModel *model = sim->getFluidModel(fluidModelIndex);
    const Real density0 = model->getDensity0();
    const int numParticles = (int)model->numActiveParticles();
...
void TimeStepDFcomputeDensityAdv(const unsigned int fluidModelIndex,
    ...
    forall_fluid_neighbors_avx_nox(compute_xj(fm_neighbor, pid); compute_Vj(fm_neighbor); compute_Vj_gradW();
                                   const Vector3f8 vj_avx =
                                           convertVec_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j],
                                                           &fm_neighbor->getVelocity(0), count);
    ...
    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
        forall_density_maps(Vector3r vj; bm_neighbor->getPointVelocity(xi, vj); delta -= (vi - vj).dot(gradRho););
    } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
        forall_volume_maps(Vector3r vj; bm_neighbor->getPointVelocity(xj, vj);
...
void TimeStepDFcomputeDensityChange(const unsigned int fluidModelIndex, const unsigned int i, const Real h) {
    Simulation *sim = Simulation::getCurrent();
    FluidModel *model = sim->getFluidModel(fluidModelIndex);
    const Vector3r &xi = model->getPosition(i);
    const Vector3r &vi = model->getVelocity(i);
    ...
    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
        forall_density_maps(Vector3r vj; bm_neighbor->getPointVelocity(xi, vj); densityAdv -= (vi - vj).dot(gradRho););
    } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
        forall_volume_maps(Vector3r vj; bm_neighbor->getPointVelocity(xj, vj);
    ...
    if (!sim->is2DSimulation()) {
        if (numNeighbors < 20) densityAdv = 0.0;
    } else {
        if (numNeighbors < 7) densityAdv = 0.0;
    }
}
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors(const Vector3r grad_p_j =
                                           -fm_neighbor->getVolume(neighborIndex) * sim->gradW(xi - xj);
                                   sum_grad_p_k += grad_p_j.squaredNorm(); grad_p_i -= grad_p_j;)

                    //////////////////////////////////////////////////////////////////////////
                    // Boundary
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(const Vector3r grad_p_j =
                                                  -bm_neighbor->getVolume(neighborIndex) * sim->gradW(xi - xj);
                                          grad_p_i -= grad_p_j;)
            }
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            // if (m_simulationData.getDensityAdv(fluidModelIndex, i) > 1.0)
            {
                ...
                forall_fluid_neighbors(
                        ...
                        const Real kSum = (ki + fm_neighbor->getDensity0() / density0 * kj); if (fabs(kSum) > m_eps) {
                    ...
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                        forall_boundary_neighbors(
                                ...
                                const Vector3r velChange =
                                        -h * (Real)1.0 * ki * grad_p_j;  // kj already contains inverse density
                                vel += velChange; bm_neighbor->addForce(xj, -model->getMass(i) * velChange * invH);)
                    } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                        forall_density_maps(const Vector3r velChange = -h * (Real)1.0 * ki *
                                                                       gradRho;  // kj already contains inverse density
                                            vel += velChange;
                                            bm_neighbor->addForce(xj, -model->getMass(i) * velChange * invH);)
                    } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                        forall_volume_maps(const Vector3r grad_p_j = -Vj * sim->gradW(xi - xj);
                                           const Vector3r velChange = -h * (Real)1.0 * ki *
                                                                      grad_p_j;  // kj already contains inverse density
                                           vel += velChange;
            ...
            forall_fluid_neighbors(
                ...
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors(
                            ...
                            const Vector3r velChange =
                                    -h * (Real)1.0 * ki * grad_p_j;  // kj already contains inverse density
                            v_i += velChange; bm_neighbor->addForce(xj, -model->getMass(i) * velChange * invH);)
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                    forall_density_maps(const Vector3r velChange =
                                                -h * (Real)1.0 * ki * gradRho;  // kj already contains inverse density
                                        v_i += velChange;
                                        bm_neighbor->addForce(xj, -model->getMass(i) * velChange * invH);)
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                    forall_volume_maps(const Vector3r grad_p_j = -Vj * sim->gradW(xi - xj);
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            // m_simulationData.getKappaV(fluidModelIndex, i) =
            // static_cast<Real>(0.5)*max(m_simulationData.getKappaV(fluidModelIndex, i)*invH, -static_cast<Real>(0.5) *
            // density0*density0);
            computeDensityChange(fluidModelIndex, i, h);
            if (m_simulationData.getDensityAdv(fluidModelIndex, i) > 0.0)
                m_simulationData.getKappaV(fluidModelIndex, i) =
...
void TimeStepDFSPH::computeDensityAdv(const unsigned int fluidModelIndex,
                                      const unsigned int i,
                                      const int numParticles,
                                      const Real h,
                                      const Real density0) {
    ...
    // Fluid
    //////////////////////////////////////////////////////////////////////////
    forall_fluid_neighbors(const Vector3r &vj = fm_neighbor->getVelocity(neighborIndex);
            ...
            //////////////////////////////////////////////////////////////////////////
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
        forall_boundary_neighbors(const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex);
...
void TimeStepDFSPH::computeDensityChange(const unsigned int fluidModelIndex, const unsigned int i, const Real h) {
    ...
    forall_fluid_neighbors(const Vector3r &vj = fm_neighbor->getVelocity(neighborIndex);
        ...
        forall_boundary_neighbors(const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex);
                                  densityAdv +=
                                  bm_neighbor->getVolume(neighborIndex) * (vi - vj).dot(sim->gradW(xi - xj));)
    }
    else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
        forall_density_maps(Vector3r vj; bm_neighbor->getPointVelocity(xi, vj); densityAdv -= (vi - vj).dot(gradRho);)
    }
...
void TimeStepDFSPH::resize() { m_simulationData.init(); }
