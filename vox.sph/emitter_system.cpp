...
EmitterSystem::~EmitterSystem() {
    for (auto &m_emitter : m_emitters) delete m_emitter;

    LOGI("Sum of emitted particles: {}", m_numberOfEmittedParticles)
    LOGI("Sum of reused particles: {}", m_numReusedParticles)
}
...
void EmitterSystem::reuseParticles() {
    if (m_reuseParticles) {
        ...
        for (unsigned int i = 0; i < m_model->numActiveParticles(); i++) {
            ...
            if ((x[0] < m_boxMin[0]) || (x[1] < m_boxMin[1]) || (x[2] < m_boxMin[2]) || (x[0] > m_boxMax[0]) ||
                (x[1] > m_boxMax[1]) || (x[2] > m_boxMax[2])) {
                m_reusedParticles.push_back(i);
                m_model->getVelocity(i) *=
                        static_cast<Real>(0.95);  // make particles slow so that they don't influence
                                                  // the CFL condition
...
#pragma omp parallel for schedule(static) default(shared)
    ...
    for (auto &m_emitter : m_emitters) {
        ...
        m_emitter->step(m_reusedParticles, indexReuse, numEmittedParticles);
...
void EmitterSystem::addEmitter(const unsigned int width,
                               const unsigned int height,
                               const Vector3r &pos,
                               const Matrix3r &rotation,
                               const Real velocity,
                               const unsigned int type) {
    m_emitters.push_back(new Emitter(m_model, width, height, pos, rotation, velocity, type));
}

void EmitterSystem::enableReuseParticles(const Vector3r &boxMin /*= Vector3r(-1, -1, -1)*/,
...
void EmitterSystem::disableReuseParticles() { m_reuseParticles = false; }

void EmitterSystem::saveState(BinaryFileWriter &binWriter) {
    binWriter.write(m_numberOfEmittedParticles);
    binWriter.write(m_numReusedParticles);
    for (auto &m_emitter : m_emitters) m_emitter->saveState(binWriter);
}

void EmitterSystem::loadState(BinaryFileReader &binReader) {
    binReader.read(m_numberOfEmittedParticles);
    binReader.read(m_numReusedParticles);
    for (auto &m_emitter : m_emitters) m_emitter->loadState(binReader);
}
