...
TimeStepPBF::TimeStepPBF() : TimeStep() {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) {
        ...
        model->addField({"lambda", FieldType::Scalar, [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getLambda(fluidModelIndex, i);
                         }});
        model->addField({"deltaX", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getDeltaX(fluidModelIndex, i)[0];
                         }});
    }
}
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)model->numActiveParticles(); i++) {
                m_simulationData.getLastPosition(fluidModelIndex, i) =
                        m_simulationData.getOldPosition(fluidModelIndex, i);
                m_simulationData.getOldPosition(fluidModelIndex, i) = model->getPosition(i);
                if (model->getParticleState(i) == ParticleState::Active)
                    TimeIntegration::semiImplicitEuler(h, model->getMass(i), model->getPosition(i),
                                                       model->getVelocity(i), model->getAcceleration(i));
            }
        }
    }
...
#pragma omp for schedule(static)
                for (int i = 0; i < (int)model->numActiveParticles(); i++) {
                    if (model->getParticleState(i) == ParticleState::Active)
                        TimeIntegration::velocityUpdateFirstOrder(h, model->getMass(i), model->getPosition(i),
                                                                  m_simulationData.getOldPosition(fluidModelIndex, i),
                                                                  model->getVelocity(i));
...
#pragma omp for schedule(static)
                for (int i = 0; i < (int)model->numActiveParticles(); i++) {
                    if (model->getParticleState(i) == ParticleState::Active)
                        TimeIntegration::velocityUpdateSecondOrder(h, model->getMass(i), model->getPosition(i),
                                                                   m_simulationData.getOldPosition(fluidModelIndex, i),
                                                                   m_simulationData.getLastPosition(fluidModelIndex, i),
                                                                   model->getVelocity(i));
                    model->getAcceleration(i).setZero();
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors(density += fm_neighbor->getVolume(neighborIndex) * sim->W(xi - xj);)

                    //////////////////////////////////////////////////////////////////////////
                    // Boundary
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(density += bm_neighbor->getVolume(neighborIndex) * sim->W(xi - xj);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
...
#pragma omp atomic
            ...
            const Real C = std::max(density - static_cast<Real>(1.0),
            ...
            if (C != 0.0) {
                ...
                // Fluid
                //////////////////////////////////////////////////////////////////////////
                forall_fluid_neighbors(const Vector3r gradC_j =
                                               -fm_neighbor->getVolume(neighborIndex) * sim->gradW(xi - xj);
                                       sum_grad_C2 += gradC_j.squaredNorm(); gradC_i -= gradC_j;)
                    ...
                    forall_density_maps(gradC_i -= gradRho;)
                }
                else  // Bender2019
                {
                    forall_volume_maps(const Vector3r gradC_j = -Vj * sim->gradW(xi - xj); gradC_i -= gradC_j;)
                }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(
                    ...
                    corr -= (m_simulationData.getLambda(fluidModelIndex, i) +
                             (fm_neighbor->getDensity0() / density0) * m_simulationData.getLambda(pid, neighborIndex)) *
                            gradC_j;)
                ...
                forall_boundary_neighbors(
                        // Boundary: Akinci2012
                        const Vector3r gradC_j = -bm_neighbor->getVolume(neighborIndex) * sim->gradW(xi - xj);
                        const Vector3r dx = m_simulationData.getLambda(fluidModelIndex, i) * gradC_j; corr -= dx;

                        bm_neighbor->addForce(xj, model->getMass(i) * dx * invH2);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(const Vector3r dx = m_simulationData.getLambda(fluidModelIndex, i) * gradRho;
                                    corr -= dx; bm_neighbor->addForce(xj, model->getMass(i) * dx * invH2);)
            }
            else  // Bender2019
            {
                forall_volume_maps(const Vector3r gradC_j = -Vj * sim->gradW(xi - xj);
...
void TimeStepPBF::resize() { m_simulationData.init(); }
