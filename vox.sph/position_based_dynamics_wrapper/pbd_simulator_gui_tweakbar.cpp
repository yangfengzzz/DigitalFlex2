...
#include "GUI/TweakBar/tweakbar_parameters.h"
...
PBD_Simulator_GUI_TweakBar::~PBD_Simulator_GUI_TweakBar(void) { delete m_shader; }
...
Shader *PBD_Simulator_GUI_TweakBar::createShader(const std::string &vertexShader,
                                                 const std::string &geometryShader,
                                                 const std::string &fragmentShader) {
    if (MiniGL::checkOpenGLVersion(3, 3)) {
        Shader *shader = new Shader();

        if (vertexShader != "") shader->compileShaderFile(GL_VERTEX_SHADER, vertexShader);
        if (geometryShader != "") shader->compileShaderFile(GL_GEOMETRY_SHADER, geometryShader);
        if (fragmentShader != "") shader->compileShaderFile(GL_FRAGMENT_SHADER, fragmentShader);
...
void PBD_Simulator_GUI_TweakBar::initShader() {
    ...
    std::string vertFile = utility::FileSystem::normalizePath(exePath + "/resources/pbd_shaders/vs_smooth.glsl");
    std::string fragFile = utility::FileSystem::normalizePath(exePath + "/resources/pbd_shaders/fs_smooth.glsl");
...
void PBD_Simulator_GUI_TweakBar::renderBVH() {
    if (m_drawAABB || (m_drawBVHDepth >= 0)) {
        ...
        for (unsigned int k = 0; k < collisionObjects.size(); k++) {
            if (m_drawAABB) renderAABB(collisionObjects[k]->m_aabb);

            if (m_drawBVHDepth >= 0) {
                if (cd.isDistanceFieldCollisionObject(collisionObjects[k])) {
                    const PBD::PointCloudBSH &bvh =
                            ((PBD::DistanceFieldCollisionDetection::DistanceFieldCollisionObject *)collisionObjects[k])
                                    ->m_bvh;

                    std::function<bool(unsigned int, unsigned int)> predicate =
                            [&](unsigned int node_index, unsigned int depth) { return (int)depth <= m_drawBVHDepth; };
                    std::function<void(unsigned int, unsigned int)> cb = [&](unsigned int node_index,
                        ...
                        if (depth == m_drawBVHDepth) {
                            ...
                            if (collisionObjects[k]->m_bodyType ==
...
void PBD_Simulator_GUI_TweakBar::renderSDF(PBD::CollisionDetection::CollisionObject *co) {
    ...
    if ((!cd.isDistanceFieldCollisionObject(co)) ||
        (co->m_bodyType != PBD::CollisionDetection::CollisionObject::RigidBodyCollisionObjectType))
        return;
    ...
    PBD::DistanceFieldCollisionDetection::DistanceFieldCollisionObject *dfco =
            (PBD::DistanceFieldCollisionDetection::DistanceFieldCollisionObject *)co;
...
void PBD_Simulator_GUI_TweakBar::renderTriangleModels() {
    ...
    for (unsigned int i = 0; i < model.getTriangleModels().size(); i++) {
        // mesh
        const utility::IndexedFaceMesh &mesh = model.getTriangleModels()[i]->getParticleMesh();
...
void PBD_Simulator_GUI_TweakBar::renderTetModels() {
    ...
    for (unsigned int i = 0; i < model.getTetModels().size(); i++) {
        ...
        if (vdVis.size() > 0) {
            const utility::IndexedFaceMesh &visMesh = model.getTetModels()[i]->getVisMesh();
            drawMesh(vdVis, visMesh, 0, surfaceColor);
        } else {
            const utility::IndexedFaceMesh &surfaceMesh = model.getTetModels()[i]->getSurfaceMesh();
...
void PBD_Simulator_GUI_TweakBar::renderConstraints() {
    ...
    PBD::SimulationModel::ParticleRigidBodyContactConstraintVector &particleRigidBodyContacts =
            model.getParticleRigidBodyContactConstraints();

    for (unsigned int i = 0; i < rigidBodyContacts.size(); i++) renderRigidBodyContact(rigidBodyContacts[i]);
...
void PBD_Simulator_GUI_TweakBar::initSimulationParameterGUI() {
    ...
    TwAddVarRW(getTweakBar(), "RenderDistanceFields", TW_TYPE_BOOLCPP, &m_drawSDF,
               " label='Render distance fields' group=PBD ");
    TwAddVarCB(getTweakBar(), "DampingCoeff", TW_TYPE_REAL, setDampingCoeff, getDampingCoeff, m_pbdWrapper,
               " label='Damping' group=PBD ");
    TwType enumType = TwDefineEnum("VelocityUpdateMethodType", NULL, 0);
    TwAddVarCB(getTweakBar(), "VelocityUpdateMethod", enumType, setVelocityUpdateMethod, getVelocityUpdateMethod,
               &timeStep,
               " label='Velocity update method' enum='0 {First Order Update}, 1 {Second Order Update}' group=PBD");
    TwAddVarCB(getTweakBar(), "MaxIter", TW_TYPE_UINT32, setMaxIterations, getMaxIterations, &timeStep,
               " label='Max. iterations'  min=1 step=1 group=PBD ");
    TwAddVarCB(getTweakBar(), "MaxIterV", TW_TYPE_UINT32, setMaxIterationsV, getMaxIterationsV, &timeStep,
               " label='Max. iterations Vel.'  min=1 step=1 group=PBD ");
    TwAddVarCB(getTweakBar(), "ContactTolerance", TW_TYPE_REAL, setContactTolerance, getContactTolerance, &cd,
               " label='Contact tolerance'  step=0.001 precision=3 group=PBD ");
    TwAddVarCB(getTweakBar(), "ContactStiffnessRigidBody", TW_TYPE_REAL, setContactStiffnessRigidBody,
               getContactStiffnessRigidBody, &model,
               " label='Contact stiffness RB'  min=0.0 step=0.1 precision=2 group=PBD ");
    TwAddVarCB(getTweakBar(), "ContactStiffnessParticleRigidBody", TW_TYPE_REAL, setContactStiffnessParticleRigidBody,
               getContactStiffnessParticleRigidBody, &model,
               " label='Contact stiffness Particle-RB'  min=0.0 step=0.1 precision=2 group=PBD ");

    TwType enumType2 = TwDefineEnum("ClothSimulationMethodType", NULL, 0);
    TwAddVarCB(getTweakBar(), "ClothSimulationMethod", enumType2, setClothSimulationMethod, getClothSimulationMethod,
               m_pbdWrapper,
               " label='Cloth sim. method' enum='0 {None}, 1 {Distance constraints}, 2 {FEM based PBD}, 3 {Strain "
               "based dynamics}' group=PBD");
    TwType enumType3 = TwDefineEnum("SolidSimulationMethodType", NULL, 0);
    TwAddVarCB(getTweakBar(), "SolidSimulationMethod", enumType3, setSolidSimulationMethod, getSolidSimulationMethod,
               m_pbdWrapper,
               " label='Solid sim. method' enum='0 {None}, 1 {Volume constraints}, 2 {FEM based PBD}, 3 {Strain based "
               "dynamics (no inversion handling)}, 4 {Shape matching (no inversion handling)}' group=PBD");
    TwAddVarCB(getTweakBar(), "Stiffness", TW_TYPE_REAL, setStiffness, getStiffness, &model,
               " label='Stiffness'  min=0.0 step=0.1 precision=4 group='Distance constraints' ");
    TwAddVarCB(getTweakBar(), "XXStiffness", TW_TYPE_REAL, setXXStiffness, getXXStiffness, &model,
               " label='Stiffness XX'  min=0.0 step=0.1 precision=4 group='Strain based dynamics' ");
    TwAddVarCB(getTweakBar(), "YYStiffness", TW_TYPE_REAL, setYYStiffness, getYYStiffness, &model,
               " label='Stiffness YY'  min=0.0 step=0.1 precision=4 group='Strain based dynamics' ");
    TwAddVarCB(getTweakBar(), "XYStiffness", TW_TYPE_REAL, setXYStiffness, getXYStiffness, &model,
               " label='Stiffness XY'  min=0.0 step=0.1 precision=4 group='Strain based dynamics' ");
    TwAddVarCB(getTweakBar(), "XXStiffnessFEM", TW_TYPE_REAL, setXXStiffness, getXXStiffness, &model,
               " label='Youngs modulus XX'  min=0.0 step=0.1 precision=4 group='FEM based PBD' ");
    TwAddVarCB(getTweakBar(), "YYStiffnessFEM", TW_TYPE_REAL, setYYStiffness, getYYStiffness, &model,
               " label='Youngs modulus YY'  min=0.0 step=0.1 precision=4 group='FEM based PBD' ");
    TwAddVarCB(getTweakBar(), "XYStiffnessFEM", TW_TYPE_REAL, setXYStiffness, getXYStiffness, &model,
               " label='Youngs modulus XY'  min=0.0 step=0.1 precision=4 group='FEM based PBD' ");
    TwAddVarCB(getTweakBar(), "XYPoissonRatioFEM", TW_TYPE_REAL, setXYPoissonRatio, getXYPoissonRatio, &model,
               " label='Poisson ratio XY'  min=0.0 step=0.1 precision=4 group='FEM based PBD' ");
    TwAddVarCB(getTweakBar(), "YXPoissonRatioFEM", TW_TYPE_REAL, setYXPoissonRatio, getYXPoissonRatio, &model,
               " label='Poisson ratio YX'  min=0.0 step=0.1 precision=4 group='FEM based PBD' ");
    TwAddVarCB(getTweakBar(), "NormalizeStretch", TW_TYPE_BOOL32, setNormalizeStretch, getNormalizeStretch, &model,
               " label='Normalize stretch' group='Strain based dynamics' ");
    TwAddVarCB(getTweakBar(), "NormalizeShear", TW_TYPE_BOOL32, setNormalizeShear, getNormalizeShear, &model,
               " label='Normalize shear' group='Strain based dynamics' ");
    TwType enumType4 = TwDefineEnum("BendingMethodType", NULL, 0);
    TwAddVarCB(getTweakBar(), "BendingMethod", enumType4, setBendingMethod, getBendingMethod, m_pbdWrapper,
               " label='Bending method' enum='0 {None}, 1 {Dihedral angle}, 2 {Isometric bending}' group=Bending");
    TwAddVarCB(getTweakBar(), "BendingStiffness", TW_TYPE_REAL, setBendingStiffness, getBendingStiffness, &model,
               " label='Bending stiffness'  min=0.0 step=0.01 precision=4 group=Bending ");
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getVelocityUpdateMethod(void *value, void *clientData) {
    *(short *)(value) = (short)((PBD::TimeStepController *)clientData)
                                ->getValue<int>(PBD::TimeStepController::VELOCITY_UPDATE_METHOD);
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getMaxIterations(void *value, void *clientData) {
    *(unsigned int *)(value) =
            ((PBD::TimeStepController *)clientData)->getValue<unsigned int>(PBD::TimeStepController::MAX_ITERATIONS);
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getMaxIterationsV(void *value, void *clientData) {
    *(unsigned int *)(value) =
            ((PBD::TimeStepController *)clientData)->getValue<unsigned int>(PBD::TimeStepController::MAX_ITERATIONS_V);
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getYXPoissonRatio(void *value, void *clientData) {
    *(Real *)(value) =
            ((PBD::SimulationModel *)clientData)->getValue<Real>(PBD::SimulationModel::CLOTH_POISSON_RATIO_YX);
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getXYPoissonRatio(void *value, void *clientData) {
    *(Real *)(value) =
            ((PBD::SimulationModel *)clientData)->getValue<Real>(PBD::SimulationModel::CLOTH_POISSON_RATIO_XY);
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getNormalizeStretch(void *value, void *clientData) {
    *(bool *)(value) =
            ((PBD::SimulationModel *)clientData)->getValue<unsigned int>(PBD::SimulationModel::CLOTH_NORMALIZE_STRETCH);
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getNormalizeShear(void *value, void *clientData) {
    *(bool *)(value) =
            ((PBD::SimulationModel *)clientData)->getValue<unsigned int>(PBD::SimulationModel::CLOTH_NORMALIZE_SHEAR);
}
...
void TW_CALL PBD_Simulator_GUI_TweakBar::getBendingStiffness(void *value, void *clientData) {
    *(Real *)(value) =
            ((PBD::SimulationModel *)clientData)->getValue<Real>(PBD::SimulationModel::CLOTH_BENDING_STIFFNESS);
}
