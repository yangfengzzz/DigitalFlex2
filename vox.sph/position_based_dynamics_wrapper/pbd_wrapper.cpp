            ...
            if (rbd.m_collisionObjectType == utility::SceneLoader::SDF) {
                if (rbd.m_collisionObjectFileName == "") {
                    ...
                    if (foundCacheFile && md5) {
                        ...
                        LOG_INFO << "Set SDF resolution: " << rbd.m_resolutionSDF[0] << ", " << rbd.m_resolutionSDF[1]
        ...
        switch (rbd.m_collisionObjectType) {
            ...
            case utility::SceneLoader::SDF: {
                if (rbd.m_collisionObjectFileName == "") {
                    ...
                    const string sdfFileName = utility::FileSystem::normalizePath(cachePath + "/" + modelFileName +
                                                                                  "_" + resStr + ".csdf");
                    m_cd.addCubicSDFCollisionObject(i,
                                                    CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                                    &(*vertices)[0], nVert, distanceFields[sdfFileName],
                                                    rbd.m_collisionObjectScale, rbd.m_testMesh, rbd.m_invertSDF);
                } else
                    m_cd.addCubicSDFCollisionObject(
                            i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType, &(*vertices)[0],
                            nVert, distanceFields[rbd.m_collisionObjectFileName], rbd.m_collisionObjectScale,
                            rbd.m_testMesh, rbd.m_invertSDF);
                break;
            }
        }
...
void PBDWrapper::initTetModelConstraints() {
    ...
    for (unsigned int cm = 0; cm < m_model.getTetModels().size(); cm++) {
        ...
        const utility::IndexedTetMesh::VertexTets *vTets =
