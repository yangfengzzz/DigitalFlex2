...
Vector3r Emitter::getSize(const Real width, const Real height, const int type) {
    ...
    if (type == 0) {
        if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
            size = {2 * supportRadius, height * diam + 2 * animationMarginAround,
                    width * diam + 2 * animationMarginAround};
        } else {
            size = {static_cast<Real>(2.0) * supportRadius,
...
void Emitter::emitParticles(std::vector<unsigned int> &reusedParticles,
    ...
    if (t < m_emitStartTime || t > m_emitEndTime) emitVel = emitDir * radius * 10 / 0.25;
...
#pragma omp parallel for schedule(static) default(shared)
    ...
    // t-m_nextEmitTime is the time that has passed between the time the particle has been emitted and the current time
    // step. timeStepSize is added because emission happens at the end of the time step, but the particles are not
    // animated anymore.
    ...
    if ((m_model->numActiveParticles() < m_model->numParticles()) || (!reusedParticles.empty())) {
        ...
        for (unsigned int i = 0; i < m_width; i++) {
            for (unsigned int j = 0; j < m_height; j++) {
                ...
                if (index < m_model->numParticles()) {
                    m_model->getPosition(index) =
                            (i * diam + startX) * axisWidth + (j * diam + startZ) * axisHeight + offset;
...
void Emitter::emitParticlesCircle(std::vector<unsigned int> &reusedParticles,
                                  unsigned int &indexReuse,
                                  unsigned int &numEmittedParticles) {
    TimeManager *tm = TimeManager::getCurrent();
    const Real t = tm->getTime();
    ...
    // shortly before the emitter starts, cleanup the emitter from particles
    if (t < m_emitStartTime || t > m_emitEndTime) emitVel = emitDir * particleRadius * 10 / 0.25;
...
void Emitter::step(std::vector<unsigned int> &reusedParticles,
                   unsigned int &indexReuse,
                   unsigned int &numEmittedParticles) {
    if (m_type == 1)
...
void Emitter::saveState(BinaryFileWriter &binWriter) const {
    binWriter.write(m_nextEmitTime);
    binWriter.write(m_emitCounter);
}

void Emitter::loadState(BinaryFileReader &binReader) {
