...
TimeStepIISPH::TimeStepIISPH() : TimeStep() {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) {
        ...
        model->addField({"a_ii", FieldType::Scalar, [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getAii(fluidModelIndex, i);
                         }});
        model->addField({"d_ii", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getDii(fluidModelIndex, i)[0];
                         }});
        model->addField({"d_ij*p_j", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getDij_pj(fluidModelIndex, i)[0];
                         }});
        model->addField({"pressure", FieldType::Scalar,
                         [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getPressure(fluidModelIndex, i);
                         },
                         true});
        // model->addField({ "last pressure", FieldType::Scalar, [this, fluidModelIndex](const unsigned int i) -> Real*
        // { return &m_simulationData.getLastPressure(fluidModelIndex, i); } });
        model->addField(
        ...
        model->addField(
                {"pressure acceleration", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real * {
...
void TimeStepIISPH::step() {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) integration(fluidModelIndex);
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            if (model->getParticleState(i) == ParticleState::Active) vel += h * accel;
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors(dii -= fm_neighbor->getVolume(neighborIndex) / density2 * sim->gradW(xi - xj);)

                    //////////////////////////////////////////////////////////////////////////
                    // Boundary
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(dii -=
                                          bm_neighbor->getVolume(neighborIndex) / density2 * sim->gradW(xi - xj);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(dii += 1.0 / density2 * gradRho;)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(dii -= Vj / density2 * sim->gradW(xi - xj);)
            }
        }
    }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(const Vector3r &vj = fm_neighbor->getVelocity(neighborIndex);
                                   densityAdv +=
                                   h * fm_neighbor->getVolume(neighborIndex) * (vi - vj).dot(sim->gradW(xi - xj));)
                ...
                forall_boundary_neighbors(const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex);
                                          densityAdv += h * bm_neighbor->getVolume(neighborIndex) *
                                                        (vi - vj).dot(sim->gradW(xi - xj));)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
            ...
            forall_fluid_neighbors(
                    // Compute d_ji
                    const Vector3r kernel = sim->gradW(xi - xj); const Vector3r dji = dpi * kernel;
                    ...
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(const Vector3r kernel = sim->gradW(xi - xj);
                                          const Vector3r dji = dpi * kernel;
                                          aii += bm_neighbor->getVolume(neighborIndex) * (dii - dji).dot(kernel);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(const Vector3r dji = -(1.0 / density2) * gradRho; aii -= (dii - dji).dot(gradRho);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(const Vector3r kernel = sim->gradW(xi - xj); const Vector3r dji = dpi * kernel;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(
                    ...
                    dij_pj -= fm_neighbor->getVolume(neighborIndex) / densityj2 *
                              m_simulationData.getLastPressure(pid, neighborIndex) * sim->gradW(xi - xj);)
        }
    }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors(const Vector3r &d_jk_pk = m_simulationData.getDij_pj(pid, neighborIndex);
                                   ...
                                   sum += fm_neighbor->getVolume(neighborIndex) *
                                          (m_simulationData.getDij_pj(fluidModelIndex, i) -
                                           m_simulationData.getDii(pid, neighborIndex) *
                                                   m_simulationData.getLastPressure(pid, neighborIndex) -
                                           (d_jk_pk - d_ji_pi))
                                                  .dot(kernel);)
                    ...
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(sum +=
                                          bm_neighbor->getVolume(neighborIndex) *
                                          m_simulationData.getDij_pj(fluidModelIndex, i).dot(sim->gradW(xi - xj));)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(sum -= m_simulationData.getDij_pj(fluidModelIndex, i).dot(gradRho);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(sum += Vj * m_simulationData.getDij_pj(fluidModelIndex, i).dot(sim->gradW(xi - xj));)
            }
            ...
            if (fabs(denom) > 1.0e-9)
                pi = max((static_cast<Real>(1.0) - omega) * lastPi + omega / denom * (b - h2 * sum),
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(
                    ...
                    ai -= fm_neighbor->getVolume(neighborIndex) * (dpi + fm_neighbor->getDensity0() / density0 * dpj) *
                          sim->gradW(xi - xj);)
                    ...
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(const Vector3r a =
                                                  bm_neighbor->getVolume(neighborIndex) * dpi * sim->gradW(xi - xj);
                                          ai -= a; bm_neighbor->addForce(xj, model->getMass(i) * a);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(const Vector3r a = -dpi * gradRho; ai -= a;
                                    bm_neighbor->addForce(xj, model->getMass(i) * a);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(const Vector3r a = Vj * dpi * sim->gradW(xi - xj); ai -= a;
                                   bm_neighbor->addForce(xj, model->getMass(i) * a);)
            }
        }
    }
...
void TimeStepIISPH::resize() { m_simulationData.init(); }
