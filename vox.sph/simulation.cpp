...
Simulation::~Simulation() {
    ...
    for (auto &m_fluidModel : m_fluidModels) delete m_fluidModel;
    m_fluidModels.clear();

    for (auto &m_boundaryModel : m_boundaryModels) delete m_boundaryModel;
...
void Simulation::initParameters() {
    ...
    auto *enumParam = static_cast<EnumParameter *>(getParameter(CFL_METHOD));
    ...
    CFL_MIN_TIMESTEPSIZE =
            createNumericParameter("cflMinTimeStepSize", "CFL - min. time step size", &m_cflMinTimeStepSize);
    ...
    CFL_MAX_TIMESTEPSIZE =
            createNumericParameter("cflMaxTimeStepSize", "CFL - max. time step size", &m_cflMaxTimeStepSize);
    ...
    ParameterBase::SetFunc<int> setSimulationFct =
            std::bind(&Simulation::setSimulationMethod, this, std::placeholders::_1);
    SIMULATION_METHOD =
            createEnumParameter("simulationMethod", "Simulation method", getSimulationFct, setSimulationFct);
    ...
    BOUNDARY_HANDLING_METHOD =
            createEnumParameter("boundaryHandlingMethod", "Boundary handling method", &m_boundaryHandlingMethod);
...
void Simulation::initKernels() const {
    // init kernel
...
void Simulation::setGradKernel(int val) {
    ...
    if (!m_sim2D) {
        if ((m_gradKernelMethod < 0) || (m_gradKernelMethod > 4)) m_gradKernelMethod = 0;
        ...
        if ((m_gradKernelMethod < 0) || (m_gradKernelMethod > 1)) m_gradKernelMethod = 0;
...
void Simulation::setKernel(int val) {
    if (val == m_kernelMethod) return;

    m_kernelMethod = val;
    if (!m_sim2D) {
        if ((m_kernelMethod < 0) || (m_kernelMethod > 4)) m_kernelMethod = 0;
        ...
        if ((m_kernelMethod < 0) || (m_kernelMethod > 1)) m_kernelMethod = 0;
    ...
    if (getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) updateBoundaryVolume();
}
...
void Simulation::updateTimeStepSize() {
    if (m_cflMethod == 1)
        ...
        updateTimeStepSizeCFL();
        const auto iterations = m_timeStep->getValue<unsigned int>(TimeStep::SOLVER_ITERATIONS);
        if (iterations > 10)
...
void Simulation::updateTimeStepSizeCFL() {
    ...
    if (getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
        for (unsigned int i = 0; i < numberOfBoundaryModels(); i++) {
            auto *bm = static_cast<BoundaryModel_Akinci2012 *>(getBoundaryModel(i));
        ...
        for (unsigned int boundaryModelIndex = 0; boundaryModelIndex < numberOfBoundaryModels(); boundaryModelIndex++) {
            auto *bm = static_cast<BoundaryModel_Koschier2017 *>(getBoundaryModel(boundaryModelIndex));
            if (bm->getRigidBodyObject()->isDynamic() || bm->getRigidBodyObject()->isAnimated()) {
        ...
        for (unsigned int boundaryModelIndex = 0; boundaryModelIndex < numberOfBoundaryModels(); boundaryModelIndex++) {
            auto *bm = static_cast<BoundaryModel_Bender2019 *>(getBoundaryModel(boundaryModelIndex));
            if (bm->getRigidBodyObject()->isDynamic() || bm->getRigidBodyObject()->isAnimated()) {
                maxVel = std::max(maxVel, bm->getMaxVel());
            }
        }
...
void Simulation::reset() {
    // reset fluid models
    for (unsigned int i = 0; i < numberOfFluidModels(); i++) getFluidModel(i)->reset();

    // reset boundary models
    for (unsigned int i = 0; i < numberOfBoundaryModels(); i++) getBoundaryModel(i)->reset();
...
void Simulation::setSimulationMethod(const int val) {
    ...
    if (m_simulationMethodChanged != nullptr) m_simulationMethodChanged();
}
...
void Simulation::setSimulationInitialized(int val) {
    ...
    if (m_simulationIsInitialized) deferredInit();
}
...
void Simulation::addBoundaryModel(BoundaryModel *bm) { m_boundaryModels.push_back(bm); }

void Simulation::addFluidModel(const std::string &id,
...
void Simulation::updateBoundaryVolume() {
    if (m_neighborhoodSearch == nullptr) return;
    ...
    for (unsigned int body = 0; body < numberOfBoundaryModels(); body++) {
        if (!getBoundaryModel(body)->getRigidBodyObject()->isDynamic() &&
            !getBoundaryModel(body)->getRigidBodyObject()->isAnimated())
    ...
    m_neighborhoodSearch->set_active(false);
    for (unsigned int i = 0; i < numberOfFluidModels(); i++) {
        for (unsigned int j = 0; j < numberOfFluidModels(); j++) m_neighborhoodSearch->set_active(i, j, true);
        for (unsigned int j = numberOfFluidModels(); j < m_neighborhoodSearch->point_sets().size(); j++)
            m_neighborhoodSearch->set_active(i, j, true);
    }
}

void Simulation::saveState(BinaryFileWriter &binWriter) {
    binWriter.write(m_W_zero);
    for (unsigned int i = 0; i < numberOfFluidModels(); i++) getFluidModel(i)->saveState(binWriter);
    for (unsigned int i = 0; i < numberOfBoundaryModels(); i++) getBoundaryModel(i)->saveState(binWriter);
    m_timeStep->saveState(binWriter);
}

void Simulation::loadState(BinaryFileReader &binReader) {
    binReader.read(m_W_zero);
    for (unsigned int i = 0; i < numberOfFluidModels(); i++) getFluidModel(i)->loadState(binReader);
    for (unsigned int i = 0; i < numberOfBoundaryModels(); i++) getBoundaryModel(i)->loadState(binReader);
