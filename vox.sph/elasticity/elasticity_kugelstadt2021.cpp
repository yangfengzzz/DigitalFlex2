...
Elasticity_Kugelstadt2021::Elasticity_Kugelstadt2021(FluidModel* model) : ElasticityBase(model) {
    ...
    m_vDiff.resize(numParticles,
                   Vector3r::Zero());  // velocity difference used for the warm start of the volume cg solver
    ...
    model->addField(
            {"rest volume", FieldType::Scalar, [&](const unsigned int i) -> Real* { return &m_restVolumes[i]; }, true});
    model->addField(
    ...
    model->addField(
            {"deformation gradient", FieldType::Matrix3, [&](const unsigned int i) -> Real* { return &m_F[i](0, 0); }});
    model->addField(
            {"correction matrix", FieldType::Matrix3, [&](const unsigned int i) -> Real* { return &m_L[i](0, 0); }});
}
...
void Elasticity_Kugelstadt2021::deferredInit() { initValues(); }
...
void Elasticity_Kugelstadt2021::initParameters() {
    ...
    ParameterBase::SetFunc<Real> setFctYM = [&](Real val) {
        ...
        m_lambda = m_youngsModulus * m_poissonRatio /
                   ((static_cast<Real>(1.0) + m_poissonRatio) *
                    (static_cast<Real>(1.0) - static_cast<Real>(2.0) * m_poissonRatio));

        if (Simulation::getCurrent()
                    ->isSimulationInitialized())  // if Young's modulus has changed, recompute the factorization
    ...
    ParameterBase::SetFunc<Real> setFctPR = [&](Real val) {
        ...
        if (Simulation::getCurrent()
                    ->isSimulationInitialized())  // if Poisson ration has changed, recompute the factorization
            Simulation::getCurrent()->reset();
    };
...
std::string Elasticity_Kugelstadt2021::computeMD5(const unsigned int objIndex) {
    ...
    for (size_t i = 0; i < numParticles; i++) {
        ...
        tempN[2 * i + 1] = 0;
        for (auto j = 0; j < numNeighbors; j++) tempN[2 * i + 1] += m_initialNeighbors[particleIndex][j] - group[0];
    }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            if (m_maxNeighbors > 0) {
                ...
                std::sort(m_initialNeighbors[i].begin(), m_initialNeighbors[i].end(),
                          Comparator(model->getPosition0(i), &model->getPosition0(0)));
...
void Elasticity_Kugelstadt2021::findObjects() {
    ...
    for (size_t groupIndex = 0; groupIndex < m_objects.size(); groupIndex++) {
        struct Comparator {
            ...
            bool operator()(unsigned int a, unsigned int b) {
                if ((m_this->m_model->getParticleState(a) != ParticleState::Active) &&
                    (m_this->m_model->getParticleState(b) == ParticleState::Active))
                    return false;
                else if ((m_this->m_model->getParticleState(a) == ParticleState::Active) &&
                         (m_this->m_model->getParticleState(b) != ParticleState::Active))
                    return true;
                else if ((m_this->m_model->getParticleState(a) != ParticleState::Active) &&
                         (m_this->m_model->getParticleState(b) != ParticleState::Active))
        ...
        std::sort(m_objects[groupIndex]->m_particleIndices.begin(), m_objects[groupIndex]->m_particleIndices.end(),
                  Comparator(this));
        ...
        LOGI("Object {} - fixed particles: {}", groupIndex, m_objects[groupIndex]->m_nFixed)
    }
}
    ...
    for (auto objIndex = 0; objIndex < numObjects; objIndex++) {
        ...
        for (size_t i = 0; i < objIndex; i++) {
            ...
            if (fluidInfos[objIndex].hasSameParticleSampling(fluidInfos[i])) {
                ...
                LOGI("Object {} is using the factorization of object ", objIndex, i)
                break;
            }
        }
        // if no factorization was found, create a new one
        if (obj->m_factorization == nullptr) obj->m_factorization = std::make_shared<Factorization>();

        // generate file name for cache file
        std::string baseName = utility::FileSystem::getFileName(fluidInfos[objIndex].samplesFile);
        std::string ext = utility::FileSystem::getFileExt(fluidInfos[objIndex].samplesFile);
        std::string cacheFileName =
                sim->getCachePath() + "/" + baseName + "_" + ext + "_" + std::to_string(fluidInfos[objIndex].mode) +
                "_" + md5 + "_" + utility::StringTools::real2String(dt) + "_" +
                utility::StringTools::real2String(m_mu) + "_" + utility::StringTools::real2String(m_alpha) + ".bin";

        // Fluid block
        if (fluidInfos[objIndex].type == 0) {
            Vector3r diag = (fluidInfos[objIndex].box.max() - fluidInfos[objIndex].box.min());
            cacheFileName = sim->getCachePath() + "/Block_" + utility::StringTools::real2String(diag.x()) + "_" +
                            utility::StringTools::real2String(diag.y()) + "_" +
                            utility::StringTools::real2String(diag.z()) + "_" +
                            std::to_string(fluidInfos[objIndex].mode) + "_" + md5 + "_" +
                            utility::StringTools::real2String(dt) + "_" + utility::StringTools::real2String(m_mu) +
                            "_" + utility::StringTools::real2String(m_alpha) + ".bin";
        }

        // check if cache file exists
        const bool foundCacheFile = utility::FileSystem::fileExists(cacheFileName);
        ...
        if (sim->getUseCache() && foundCacheFile) {
            ...
            if (!foundFactorization) {
                LOGI("Read cached factorization: {}", cacheFileName)
            ...
            for (int i = 0; i < vecSize; i++) obj->m_quats_avx[i] = Quaternion8f();
#else
        ...
        } else  // no cache found
        {
            ...
            // write cache file
            if (sim->getUseCache() && (utility::FileSystem::makeDir(sim->getCachePath()) == 0)) {
 ...
 */
void Elasticity_Kugelstadt2021::initFactorization(std::shared_ptr<Factorization> factorization,
    ...
    for (int i = 0; i < numParticles; i++) groupInv[group[i]] = i;
    ...
    for (int i = 0; i < (int)numParticles; i++) {
        ...
        for (int j = 0; j < 3; j++) triplets_K.push_back(Eigen::Triplet<double>(3 * i + j, 3 * i + j, Kreal));
        ...
        for (unsigned int j = 0; j < numNeighbors; j++) {
            ...
            const Eigen::Vector3d correctedKernelGradient =
                    m_L[particleIndex].cast<double>() * sim->gradW((xi0d - xj0d).cast<Real>()).cast<double>();
            ...
            const double Kreal2 = alphad * dtd * dtd * mud * static_cast<double>(m_restVolumes[i]) *
                                  static_cast<double>(m_restVolumes[neighborIndex]) *
                                  (sim->W(xi_xj_0.cast<Real>()) / xixj0_l2);
            ...
            for (unsigned int k = 0; k < numNeighbors; k++) {
                ...
                const Eigen::Vector3d correctedKernelGradientd =
                        m_L[particleIndex].cast<double>() * sim->gradW((xi0d - xk0d).cast<Real>()).cast<double>();
    ...
    LOGI("Non zero elements (H^T * K * H): {}", factorization->m_matHTH.nonZeros())
    ...
    if (solverLLT->info() != Eigen::Success) {
        LOGE("Cholesky decomposition failed. {}", solverLLT->info())
        return;
    }
    ...
    LOGI("Non zero elements (A): {}", M_plus_DT_K_D.nonZeros())
}
...
void Elasticity_Kugelstadt2021::reset() { initValues(); }
...
#pragma omp for schedule(static)
            for (int i = 0; i < vecSize; i++) {
                ...
                int idx[8];
                for (int j = 0; j < count; j++) idx[j] = m_initial_to_current_index[group[8 * i + j]];
                for (int j = count; j < 8; j++) idx[j] = 0;

                Quaternionr q[8];
                for (auto j = 0; j < count; j++) q[j] = Quaternionr(m_rotations[idx[j]].transpose());
                for (auto j = count; j < 8; j++) q[j] = Quaternionr();
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)numParticles; i++) {
                ...
                m_F[particleIndex] << x0[0], x0[1], x0[2], x1[0], x1[1], x1[2], x2[0], x2[1], x2[2];
            }
...
#pragma omp for schedule(static)
            for (int i = 0; i < vecSize; i++) {
                ...
                F1 = Vector3f8(m_F[idx[0]].col(0), m_F[idx[1]].col(0), m_F[idx[2]].col(0), m_F[idx[3]].col(0),
                               m_F[idx[4]].col(0), m_F[idx[5]].col(0), m_F[idx[6]].col(0), m_F[idx[7]].col(0));
                F2 = Vector3f8(m_F[idx[0]].col(1), m_F[idx[1]].col(1), m_F[idx[2]].col(1), m_F[idx[3]].col(1),
                               m_F[idx[4]].col(1), m_F[idx[5]].col(1), m_F[idx[6]].col(1), m_F[idx[7]].col(1));
                F3 = Vector3f8(m_F[idx[0]].col(2), m_F[idx[1]].col(2), m_F[idx[2]].col(2), m_F[idx[3]].col(2),
                               m_F[idx[4]].col(2), m_F[idx[5]].col(2), m_F[idx[6]].col(2), m_F[idx[7]].col(2));
...
#pragma omp for schedule(static)
            for (int i = 0; i < vecSize; i++) {
                ...
                for (auto j = 0; j < count; j++) {
                    ...
                    if (8 * i + j < RHS.size()) RHS[8 * i + j] = Scalarf8(0.0f);
                }
            }
        }
        STOP_TIMING_AVG
...
#pragma omp for schedule(static)
            for (int k = 0; k < DT_K.outerSize(); ++k) {
                for (Eigen::SparseMatrix<float, Eigen::RowMajor>::InnerIterator it(DT_K, k); it; ++it) {
                    if (it.row() < (int)RHS.size()) RHS[it.row()] += Scalarf8(it.value()) * f_avx[it.col()];
                }
            }
        }
        ...
        for (int i = 0; i < (int)numParticles; i++) {
            if (model->getParticleState(i) == ParticleState::Active) {
                ...
                for (unsigned int j = 0; j < numNeighbors; j += 8) {
                    ...
                    force_avx += (V_RL_gradWi * (restVolume_i_avx * stress_i_avx) +
                                  V_RL_gradWj * (restVolume_j_avx * stress_j_avx));
                }
...
void Elasticity_Kugelstadt2021::computeF(const unsigned int i, const Real* x, Elasticity_Kugelstadt2021* e) {
    ...
    for (unsigned int j = 0; j < numNeighbors; j += 8) {
        const unsigned int count = std::min(numNeighbors - j, 8u);
        unsigned int nIndices[8];
        for (auto k = 0u; k < count; k++) nIndices[k] = e->m_initial_to_current_index[e->m_initialNeighbors[i0][j + k]];
    ...
    if (Simulation::getCurrent()->is2DSimulation()) e->m_F[i](2, 2) = 1.0;
}
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            if (model->getParticleState(i) == ParticleState::Active) computeF(i, &m_model->getPosition(0)[0], this);
        }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            if (m_model->getParticleState(i) == ParticleState::Active) {
                ...
                const Real trace = m_rotations[i].col(0).dot(m_F[i].col(0)) + m_rotations[i].col(1).dot(m_F[i].col(1)) +
...
#pragma omp for schedule(static)
            for (int k = 0; k < DT_K.outerSize(); ++k) {
                for (Eigen::SparseMatrix<Real, Eigen::RowMajor>::InnerIterator it(DT_K, k); it; ++it) {
                    if (it.row() < (int)RHS.size()) RHS[it.row()] += it.value() * f[it.col()];
                }
            }
        }
...
#pragma omp for schedule(static)
                for (int k = 0; k < HT_K_H.outerSize(); ++k) {
                    for (Eigen::SparseMatrix<Real, Eigen::ColMajor>::InnerIterator it(HT_K_H, k); it; ++it) {
                        if (it.col() < (int)RHS.size()) RHS[it.col()] -= it.value() * sol[it.row()];
                    }
                }
            }
        }
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)RHS.size(); i++) RHS_perm[permInd[i]] = RHS[i];
        }
...
// invert permutation
#pragma omp for schedule(static)
            for (int i = 0; i < (int)RHS.size(); i++) RHS[permInvInd[i]] = RHS_perm[i];
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            if (m_model->getParticleState(i) == ParticleState::Active) {
                ...
                if (sim->is2DSimulation()) m_F[i](2, 2) = 1.0;
...
void Elasticity_Kugelstadt2021::precomputeValues() {
    ...
    for (int i = 0; i < numParticles; i++) {
        ...
        if (numNeighbors % 8 != 0) sumNeighborParticles8++;
#else
