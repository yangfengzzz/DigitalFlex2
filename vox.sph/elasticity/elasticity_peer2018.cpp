...
Elasticity_Peer2018::Elasticity_Peer2018(FluidModel *model) : ElasticityBase(model) {
...
Elasticity_Peer2018::~Elasticity_Peer2018() {
...
void Elasticity_Peer2018::deferredInit() { initValues(); }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            for (unsigned int neighborIndex : m_initialNeighbors[i]) {
...
void Elasticity_Peer2018::reset() { initValues(); }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            if (sim->is2DSimulation()) F(2, 2) = 1.0;
...
void Elasticity_Peer2018::computeRHS(VectorXr &rhs) {
    ...
    Real lambda = m_youngsModulus * m_poissonRatio /
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            for (unsigned int j = 0; j < numNeighbors; j++) {
                ...
                m_F[i] += m_restVolumes[neighborIndex] * (xj_xi - m_rotations[i] * (xj0 - xi0)) *
                          correctedRotatedKernel.transpose();
            }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            if (model->getParticleState(i) == ParticleState::Active)
                rhs.segment<3>(3 * i) =
                        model->getVelocity(i) + dt * (model->getAcceleration(i) + 1.0 / model->getMass(i) * force);
            else
...
void Elasticity_Peer2018::matrixVecProd(const Real *vec, Real *result, void *userData) {
    Simulation *sim = Simulation::getCurrent();
    auto *elasticity = static_cast<Elasticity_Peer2018 *>(userData);
    ...
    Real lambda = youngsModulus * poissonRatio /
                  ((static_cast<Real>(1.0) + poissonRatio) *
        ...
        for (int i = 0; i < (int)numParticles; i++) {
            if (model->getParticleState(i) == ParticleState::Active) {
                ...
                for (unsigned int j = 0; j < numNeighbors; j++) {
                    ...
                    Vector3r PWi, PWj;
                    vox::Elasticity_Peer2018::symMatTimesVec(stress[i], correctedRotatedKernel_i, PWi);
                    vox::Elasticity_Peer2018::symMatTimesVec(stress[neighborIndex], correctedRotatedKernel_j, PWj);
...
void Elasticity_Peer2018::saveState(BinaryFileWriter &binWriter) {
...
void Elasticity_Peer2018::loadState(BinaryFileReader &binReader) {
