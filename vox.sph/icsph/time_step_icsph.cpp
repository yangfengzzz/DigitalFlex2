...
int TimeStepICLAMBDA = -1;
int TimeStepICPRESSURE_CLAMPING = -1;

TimeStepICSPH::TimeStepICSPH() : TimeStep() {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) {
        ...
        model->addField(
                {"pressure gradient", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real * {
                     return &m_simulationData.getPressureGradient(fluidModelIndex, i)[0];
                 }});
    }
}
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors(const Real Vj =
                                           fm_neighbor->getMass(neighborIndex) / fm_neighbor->getDensity(neighborIndex);
                ...
                forall_boundary_neighbors(const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex);
                                          delta +=
                                          bm_neighbor->getVolume(neighborIndex) * (vj - vi).dot(sim->gradW(xi - xj));)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            ...
            forall_fluid_neighbors(
                    ...
                    Vj_gradW += Vj * sim->gradW(xi - xj); Vi_Vj_gradW2 += Vi * Vj * sim->gradW(xi - xj).squaredNorm();)

                    //////////////////////////////////////////////////////////////////////////
                    // Boundary
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(Vb_gradW += bm_neighbor->getVolume(neighborIndex) * sim->gradW(xi - xj);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(Vb_gradW -= gradRho;)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(Vb_gradW += Vj * sim->gradW(xi - xj);)
            }

            Real &aii = m_simulationData.getAii(fluidModelIndex, i);
            aii = -density0 / m_lambda - dt2 * Vi_Vj_gradW2 -
                  dt2 * (Vj_gradW + m_psi * Vb_gradW).dot(Vj_gradW + Vb_gradW);
        }
    }
}
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(const Real Vj =
                    ...
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(grad_pi +=
                                          m_psi * bm_neighbor->getVolume(neighborIndex) * pi * sim->gradW(xi - xj);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(grad_pi -= pi * gradRho;)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(grad_pi += Vj * pi * sim->gradW(xi - xj);)
            }
        }
...
#pragma omp for reduction(+ : density_error) schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(const Real Vj =
                    ...
                    //////////////////////////////////////////////////////////////////////////
                    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors(laplacian_p +=
                                          bm_neighbor->getVolume(neighborIndex) * (-grad_pi).dot(sim->gradW(xi - xj));)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(laplacian_p += grad_pi.dot(gradRho);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(laplacian_p += Vj * (-grad_pi).dot(sim->gradW(xi - xj));)
            }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(
                    ...
                    const Real dpj =
                            (m_simulationData.getPressure(pid, neighborIndex) / fm_neighbor->getDensity0()) / densityj2;
...
void TimeStepICSPH::resize() { m_simulationData.init(); }
