...
PoissonDiskSampling::PoissonDiskSampling() = default;

void PoissonDiskSampling::sampleMesh(const unsigned int numVertices,
                                     const Vector3r *vertices,
                                     const unsigned int numFaces,
                                     const unsigned int *faces,
                                     const Real minRadius,
    ...
    const auto numInitialPoints = (unsigned int)(40.0 * (m_totalArea / circleArea));
...
#pragma omp parallel for schedule(static)
    for (auto &i : m_initialInfoVec) {
        const Vector3r &v = i.pos;
        ...
        i.cP = CellPos(cellPos1, cellPos2, cellPos3);
    }
    ...
    for (auto &m_phaseGroup : m_phaseGroups) {
        m_phaseGroup.clear();
    }
    m_phaseGroups.clear();
}

void PoissonDiskSampling::determineTriangleAreas(const unsigned int numVertices,
                                                 const Vector3r *vertices,
                                                 const unsigned int numFaces,
                                                 const unsigned int *faces) {
...
void PoissonDiskSampling::generateInitialPointSet(const unsigned int numVertices,
                                                  const Vector3r *vertices,
                                                  const unsigned int numFaces,
                                                  const unsigned int *faces) {
    m_totalArea = 0.0;
    ...
    for (int i = 0; i < maxThreads; ++i) generators.emplace_back(default_random_engine(r()));
...
#pragma omp for schedule(static)
        for (auto &i : m_initialInfoVec) {
            ...
            i.pos = bc1 * v1 + bc2 * v2 + bc3 * v3;
            i.ID = randIndex;
        }
    }
}

unsigned int PoissonDiskSampling::getAreaIndex(const vector<Real> &areas,
                                               const Real totalArea,
    ...
    while (notaccepted) {
        ...
        if (distribution(generator) < areas[index] / m_maxArea) notaccepted = false;
    }
    return index;
}
...
void PoissonDiskSampling::parallelUniformSurfaceSampling(std::vector<Vector3r> &samples) {
    ...
    for (int k = 0; k < (int)m_numTrials; k++) {
        ...
        for (auto &cells : m_phaseGroups) {
// Loop over the cells in each cell group
#pragma omp parallel for schedule(static)
            for (auto &cell : cells) {
                const auto entryIt = hMap.find(cell);
...
bool PoissonDiskSampling::nbhConflict(const unordered_map<CellPos, HashEntry, CellPosHasher> &hMap,
                                      const InitialPointInfo &iPI) {
    CellPos nbPos = iPI.cP;

    // check neighboring cells inside to outside
    if (checkCell(hMap, nbPos, iPI)) return true;
...
bool PoissonDiskSampling::checkCell(const unordered_map<CellPos, HashEntry, CellPosHasher> &hMap,
                                    const CellPos &cell,
                                    const InitialPointInfo &iPI) {
    ...
    if (nbEntryIt != hMap.end()) {
        ...
        for (unsigned int sample : nbEntry.samples) {
            const InitialPointInfo &info = m_initialInfoVec[sample];
            Real dist;
...
int PoissonDiskSampling::partition(int left, int right) {
    ...
    while (i <= j) {
        while (compareCellID(m_initialInfoVec[i].cP, pivot)) i++;

        while (compareCellID(pivot, m_initialInfoVec[j].cP)) j--;
...
void PoissonDiskSampling::computeFaceNormals(const unsigned int numVertices,
                                             const Vector3r *vertices,
                                             const unsigned int numFaces,
                                             const unsigned int *faces) {
