...
SimulatorBase::SimulatorBase() {
    utility::logger.addSink(unique_ptr<utility::ConsoleSink>(new utility::ConsoleSink(utility::LogLevel::INFO)));
...
void SimulatorBase::initParameters() {
    ...
    setDescription(STOP_AT,
                   "Stop simulation at the given time. When the value is negative, the simulation is not stopped.");
    ...
    PARTICLE_EXPORT_ATTRIBUTES =
            createStringParameter("particleAttributes", "Export attributes", &m_particleAttributes);
    ...
    EXPORT_OBJECT_SPLITTING =
            createBoolParameter("enableObjectSplitting", "Split SPH objects", &m_enableObjectSplitting);
    setGroup(EXPORT_OBJECT_SPLITTING, "Export");
    setDescription(EXPORT_OBJECT_SPLITTING,
                   "Enable/disable an export of the SPH particles with an indiviual file per object.");
...
void SimulatorBase::init(std::vector<std::string> argv, const std::string &windowName) {
    ...
    for (auto &a : argv) m_argv_vec.push_back(&a[0]);
...
void SimulatorBase::init(int argc, char **argv, const std::string &windowName) {
    ...
    try {
        cxxopts::Options options(
                argv[0], "SPlisHSPlasH - An open-source library for the physically-based simulation of fluids.");
        options.positional_help("[scene file]").show_positional_help();
        ...
        options.add_options()("h,help", "Print help")("v,version", "Print version")(
                ...
                "no-initial-pause", "Disable initial pause when starting the simulation.")("no-gui", "Disable GUI.")(
                "stopAt", "Sets or overwrites the stopAt parameter of the scene.", cxxopts::value<Real>())(
                "param",
                "Sets or overwrites a parameter of the scene.\n\n"
        ...
        if (result.count("param")) {
            ...
            utility::StringTools::tokenize(paramStr, m_paramTokens, ":");
        ...
        if (result.count("scene-file")) {
            ...
            if (!FileSystem::isDirectory(scenePath)) scenePath = m_exePath;
    ...
    utility::logger.addSink(
            unique_ptr<utility::FileSink>(new utility::FileSink(utility::LogLevel::DEBUG, logPath + "/SPH_log.txt")));
...
void SimulatorBase::initSimulation() {
    ...
    for (unsigned int i = 0; i < scene.boundaryModels.size(); i++) {
        ...
        if (mapFileName != "") {
            ...
            FileSystem::copyFile(
                    scene.boundaryModels[i]->mapFile,
                    modelsFilePath + "/" + FileSystem::getFileNameWithExt(scene.boundaryModels[i]->mapFile));
        }
    }
    ...
    for (unsigned int i = 0; i < scene.fluidModels.size(); i++) {
        ...
        if (samplesFileName != "") {
            ...
            FileSystem::copyFile(samplesFileName,
                                 modelsFilePath + "/" + FileSystem::getFileNameWithExt(samplesFileName));
        }
    }
    ...
    if (m_useGUI) m_gui->init(m_argc, m_argv, m_windowName.c_str());
    ...
    if (m_useGUI) {
        ...
        for (unsigned int i = 0; i < sim->numberOfFluidModels(); i++) {
            ...
            model->setSurfaceMethodChangedCallback([this, model]() {
                m_gui->initSimulationParameterGUI();
                getSceneLoader()->readMaterialParameterObject(model->getId(),
                                                              (ParameterObject *)model->getSurfaceTensionBase());
            });
            model->setViscosityMethodChangedCallback([this, model]() {
                m_gui->initSimulationParameterGUI();
                getSceneLoader()->readMaterialParameterObject(model->getId(),
                                                              (ParameterObject *)model->getViscosityBase());
            });
            model->setVorticityMethodChangedCallback([this, model]() {
                m_gui->initSimulationParameterGUI();
                getSceneLoader()->readMaterialParameterObject(model->getId(),
                                                              (ParameterObject *)model->getVorticityBase());
            });
            model->setElasticityMethodChangedCallback([this, model]() {
                reset();
                m_gui->initSimulationParameterGUI();
                getSceneLoader()->readMaterialParameterObject(model->getId(),
                                                              (ParameterObject *)model->getElasticityBase());
            });
        }
...
void SimulatorBase::runSimulation() {
    ...
    if (getStateFile() != "") {
        ...
        if (m_cmdLineStopAt) m_stopAt = temp;
        if (m_cmdLineNoInitialPause) m_doPause = false;
    }
...
void SimulatorBase::setCommandLineParameter() {
    Simulation *sim = Simulation::getCurrent();
    if (m_paramTokens.size() != 3) return;
...
void SimulatorBase::setCommandLineParameter(ParameterObject *paramObj) {
        ...
        if (m_paramTokens[1] == paramBase->getName()) {
            if (paramBase->getType() == RealParameterType) {
                ...
                if (static_cast<VectorParameter<Real> *>(paramBase)->getDim() == 3) {
                    ...
                    utility::StringTools::tokenize(m_paramTokens[2], tokens, ",");
                    if (tokens.size() == 3) {
...
void SimulatorBase::buildModel() {
    ...
    if (sim->getTimeStep()) sim->getTimeStep()->resize();
...
#ifdef USE_EMBEDDED_PYTHON
    if (m_scriptObject) m_scriptObject->execResetFct();
#endif
}
...
void SimulatorBase::singleTimeStep() {
    ...
    const Real stopAt = getValue<Real>(SimulatorBase::STOP_AT);
    if (m_gui && (stopAt > 0.0) && (stopAt < TimeManager::getCurrent()->getTime())) m_gui->stop();
}
...
void SimulatorBase::timeStep() {
    ...
    if ((pauseAt > 0.0) && (pauseAt < TimeManager::getCurrent()->getTime())) setValue(SimulatorBase::PAUSE, true);

    if (getValue<bool>(SimulatorBase::PAUSE)) return;
    ...
    for (unsigned int i = 0; i < numSteps; i++) {
        ...
        if (m_timeStepCB) m_timeStepCB();

#ifdef USE_EMBEDDED_PYTHON
        if (m_scriptObject) m_scriptObject->execStepFct();
#endif
    }
...
bool SimulatorBase::timeStepNoGUI() {
    const Real stopAt = getValue<Real>(SimulatorBase::STOP_AT);
    if ((stopAt > 0.0) && (stopAt < TimeManager::getCurrent()->getTime())) return false;
...
void SimulatorBase::setInitialVelocity(const Vector3r &vel,
...
void SimulatorBase::initFluidData() {
    ...
    for (unsigned int i = 0; i < scene.fluidModels.size(); i++) {
        ...
        if (ext == "OBJ") {
            ...
            std::string md5FileName = FileSystem::normalizePath(
                    cachePath + "/" + FileSystem::getFileNameWithExt(fileName) + "_fluid.md5");
            bool md5 = false;
            if (useCache) {
            ...
            const string scaleStr = StringTools::real2String(scene.fluidModels[i]->scale[0]) + "_" +
                                    StringTools::real2String(scene.fluidModels[i]->scale[1]) + "_" +
                                    StringTools::real2String(scene.fluidModels[i]->scale[2]);
            const string resStr = to_string(scene.fluidModels[i]->resolutionSDF[0]) + "_" +
                                  to_string(scene.fluidModels[i]->resolutionSDF[1]) + "_" +
                                  to_string(scene.fluidModels[i]->resolutionSDF[2]);
            const string particleFileName = FileSystem::normalizePath(
                    cachePath + "/" + mesh_file_name + "_fluid_" + StringTools::real2String(scene.particleRadius) +
                    "_i" + invert + "_m" + mode + "_s" + scaleStr + "_r" + resStr + ".bgeo");
            ...
            if (!useCache || !foundCacheFile || !md5) {
                ...
                if (useCache && (FileSystem::makeDir(cachePath) == 0)) {
                    ...
                    // PartioReaderWriter::writeParticles(particleFileName, (unsigned
                    // int)fluidParticles[fluidIndex].size(), fluidParticles[fluidIndex].data(),
                    // fluidVelocities[fluidIndex].data(), 0.0);
                    FileSystem::writeMD5File(fileName, md5FileName);
                }

                // transform particles
                for (unsigned int j = size_before_sampling; j < size_after_sampling; j++)
                    fluidParticles[fluidIndex][j] = scene.fluidModels[i]->rotation * fluidParticles[fluidIndex][j] +
        ...
        setInitialVelocity(scene.fluidModels[i]->initialVelocity, scene.fluidModels[i]->initialAngularVelocity,
                           numAddedParticles, &fluidParticles[fluidIndex][startIndex],
                           &fluidVelocities[fluidIndex][startIndex]);
        ...
        for (auto j = 0; j < numAddedParticles; j++) fluidObjectIds[fluidIndex][startIndex + j] = m_currentObjectId;
    ...
    for (unsigned int i = 0; i < scene.emitters.size(); i++) {
        ...
        if (j < sim->numberOfFluidModels()) {
            if (sim->is2DSimulation()) {
                ...
                // and set width to 1
                if (ed->type == 1) ed->height = ed->width;
                ed->width = 1;
                ed->type = 0;
            }
            model->getEmitterSystem()->addEmitter(ed->width, ed->height, ed->x, ed->rotation, ed->velocity, ed->type);
            ...
            emitterBoundary->scale =
                    Emitter::getSize(static_cast<Real>(ed->width), static_cast<Real>(ed->height), ed->type);
            ...
            if (sim->is2DSimulation()) emitterBoundary->scale[2] = 2 * supportRadius;
            ...
            if (emitterReuseParticles) model->getEmitterSystem()->enableReuseParticles(emitterBoxMin, emitterBoxMax);
...
void SimulatorBase::createFluidBlocks(std::map<std::string, unsigned int> &fluidIDs,
                                      std::vector<std::vector<Vector3r>> &fluidParticles,
                                      std::vector<std::vector<Vector3r>> &fluidVelocities,
                                      std::vector<std::vector<unsigned int>> &fluidObjectIds) {
    const utility::SceneLoader::Scene &scene = SceneConfiguration::getCurrent()->getScene();
    for (unsigned int i = 0; i < scene.fluidBlocks.size(); i++) {
        ...
        setInitialVelocity(scene.fluidBlocks[i]->initialVelocity, scene.fluidBlocks[i]->initialAngularVelocity,
                           numAddedParticles, &fluidParticles[fluidIndex][startIndex],
                           &fluidVelocities[fluidIndex][startIndex]);
...
void SimulatorBase::step() {
    ...
    if (TimeManager::getCurrent()->getTime() >= m_nextFrameTimeState) {
        ...
        if (m_enableStateExport) saveState();
    }
#ifdef DL_OUTPUT
...
#pragma omp for schedule(static)
                for (int j = 0; j < (int)bm->numberOfParticles(); j++) {
                    ...
                    if (rbo->isDynamic() || rbo->isAnimated())
                        bm->getVelocity(j) = rbo->getAngularVelocity().cross(bm->getPosition(j) - rbo->getPosition()) +
                                             rbo->getVelocity();
                    else
            ...
            if (forceUpdate) sim->getNeighborhoodSearch()->update_point_sets();
#endif
        }
    }
}

void SimulatorBase::updateDMVelocity() {
    Simulation *sim = Simulation::getCurrent();
    const utility::SceneLoader::Scene &scene = SceneConfiguration::getCurrent()->getScene();
...
void SimulatorBase::updateVMVelocity() {
    Simulation *sim = Simulation::getCurrent();
    const utility::SceneLoader::Scene &scene = SceneConfiguration::getCurrent()->getScene();
    const unsigned int nObjects = sim->numberOfBoundaryModels();
    for (unsigned int i = 0; i < nObjects; i++) {
...
void SimulatorBase::saveState(const std::string &stateFile) {
...
#ifdef WIN32
void SimulatorBase::loadStateDialog() {
...
void SimulatorBase::loadState(const std::string &stateFile) {
    Simulation *sim = Simulation::getCurrent();
    const std::string &sceneFile = SceneConfiguration::getCurrent()->getSceneFile();
    string md5Str = FileSystem::getFileMD5(sceneFile);
    ...
    if (!binReader.openFile(stateFile)) return;
...
void SimulatorBase::writeParameterObjectState(BinaryFileWriter &binWriter, ParameterObject *paramObj) {
    if (paramObj == nullptr) return;
...
void SimulatorBase::readParameterObjectState(BinaryFileReader &binReader, ParameterObject *paramObj) {
    if (paramObj == nullptr) return;
...
void SimulatorBase::readBoundaryState(const std::string &fileName, BoundaryModel *bm) {
    ...
    if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
        ...
        neighborhoodSearch->resize_point_set(model->getPointSetIndex(), &model->getPosition(0)[0],
                                             model->numberOfParticles());
    }
}

void SimulatorBase::initDensityMap(std::vector<Vector3r> &x,
    ...
    const string scaleStr = "s" + StringTools::real2String(boundaryData->scale[0]) + "_" +
                            StringTools::StringTools::real2String(boundaryData->scale[1]) + "_" +
                            StringTools::real2String(boundaryData->scale[2]);
    const string resStr =
    ...
    if (isDynamic)
        densityMapFileName = FileSystem::normalizePath(
                ...
                scaleStr + "_" + resStr + "_" + invertStr + "_" + thicknessStr + "_" + kernelStr + ".cdm");
    else
        densityMapFileName = FileSystem::normalizePath(
                cachePath + "/" + mesh_file_name + "_sb_dm_" + StringTools::real2String(scene.particleRadius) + "_" +
                scaleStr + "_" + resStr + "_" + invertStr + "_" + thicknessStr + "_" + kernelStr + ".cdm");

    // check MD5 if cache file is available
    bool foundCacheFile = false;

    if (useCache) foundCacheFile = FileSystem::fileExists(densityMapFileName);
    ...
    if (!useCache || !foundCacheFile || !md5) {
        ...
        for (unsigned int i = 0; i < x.size(); i++)
            for (unsigned int j = 0; j < 3; j++) doubleVec[3 * i + j] = x[i][j];
        ...
        densityMap = new Discregrid::CubicLagrangeDiscreteGrid(
        ...
        //////////////////////////////////////////////////////////////////////////
        if (sim2D) SimpleQuadrature::determineSamplePointsInCircle(supportRadius, 30);

        auto int_domain = Eigen::AlignedBox3d(Eigen::Vector3d::Constant(-supportRadius),
                                              Eigen::Vector3d::Constant(supportRadius));
        Real factor = 5.0;
        if (sim2D) factor = 1.75;
        auto density_func = [&](Eigen::Vector3d const &x) {
            ...
            auto integrand = [&](Eigen::Vector3d const &xi) {
                if (xi.squaredNorm() > supportRadius * supportRadius) return 0.0;

                auto dist = densityMap->interpolate(0u, x + xi);

                // Linear function gamma
                if (dist > 1.0 / factor * supportRadius) return 0.0;
        ...
        if (!no_reduction) {
            ...
            densityMap->reduceField(0u,
                                    [&](const Eigen::Vector3d &, double v) { return fabs(v) < 2.5 * supportRadius; });
            densityMap->reduceField(1u, [&](const Eigen::Vector3d &, double v) -> double {
                if (v == std::numeric_limits<double>::max()) return false;
                return true;
            });
...
void SimulatorBase::initVolumeMap(std::vector<Vector3r> &x,
                                  std::vector<unsigned int> &faces,
                                  const utility::SceneLoader::BoundaryData *boundaryData,
                                  const bool md5,
                                  const bool isDynamic,
    ...
    Eigen::Matrix<unsigned int, 3, 1> resolutionSDF = boundaryData->mapResolution;
    const string scaleStr = "s" + StringTools::real2String(boundaryData->scale[0]) + "_" +
                            StringTools::real2String(boundaryData->scale[1]) + "_" +
                            StringTools::real2String(boundaryData->scale[2]);
    const string resStr =
            "r" + to_string(resolutionSDF[0]) + "_" + to_string(resolutionSDF[1]) + "_" + to_string(resolutionSDF[2]);
    const string invertStr = "i" + to_string((int)boundaryData->mapInvert);
    ...
    if (isDynamic)
        volumeMapFileName = FileSystem::normalizePath(cachePath + "/" + mesh_file_name + "_db_vm_" +
                                                      ...
                                                      "_" + resStr + "_" + invertStr + "_" + thicknessStr + ".cdm");
    else
        volumeMapFileName = FileSystem::normalizePath(cachePath + "/" + mesh_file_name + "_sb_vm_" +
                                                      StringTools::real2String(scene.particleRadius) + "_" + scaleStr +
                                                      "_" + resStr + "_" + invertStr + "_" + thicknessStr + ".cdm");
    ...
    if (useCache) foundCacheFile = FileSystem::fileExists(volumeMapFileName);
    ...
    if (!useCache || !foundCacheFile || !md5) {
        ...
        LOG_INFO << "Set SDF resolution: " << resolutionSDF[0] << ", " << resolutionSDF[1] << ", " << resolutionSDF[2];
        volumeMap = new Discregrid::CubicLagrangeDiscreteGrid(
                domain, std::array<unsigned int, 3>({resolutionSDF[0], resolutionSDF[1], resolutionSDF[2]}));
        auto func = Discregrid::DiscreteGrid::ContinuousFunction{};
        ...
        auto volume_func = [&](Eigen::Vector3d const &x) {
            ...
            auto integrand = [&](Eigen::Vector3d const &xi) -> double {
                ...
                if (dist <= 0.0) return 1.0;  // -0.001 * dist / supportRadius;
                if (dist < 1.0 / factor * supportRadius)
                    return static_cast<double>(CubicKernel::W(factor * static_cast<Real>(dist)) /
                                               CubicKernel::W_zero());
                return 0.0;
            };
        ...
        if (!no_reduction) {
            std::cout << "Reduce discrete fields...";
            volumeMap->reduceField(0u,
                                   [&](const Eigen::Vector3d &, double v) { return fabs(v) < 4.0 * supportRadius; });
