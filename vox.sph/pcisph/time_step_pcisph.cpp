...
TimeStepPCISPH::TimeStepPCISPH() : TimeStep() {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) {
        ...
        model->addField({"pressure", FieldType::Scalar, [this, fluidModelIndex](const unsigned int i) -> Real * {
        ...
        for (int i = 0; i < numParticles; i++) {
            if (model->getParticleState(i) == ParticleState::Active) {
                const Vector3r accel =
                        model->getAcceleration(i) + m_simulationData.getPressureAccel(fluidModelIndex, i);
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors(const Vector3r &pred_xj = m_simulationData.getPredictedPosition(pid, neighborIndex);
                ...
                forall_boundary_neighbors(densityAdv += bm_neighbor->getVolume(neighborIndex) * sim->W(pred_xi - xj);)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                forall_density_maps(densityAdv += rho;)
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(densityAdv += Vj * sim->W(pred_xi - xj);)
            }
            ...
            pressure += invH2 * m_simulationData.getPCISPH_ScalingFactor(fluidModelIndex) *
                        (densityAdv - static_cast<Real>(1.0));
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(
                    ...
                    ai -= fm_neighbor->getVolume(neighborIndex) *
                          (dpi + (fm_neighbor->getDensity0() / density0) * dpj) * sim->gradW(xi - xj);)
                ...
                forall_boundary_neighbors(
                        // Pressure
                        const Vector3r a = bm_neighbor->getVolume(neighborIndex) * dpi * sim->gradW(xi - xj); ai -= a;
...
void TimeStepPCISPH::resize() { m_simulationData.init(); }
