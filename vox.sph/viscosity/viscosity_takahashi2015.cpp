...
Viscosity_Takahashi2015::Viscosity_Takahashi2015(FluidModel *model) : ViscosityBase(model) {
    ...
    model->addField({"viscous stress", FieldType::Matrix3,
                     [&](const unsigned int i) -> Real * { return &m_viscousStress[i](0, 0); }});
    model->addField(
            {"accel (visco)", FieldType::Vector3, [&](const unsigned int i) -> Real * { return &m_accel[i][0]; }});
}

Viscosity_Takahashi2015::~Viscosity_Takahashi2015() {
...
void Viscosity_Takahashi2015::matrixVecProd(const Real *vec, Real *result, void *userData) {
    auto *visco = (Viscosity_Takahashi2015 *)userData;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase(const Real density_j = model->getDensity(neighborIndex);
                                                 ...
                                                 ai += model->getMass(neighborIndex) *
                                                       (visco->getViscousStress(i) / density_i_2 +
                                                        visco->getViscousStress(neighborIndex) / density_j_2) *
                                                       gradW;)
        }
    }
}
        ...
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                ...
                forall_density_maps(Vector3r vj; bm_neighbor->getPointVelocity(xi, vj);
                                    const Vector3r a = -invH * 0.1 * (density0 / density_i) * (vi - vj) * rho; ai += a;
                ...
                forall_volume_maps(Vector3r vj; bm_neighbor->getPointVelocity(xj, vj);
                                   const Vector3r a =
                                           -invH * 0.1 * (density0 * Vj / density_i) * (vi - vj) * sim->W(xi - xj);
                                   ai += a; bm_neighbor->addForce(xj, -m_model->getMass(i) * a);)
            }
        }
    }
