...
Viscosity_Bender2017::Viscosity_Bender2017(FluidModel *model) : ViscosityBase(model) {
    ...
    model->addField({"target strain rate", FieldType::Vector6,
                     [&](const unsigned int i) -> Real * { return &m_targetStrainRate[i][0]; }});
    model->addField({"viscosity factor", FieldType::Matrix6,
                     [&](const unsigned int i) -> Real * { return &m_viscosityFactor[i](0, 0); }});
    model->addField({"viscosity lambda", FieldType::Vector6,
                     [&](const unsigned int i) -> Real * { return &m_viscosityLambda[i][0]; }});
}

Viscosity_Bender2017::~Viscosity_Bender2017() {
...
void Viscosity_Bender2017::initParameters() {
    ...
    auto *rparam = static_cast<RealParameter *>(getParameter(MAX_ERROR));
...
#pragma omp for schedule(static) nowait
            for (int i = 0; i < numParticles; i++) {
                ...
                forall_fluid_neighbors_in_same_phase(
                        ...
                        const Vector3r gradW = sim->gradW(xi - xj); const Vector3r vji = vj - vi;

                        const Real m = m_model->getMass(neighborIndex); const Real m2 = m * static_cast<Real>(2.0);
                        viscosityC[0] += m2 * vji[0] * gradW[0]; viscosityC[1] += m2 * vji[1] * gradW[1];
        ...
        for (int i = 0; i < (int)numParticles; i++)
            for (unsigned int j = 0; j < 6; j++) avgStrainRateError += fabs(getViscosityLambda(i)[j]);
        avgStrainRateError =
                (static_cast<Real>(1.0) / (static_cast<Real>(6.0) * (Real)numParticles)) * avgStrainRateError;
...
#pragma omp for schedule(static) nowait
            for (int i = 0; i < numParticles; i++) {
                ...
                forall_fluid_neighbors_in_same_phase(
                        ...
                        gradT.setZero(); gradT(0, 0) = static_cast<Real>(2.0) * gradW[0]; gradT(0, 3) = gradW[1];
                        gradT(0, 4) = gradW[2];

                        gradT(1, 1) = static_cast<Real>(2.0) * gradW[1]; gradT(1, 3) = gradW[0]; gradT(1, 5) = gradW[2];

                        gradT(2, 2) = static_cast<Real>(2.0) * gradW[2]; gradT(2, 4) = gradW[0]; gradT(2, 5) = gradW[1];

                        vi -= m_model->getMass(neighborIndex) * 0.5 * gradT *
                              ((m_model->getMass(neighborIndex) / (density_i * density_i)) * getViscosityLambda(i) +
                               (m_model->getMass(neighborIndex) / (density_j * density_j)) *
                                       getViscosityLambda(neighborIndex));)
            }
        }
        m_iterations++;

        if (avgStrainRateError < maxError) break;
    }
        ...
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                ...
                forall_density_maps(Vector3r vj; bm_neighbor->getPointVelocity(xi, vj);
                                    const Vector3r a =
                                            -invH * 0.1 * m_viscosity * (density0 / density_i) * (vi - vj) * rho;
                                    ai += a; bm_neighbor->addForce(xj, -m_model->getMass(i) * a);)
            } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                forall_volume_maps(Vector3r vj; bm_neighbor->getPointVelocity(xj, vj);
                                   const Vector3r a = -invH * 0.1 * m_viscosity * (density0 * Vj / density_i) *
                                                      (vi - vj) * sim->W(xi - xj);
                                   ai += a; bm_neighbor->addForce(xj, -m_model->getMass(i) * a);)
            }
        }
...
#pragma omp for schedule(static)
        for (int i = 0; i < numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase(
                    ...
                    grad_j.setZero(); grad_j(0, 0) = static_cast<Real>(2.0) * gradW[0]; grad_j(3, 0) = gradW[1];
                    grad_j(4, 0) = gradW[2];

                    grad_j(1, 1) = static_cast<Real>(2.0) * gradW[1]; grad_j(3, 1) = gradW[0]; grad_j(5, 1) = gradW[2];

                    grad_j(2, 2) = static_cast<Real>(2.0) * gradW[2]; grad_j(4, 2) = gradW[0]; grad_j(5, 2) = gradW[1];

                    grad_j = ((0.5 / density_i) * m_model->getMass(neighborIndex)) * grad_j; grad_i -= grad_j;
...
#pragma omp for schedule(static) nowait
        for (int i = 0; i < numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase(
                    ...
                    strainRate[3] +=
                    (static_cast<Real>(1.0) - m_viscosity) * m * (vji[0] * gradW[1] + vji[1] * gradW[0]);
                    strainRate[4] +=
                    (static_cast<Real>(1.0) - m_viscosity) * m * (vji[0] * gradW[2] + vji[2] * gradW[0]);
                    strainRate[5] +=
                    (static_cast<Real>(1.0) - m_viscosity) * m * (vji[1] * gradW[2] + vji[2] * gradW[1]);) strainRate =
                    (static_cast<Real>(0.5) / density_i) * strainRate;
        }
    }
}
