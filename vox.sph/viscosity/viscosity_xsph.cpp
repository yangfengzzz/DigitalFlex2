...
Viscosity_XSPH::Viscosity_XSPH(FluidModel *model) : ViscosityBase(model) { m_boundaryViscosity = 0.0; }

Viscosity_XSPH::~Viscosity_XSPH(void) = default;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors(const Vector3r &vj = fm_neighbor->getVelocity(neighborIndex);
                                   ...
                                   ai -= invH * m_viscosity * (fm_neighbor->getMass(neighborIndex) / density_j) *
                                         (vi - vj) * sim->W(xi - xj);)
                ...
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    ...
                    forall_density_maps(Vector3r vj; bm_neighbor->getPointVelocity(xi, vj);
                                        const Vector3r a =
                                                -invH * m_boundaryViscosity * (density0 / density_i) * (vi - vj) * rho;
                                        ai += a; bm_neighbor->addForce(xj, -m_model->getMass(i) * a);)
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                    forall_volume_maps(Vector3r vj; bm_neighbor->getPointVelocity(xj, vj);
                                       const Vector3r a = -invH * m_boundaryViscosity * (density0 * Vj / density_i) *
                                                          (vi - vj) * sim->W(xi - xj);
                                       ai += a; bm_neighbor->addForce(xj, -m_model->getMass(i) * a);)
                }
            }
