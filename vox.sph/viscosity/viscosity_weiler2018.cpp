...
Viscosity_Weiler2018::Viscosity_Weiler2018(FluidModel *model) : ViscosityBase(model), m_vDiff() {
    m_maxIter = 100;
    ...
    model->addField({"velocity difference", FieldType::Vector3,
                     [&](const unsigned int i) -> Real * { return &m_vDiff[i][0]; }, true});
}

Viscosity_Weiler2018::~Viscosity_Weiler2018() {
...
void Viscosity_Weiler2018::initParameters() {
    ...
    auto *rparam = static_cast<RealParameter *>(getParameter(VISCOSITY_COEFFICIENT_BOUNDARY));
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase_avx(
                    ...
                    const Vector3f8 vj_avx = convertVec_zero(
                            &sim->getNeighborList(fluidModelIndex, fluidModelIndex, i)[j], &vec[0], count);

                    delta_ai_avx += V_gradW * ((d_mu_rho0 / density_j_avx) * (vi_avx - vj_avx).dot(xixj) /
                                               (xixj.squaredNorm() + h2_001)););
            ...
            if (mub != 0.0) {
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors_avx(
                            ...
                            const Vector3f8 a = gradW * (d_mub * (density0_avx * Vj_avx / density_i_avx) *
                                                         (vi_avx).dot(xixj) / (xixj.squaredNorm() + h2_001));
                            delta_ai_avx += a;);
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                    forall_density_maps(
                            const Vector3r xixj = xi - xj; Vector3r normal = -xixj; const Real nl = normal.norm();
                            if (nl > static_cast<Real>(0.0001)) {
                                ...
                                const Vector3r a1 =
                                        (d * mub * vol * (vi).dot(xix1) / (xix1.squaredNorm() + 0.01 * h2)) * gradW1;
                                const Vector3r a2 =
                                        (d * mub * vol * (vi).dot(xix2) / (xix2.squaredNorm() + 0.01 * h2)) * gradW2;
                                const Vector3r a3 =
                                        (d * mub * vol * (vi).dot(xix3) / (xix3.squaredNorm() + 0.01 * h2)) * gradW3;
                                const Vector3r a4 =
                                        (d * mub * vol * (vi).dot(xix4) / (xix4.squaredNorm() + 0.01 * h2)) * gradW4;
                                ai += a1 + a2 + a3 + a4;
                            });
...
void Viscosity_Weiler2018::matrixVecProd(const Real *vec, Real *result, void *userData) {
    Simulation *sim = Simulation::getCurrent();
    auto *visco = (Viscosity_Weiler2018 *)userData;
    FluidModel *model = visco->getModel();
    const unsigned int numParticles = model->numActiveParticles();
    const unsigned int fluidModelIndex = model->getPointSetIndex();
    const unsigned int nFluids = sim->numberOfFluidModels();
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase(
                    const Real density_j = model->getDensity(neighborIndex); const Vector3r gradW = sim->gradW(xi - xj);

                    const Vector3r &vj = Eigen::Map<const Vector3r>(&vec[3 * neighborIndex]);
                    const Vector3r xixj = xi - xj;

                    ai += d * mu * (model->getMass(neighborIndex) / density_j) * (vi - vj).dot(xixj) /
                          (xixj.squaredNorm() + 0.01 * h2) * gradW;)
                ...
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors(const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex);
                                              const Vector3r xixj = xi - xj; const Vector3r gradW = sim->gradW(xixj);
                                              const Vector3r a =
                                                      d * mub *
                                                      (density0 * bm_neighbor->getVolume(neighborIndex) / density_i) *
                                                      (vi).dot(xixj) / (xixj.squaredNorm() + 0.01 * h2) * gradW;
                                              ai += a;)
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                    forall_density_maps(const Vector3r xixj = xi - xj; Vector3r normal = -xixj;
                                        const Real nl = normal.norm(); if (nl > static_cast<Real>(0.0001)) {
                                            ...
                                            // compute forces for both sample point
                                            const Vector3r a1 = d * mub * vol * (vi).dot(xix1) /
                                                                (xix1.squaredNorm() + 0.01 * h2) * gradW1;
                                            const Vector3r a2 = d * mub * vol * (vi).dot(xix2) /
                                                                (xix2.squaredNorm() + 0.01 * h2) * gradW2;
                                            const Vector3r a3 = d * mub * vol * (vi).dot(xix3) /
                                                                (xix3.squaredNorm() + 0.01 * h2) * gradW3;
                                            const Vector3r a4 = d * mub * vol * (vi).dot(xix4) /
                                                                (xix4.squaredNorm() + 0.01 * h2) * gradW4;
                                            ai += a1 + a2 + a3 + a4;
                                        })
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
                    forall_volume_maps(const Vector3r xixj = xi - xj; Vector3r normal = -xixj;
                                       const Real nl = normal.norm(); if (nl > static_cast<Real>(0.0001)) {
                                           normal /= nl;
...
void Viscosity_Weiler2018::diagonalMatrixElement(const unsigned int i, Matrix3r &result, void *userData) {
    ...
    //////////////////////////////////////////////////////////////////////////
    forall_fluid_neighbors_in_same_phase_avx(
            const Scalarf8 density_j_avx = convert_one(&sim->getNeighborList(fluidModelIndex, fluidModelIndex, i)[j],
                                                       &model->getDensity(0), count);
            const Vector3f8 xixj = xi_avx - xj_avx; const Vector3f8 gradW = CubicKernel_AVX::gradW(xixj);
            const Scalarf8 mj_avx = convert_zero(model->getMass(0), count);  // all particles have the same mass
            Matrix3f8 gradW_xij; dyadicProduct(gradW, xixj, gradW_xij);
    ...
    if (mub != 0.0) {
        if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
            forall_boundary_neighbors_avx(
                    ...
                    Matrix3f8 gradW_xij; dyadicProduct(gradW, xixj, gradW_xij);
                    res_avx +=
                    gradW_xij * (d_mub * (density0_avx * Vj_avx / density_i_avx) / (xixj.squaredNorm() + h2_001)););
        } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
            forall_density_maps(
                    const Vector3r xixj = xi - xj; Vector3r normal = -xixj; const Real nl = normal.norm();
                    if (nl > static_cast<Real>(0.0001)) {
...
void Viscosity_Weiler2018::diagonalMatrixElement(const unsigned int i, Matrix3r &result, void *userData) {
    ...
    // Fluid
    //////////////////////////////////////////////////////////////////////////
    forall_fluid_neighbors_in_same_phase(const Real density_j = model->getDensity(neighborIndex);
                                         const Vector3r gradW = sim->gradW(xi - xj); const Vector3r xixj = xi - xj;
                                         result += d * mu * (model->getMass(neighborIndex) / density_j) /
                                                   (xixj.squaredNorm() + 0.01 * h2) * (gradW * xixj.transpose());)
            ...
            if (mub != 0.0) {
        if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
            forall_boundary_neighbors(const Vector3r xixj = xi - xj; const Vector3r gradW = sim->gradW(xixj);
                                      result += d * mub *
                                                (density0 * bm_neighbor->getVolume(neighborIndex) / density_i) /
                                                (xixj.squaredNorm() + 0.01 * h2) * (gradW * xixj.transpose());)
        } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
...
void Viscosity_Weiler2018::diagonalMatrixElement(const unsigned int i, Vector3r &result, void *userData) {
    ...
    forall_fluid_neighbors_in_same_phase(const Real density_j = model->getDensity(neighborIndex);
                                         const Vector3r gradW = sim->gradW(xi - xj); const Vector3r xixj = xi - xj;
                                         Matrix3r r = d * mu * (model->getMass(neighborIndex) / density_j) /
                                                      (xixj.squaredNorm() + 0.01 * h2) * (gradW * xixj.transpose());
                                         result += r.diagonal(););
    ...
    if (mub != 0.0) {
        if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
            ...
            forall_density_maps(const Vector3r xixj = xi - xj; Vector3r normal = -xixj; const Real nl = normal.norm();
                                if (nl > 0.0001) {
                                    normal /= nl;
                                    ...
                                    // compute forces for both sample point
                                    Matrix3r r = d * mub * vol / (xix1.squaredNorm() + 0.01 * h2) *
                                                 (gradW1 * xix1.transpose());
            ...
            forall_volume_maps(const Vector3r xixj = xi - xj; Vector3r normal = -xixj; const Real nl = normal.norm();
                               if (nl > 0.0001) {
                                   normal /= nl;
                ...
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors(const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex);
                                              ...
                                              const Vector3r a =
                                                      d * mub *
                                                      (density0 * bm_neighbor->getVolume(neighborIndex) / density_i) *
                                                      (newVi - vj).dot(xixj) / (xixj.squaredNorm() + 0.01 * h2) * gradW;
                    ...
                    forall_density_maps(const Vector3r xixj = xi - xj; Vector3r normal = -xixj;
                                        const Real nl = normal.norm(); if (nl > static_cast<Real>(0.0001)) {
                                            ...
                                            // compute forces for both sample point
                                            const Vector3r a1 = d * mub * vol * (newVi - v1).dot(xix1) /
                                                                (xix1.squaredNorm() + 0.01 * h2) * gradW1;
                                            const Vector3r a2 = d * mub * vol * (newVi - v2).dot(xix2) /
                                                                (xix2.squaredNorm() + 0.01 * h2) * gradW2;
                                            const Vector3r a3 = d * mub * vol * (newVi - v3).dot(xix3) /
                                                                (xix3.squaredNorm() + 0.01 * h2) * gradW3;
                                            const Vector3r a4 = d * mub * vol * (newVi - v4).dot(xix4) /
                                                                (xix4.squaredNorm() + 0.01 * h2) * gradW4;
                ...
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors(const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex);
                                              ...
                                              const Vector3r a =
                                                      d * mub *
                                                      (density0 * bm_neighbor->getVolume(neighborIndex) / density_i) *
                                                      (vj).dot(xixj) / (xixj.squaredNorm() + 0.01 * h2) * gradW;
                                              bi += a;)
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017) {
                    forall_density_maps(const Vector3r xixj = xi - xj; Vector3r normal = -xixj;
                                        const Real nl = normal.norm(); if (nl > static_cast<Real>(0.0001)) {
                                            ...
                                            // compute forces for both sample point
                                            const Vector3r a1 = d * mub * vol * (v1).dot(xix1) /
                                                                (xix1.squaredNorm() + 0.01 * h2) * gradW1;
                                            const Vector3r a2 = d * mub * vol * (v2).dot(xix2) /
                                                                (xix2.squaredNorm() + 0.01 * h2) * gradW2;
                                            const Vector3r a3 = d * mub * vol * (v3).dot(xix3) /
                                                                (xix3.squaredNorm() + 0.01 * h2) * gradW3;
                                            const Vector3r a4 = d * mub * vol * (v4).dot(xix4) /
                                                                (xix4.squaredNorm() + 0.01 * h2) * gradW4;
                                            bi += a1 + a2 + a3 + a4;
                                        })
                } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
