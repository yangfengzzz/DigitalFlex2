...
Viscosity_Standard::~Viscosity_Standard(void) = default;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase_avx(
                    ...
                    delta_ai +=
                    V_gradW * ((dvisc / density_j_avx) * (vi_avx - vj_avx).dot(xixj) / (xixj.squaredNorm() + h2_avx)););
                ...
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors_avx(
                            ...
                            const Vector3f8 delta_a = CubicKernel_AVX::gradW(xixj) *
                                                      (dbvisc * (density0_avx * Vj_avx / density_i_avx) *
                                                       (vi_avx - vj_avx).dot(xixj) / (xixj.squaredNorm() + h2_avx));
                    ...
                    forall_density_maps(const Vector3r xixj = xi - xj; Vector3r normal = -xixj;
                                        const Real nl = normal.norm(); if (nl > static_cast<Real>(0.0001)) {
                                            ...
                                            const Vector3r a1 = (d * m_boundaryViscosity * vol * (vi - v1).dot(xix1) /
                                                                 (xix1.squaredNorm() + static_cast<Real>(0.01) * h2)) *
                                                                gradW1;
                                            const Vector3r a2 = (d * m_boundaryViscosity * vol * (vi - v2).dot(xix2) /
                                                                 (xix2.squaredNorm() + static_cast<Real>(0.01) * h2)) *
                                                                gradW2;
                                            const Vector3r a3 = (d * m_boundaryViscosity * vol * (vi - v3).dot(xix3) /
                                                                 (xix3.squaredNorm() + static_cast<Real>(0.01) * h2)) *
                                                                gradW3;
                                            const Vector3r a4 = (d * m_boundaryViscosity * vol * (vi - v4).dot(xix4) /
                                                                 (xix4.squaredNorm() + static_cast<Real>(0.01) * h2)) *
                                                                gradW4;
                                            ai += a1 + a2 + a3 + a4;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors_in_same_phase(const Vector3r &vj = model->getVelocity(neighborIndex);
                                                 ...
                                                 ai += d * m_viscosity * (model->getMass(neighborIndex) / density_j) *
                                                       (vi - vj).dot(xixj) / (xixj.squaredNorm() + 0.01 * h2) *
                                                       sim->gradW(xi - xj);)
                ...
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors(
                            const Vector3r &vj = bm_neighbor->getVelocity(neighborIndex); const Vector3r xixj = xi - xj;
                            const Vector3r a = d * m_boundaryViscosity *
