...
Viscosity_Peer2016::Viscosity_Peer2016(FluidModel *model) : ViscosityBase(model) {
    ...
    model->addField({"target nablaV", FieldType::Matrix3,
                     [&](const unsigned int i) -> Real * { return &m_targetNablaV[i](0, 0); }});
    model->addField(
            {"omega (visco)", FieldType::Vector3, [&](const unsigned int i) -> Real * { return &m_omega[i][0]; }});
}

Viscosity_Peer2016::~Viscosity_Peer2016() {
...
void Viscosity_Peer2016::initParameters() {
    ...
    ITERATIONS_OMEGA =
            createNumericParameter("viscoIterationsOmega", "Iterations (vorticity diffusion)", &m_iterationsOmega);
    ...
    auto *rparam = static_cast<RealParameter *>(getParameter(MAX_ERROR_V));
    rparam->setMinValue(1e-6);

    MAX_ITERATIONS_OMEGA =
            createNumericParameter("viscoMaxIterOmega", "Max. iterations (vorticity diffusion)", &m_maxIterOmega);
...
void Viscosity_Peer2016::matrixVecProdV(const Real *vec, Real *result, void *userData) {
    Simulation *sim = Simulation::getCurrent();
    auto *visco = (Viscosity_Peer2016 *)userData;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors_in_same_phase(result[i] -=
                                                 model->getMass(neighborIndex) * sim->W(xi - xj) * vec[neighborIndex];)
        }
    }
}
...
void Viscosity_Peer2016::matrixVecProdOmega(const Real *vec, Real *result, void *userData) {
    Simulation *sim = Simulation::getCurrent();
    auto *model = (FluidModel *)userData;
    const unsigned int numParticles = model->numActiveParticles();
    if (numParticles == 0) return;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors_in_same_phase(density_i += model->getMass(neighborIndex) * sim->W(xi - xj);)
...
#pragma omp for schedule(static) nowait
        for (int i = 0; i < numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors_in_same_phase(const Vector3r &vj = m_model->getVelocity(neighborIndex);
...
#pragma omp for schedule(static) nowait
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors_in_same_phase(const Real m = m_model->getMass(neighborIndex);
                                                 const Vector3r xij = xi - xj; const Real W = sim->W(xij);
...
#pragma omp for schedule(static) nowait
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase(
                    const Real m = m_model->getMass(neighborIndex); const Vector3r xij = xi - xj;
