...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase_avx(
                    const Scalarf8 Vj_avx = convert_zero(model->getVolume(0), count); compute_Vj_gradW_samephase();

                    const Vector3f8 vj_avx =
                            convertVec_zero(&sim->getNeighborList(fluidModelIndex, fluidModelIndex, i)[j],
                                            &model->getVelocity(0), count);
                    const Vector3f8 omegaj_avx = convertVec_zero(
                            &sim->getNeighborList(fluidModelIndex, fluidModelIndex, i)[j], &m_omega[0], count);
                    ...
                    delta_angAcceli_avx -=
                    omegaij * factor_avx * (Vj_avx / density_j_avx) * CubicKernel_AVX::W(xi_avx - xj_avx);

                    // difference curl
                    delta_ai_avx += (omegaij % V_gradW) * (nut_density_i * density0_avx);
                    delta_angAcceli_avx +=
                    ((vi_avx - vj_avx) % V_gradW) * (nut_density_i_intertiaInverse * density0_avx););
            ...
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                forall_boundary_neighbors_avx(
                        ...
                        // XSPH for angular velocity field
                        // delta_angAcceli_avx -= omegaij * factor_avx * (mj_avx / density_i_avx) *
                        // CubicKernel_AVX::W(xixj);
                ...
                forall_density_maps(
                        Vector3r vj; bm_neighbor->getPointVelocity(xi, vj);
                ...
                forall_volume_maps(Vector3r vj; bm_neighbor->getPointVelocity(xj, vj);
                                   ...
                                   // XSPH for angular velocity field
                                   // angAcceli -= (invDt * m_inertiaInverse * zeta * (density0 * Vj / density_i) *
                                   // sim->W(xij)) * omegaij;

                                   // difference curl
                                   const Vector3r a = (nu_t * 1.0 / density_i * density0 * Vj) * (omegaij.cross(gradW));
                                   ai += a; angAcceli += (nu_t * 1.0 / density_i * m_inertiaInverse * density0 * Vj) *
                                                         (vi - vj).cross(gradW);
