...
#define forall_boundary_neighbors(code)                                                         \
    for (unsigned int pid = nFluids; pid < sim->numberOfPointSets(); pid++) {                   \
        BoundaryModel_Akinci2012* bm_neighbor =                                                 \
...
#define forall_fluid_neighbors_avx(code)                                                                \
    for (unsigned int pid = 0; pid < nFluids; pid++) {                                                  \
        ...
        for (unsigned int j = 0; j < maxN; j += 8) {                                                    \
            ...
            const Vector3f8 xj_avx = convertVec_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j], \
                                                     &fm_neighbor->getPosition(0), count);              \
            code                                                                                        \
        }                                                                                               \
    }
...
#define forall_fluid_neighbors_in_same_phase_avx(code)                                                          \
    ...
    for (unsigned int j = 0; j < maxN; j += 8) {                                                                \
        const unsigned int count = std::min(maxN - j, 8u);                                                      \
        const Vector3f8 xj_avx = convertVec_zero(&sim->getNeighborList(fluidModelIndex, fluidModelIndex, i)[j], \
                                                 &model->getPosition(0), count);                                \
        code                                                                                                    \
    }
...
#define forall_boundary_neighbors_avx(code)                                                             \
    for (unsigned int pid = nFluids; pid < sim->numberOfPointSets(); pid++) {                           \
        ...
        for (unsigned int j = 0; j < maxN; j += 8) {                                                    \
            const unsigned int count = std::min(maxN - j, 8u);                                          \
            const Vector3f8 xj_avx = convertVec_zero(&sim->getNeighborList(fluidModelIndex, pid, i)[j], \
                                                     &bm_neighbor->getPosition(0), count);              \
            code                                                                                        \
        }                                                                                               \
    }
        ...
        bool hasSameParticleSampling(const FluidInfo& other) {
            ...
            if ((type == 1) && (other.type == 1) && (scale == other.scale)) {
                if (samplesFile == other.samplesFile) {
                    std::string ext = utility::FileSystem::getFileExt(samplesFile);
                ...
                if (((box.max() - box.min()).isApprox(other.box.max() - other.box.min()), 1.0e-9) &&
                    (mode == other.mode))
                    return true;
            }
            return false;
...
public:
    ...
    void init(Real particleRadius, bool sim2D);
    ...
    void addFluidModel(const std::string& id,
    ...
    FluidModel* getFluidModelFromPointSet(const unsigned int pointSetIndex) {
        return static_cast<FluidModel*>(m_neighborhoodSearch->point_set(pointSetIndex).get_user_data());
    }
    [[nodiscard]] unsigned int numberOfFluidModels() const { return static_cast<unsigned int>(m_fluidModels.size()); }
    ...
    BoundaryModel* getBoundaryModelFromPointSet(const unsigned int pointSetIndex) {
        return static_cast<BoundaryModel*>(m_neighborhoodSearch->point_set(pointSetIndex).get_user_data());
    }
    [[nodiscard]] unsigned int numberOfBoundaryModels() const {
        return static_cast<unsigned int>(m_boundaryModels.size());
    }
    ...
    [[nodiscard]] BoundaryHandlingMethods getBoundaryHandlingMethod() const {
        return (BoundaryHandlingMethods)m_boundaryHandlingMethod;
    }
    void setBoundaryHandlingMethod(BoundaryHandlingMethods val) { m_boundaryHandlingMethod = (int)val; }

    [[nodiscard]] int getKernel() const { return m_kernelMethod; }
    void setKernel(int val);
    [[nodiscard]] int getGradKernel() const { return m_gradKernelMethod; }
    void setGradKernel(int val);

    [[nodiscard]] int isSimulationInitialized() const { return m_simulationIsInitialized; }
    void setSimulationInitialized(int val);

    [[nodiscard]] FORCE_INLINE Real W_zero() const { return m_W_zero; }
    [[nodiscard]] FORCE_INLINE Real W(const Vector3r& r) const { return m_kernelFct(r); }
    FORCE_INLINE Vector3r gradW(const Vector3r& r) { return m_gradKernelFct(r); }

    [[nodiscard]] int getSimulationMethod() const { return static_cast<int>(m_simulationMethod); }
    void setSimulationMethod(int val);

    void setSimulationMethodChangedCallback(std::function<void()> const& callBackFct);

    TimeStep* getTimeStep() { return m_timeStep; }

    [[nodiscard]] bool is2DSimulation() const { return m_sim2D; }
    [[nodiscard]] bool zSortEnabled() const { return m_enableZSort; }

    void initKernels() const;
    ...
    void emitParticles();
    virtual void emittedParticles(FluidModel* model, unsigned int startIndex);
    ...
    [[nodiscard]] const std::string& getCachePath() const { return m_cachePath; }
    ...
    void addDragMethod(const std::string& name, const std::function<NonPressureForceBase*(FluidModel*)>& creator) {
        m_dragMethods.push_back({name, creator, -1});
    }
    std::vector<NonPressureForceMethod>& getDragMethods() { return m_dragMethods; }

    void addElasticityMethod(const std::string& name,
                             const std::function<NonPressureForceBase*(FluidModel*)>& creator) {
        m_elasticityMethods.push_back({name, creator, -1});
    }
    std::vector<NonPressureForceMethod>& getElasticityMethods() { return m_elasticityMethods; }

    void addSurfaceTensionMethod(const std::string& name,
                                 const std::function<NonPressureForceBase*(FluidModel*)>& creator) {
        m_surfaceTensionMethods.push_back({name, creator, -1});
    }
    std::vector<NonPressureForceMethod>& getSurfaceTensionMethods() { return m_surfaceTensionMethods; }

    void addViscosityMethod(const std::string& name, const std::function<NonPressureForceBase*(FluidModel*)>& creator) {
        m_viscoMethods.push_back({name, creator, -1});
    }
    std::vector<NonPressureForceMethod>& getViscosityMethods() { return m_viscoMethods; }

    void addVorticityMethod(const std::string& name, const std::function<NonPressureForceBase*(FluidModel*)>& creator) {
        m_vorticityMethods.push_back({name, creator, -1});
    }
    ...
    [[nodiscard]] FORCE_INLINE unsigned int numberOfPointSets() const {
        return static_cast<unsigned int>(m_neighborhoodSearch->n_point_sets());
    }

    [[nodiscard]] FORCE_INLINE unsigned int numberOfNeighbors(const unsigned int pointSetIndex,
                                                              const unsigned int neighborPointSetIndex,
                                                              const unsigned int index) const {
        return static_cast<unsigned int>(
                m_neighborhoodSearch->point_set(pointSetIndex).n_neighbors(neighborPointSetIndex, index));
    }

    [[nodiscard]] FORCE_INLINE unsigned int getNeighbor(const unsigned int pointSetIndex,
                                                        const unsigned int neighborPointSetIndex,
    ...
    [[nodiscard]] FORCE_INLINE const unsigned int* getNeighborList(const unsigned int pointSetIndex,
                                                                   const unsigned int neighborPointSetIndex,
                                                                   const unsigned int index) const {
