...
BoundaryModel_Akinci2012::BoundaryModel_Akinci2012() : m_x0(), m_x(), m_v(), m_V() {
    m_sorted = false;
    m_pointSetIndex = 0;
}
            ...
            for (unsigned int pid = nFluids; pid < sim->numberOfPointSets(); pid++) {
                ...
                for (unsigned int j = 0; j < neighborhoodSearch->point_set(m_pointSetIndex).n_neighbors(pid, i); j++) {
                    const unsigned int neighborIndex =
                            neighborhoodSearch->point_set(m_pointSetIndex).neighbor(pid, i, j);
...
void BoundaryModel_Akinci2012::initModel(RigidBodyObject *rbo,
...
#pragma omp for schedule(static)
    ...
    m_pointSetIndex = neighborhoodSearch->add_point_set(
            &m_x[0][0], m_x.size(), m_rigidBody->isDynamic() || m_rigidBody->isAnimated(), false, true, this);
}
...
void BoundaryModel_Akinci2012::performNeighborhoodSearchSort() {
    ...
    if ((numPart == 0) || (!m_rigidBody->isDynamic() && !m_rigidBody->isAnimated() && m_sorted)) return;
...
void BoundaryModel_Akinci2012::saveState(BinaryFileWriter &binWriter) {
    binWriter.write(m_sorted);
    binWriter.write(m_pointSetIndex);
}

void BoundaryModel_Akinci2012::loadState(BinaryFileReader &binReader) {
    binReader.read(m_sorted);
    binReader.read(m_pointSetIndex);
}

void BoundaryModel_Akinci2012::resize(const unsigned int numBoundaryParticles) {
    m_x0.resize(numBoundaryParticles);
    m_x.resize(numBoundaryParticles);
    m_v.resize(numBoundaryParticles);
    m_V.resize(numBoundaryParticles);
