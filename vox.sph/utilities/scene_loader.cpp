...
void SceneLoader::readScene(const char* fileName, Scene& scene) {
    ...
    if (m_jsonData.find("FluidModels") != m_jsonData.end()) {
        ...
        for (auto& fluidModel : fluidModels) {
            ...
            if (readValue<std::string>(fluidModel["particleFile"], particleFile)) {
                ...
                Real angle = 0.0;
                data->rotation = Matrix3r::Identity();
                if (readVector(fluidModel["rotationAxis"], axis) && readValue<Real>(fluidModel["rotationAngle"], angle))
                    data->rotation = AngleAxisr(angle, axis);
    ...
    if (m_jsonData.find("FluidBlocks") != m_jsonData.end()) {
        ...
        for (auto& fluidBlock : fluidBlocks) {
            ...
            Vector3r minX, maxX;
            if (readVector(fluidBlock["start"], minX) && readVector(fluidBlock["end"], maxX)) {
                auto* block = new FluidBlock();
    ...
    if (m_jsonData.find("Emitters") != m_jsonData.end()) {
        ...
        for (auto& emitter : emitters) {
            ...
            Real angle = 0.0;
            data->rotation = Matrix3r::Identity();
            if (readVector(emitter["rotationAxis"], axis) && readValue(emitter["rotationAngle"], angle)) {
                // in 2D simulations always rotate around z-axis
                if (scene.sim2D) axis = {0.0, 0.0, 1.0};
                axis.normalize();
    ...
    if (m_jsonData.find("AnimationFields") != m_jsonData.end()) {
        ...
        for (auto& field : fields) {
            auto* data = new AnimationFieldData();
            ...
            Real angle = 0.0;
            data->rotation = Matrix3r::Identity();
            if (readVector(field["rotationAxis"], axis) && readValue(field["rotationAngle"], angle)) {
                axis.normalize();
                data->rotation = AngleAxisr(angle, axis).toRotationMatrix();
            }
...
void SceneLoader::readParameterObject(nlohmann::json& config, ParameterObject* paramObj) {
    ...
    for (unsigned int i = 0; i < numParams; i++) {
        ...
        if (paramBase->getType() == RealParameterType) {
            ...
            int val;
            if (readValue(config[paramBase->getName()], val)) static_cast<EnumParameter*>(paramBase)->setValue(val);
        } else if (paramBase->getType() == ParameterBase::BOOL) {
            bool val;
            if (readValue(config[paramBase->getName()], val)) static_cast<BoolParameter*>(paramBase)->setValue(val);
            ...
            std::string val;
            if (readValue(config[paramBase->getName()], val)) static_cast<StringParameter*>(paramBase)->setValue(val);
        }
    }
}
