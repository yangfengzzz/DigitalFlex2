...
#pragma omp critical
    ...
    const auto less = [](const Vector3r& a, const Vector3r& b) -> bool {
        return (a[0] < b[0]) || (a[0] == b[0] && a[1] < b[1] || (a[0] == b[0] && a[1] == b[1] && a[2] < b[2]));
    };
    std::sort(samples.begin(), samples.end(), less);

    const Real squaredMinDist = 1e-12f;
    const auto equals = [squaredMinDist](const Vector3r& a, const Vector3r& b) -> bool {
        return (a - b).squaredNorm() < squaredMinDist;
    };
