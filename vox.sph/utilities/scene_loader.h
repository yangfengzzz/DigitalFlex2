...
protected:
    ...
    void readParameterObject(nlohmann::json &config, ParameterObject *paramObj);
...
public:
    ...
    bool readVector(const nlohmann::json &j, Eigen::Matrix<T, size, 1, Eigen::DontAlign> &vec) {
        ...
        for (unsigned int i = 0; i < values.size(); i++) vec[i] = values[i];
        return true;
    }
    ...
    template <typename T, int size>
    bool readVector(const std::string &section,
                    const std::string &key,
                    Eigen::Matrix<T, size, 1, Eigen::DontAlign> &vec) {
        if (m_jsonData.find(section) != m_jsonData.end()) {
            nlohmann::json j = m_jsonData[section];
            if (j.is_null()) return false;
    ...
    void readMaterialParameterObject(const std::string &key, ParameterObject *paramObj);
    void readParameterObject(const std::string &key, ParameterObject *paramObj);
};
