...
template <>
struct traits<vox::MatrixReplacement> : public Eigen::internal::traits<SystemMatrixType> {};
}  // namespace Eigen::internal
...
public:
    ...
    [[nodiscard]] Index rows() const { return m_dim; }
    [[nodiscard]] Index cols() const { return m_dim; }
...
public:
    ...
    JacobiPreconditioner1D() = default;
    ...
    [[nodiscard]] Eigen::Index rows() const { return m_dim; }
    [[nodiscard]] Eigen::Index cols() const { return m_dim; }

    static Eigen::ComputationInfo info() { return Eigen::Success; }
...
#pragma omp for schedule(static)
    ...
    template <typename Rhs>
    inline Eigen::Solve<JacobiPreconditioner1D, Rhs> solve(const Eigen::MatrixBase<Rhs>& b) const {
...
public:
    ...
    JacobiPreconditioner3D() = default;
    ...
    [[nodiscard]] Eigen::Index rows() const { return 3 * m_dim; }
    [[nodiscard]] Eigen::Index cols() const { return 3 * m_dim; }
...
#pragma omp for schedule(static)
    ...
    template <typename Rhs>
    inline Eigen::Solve<JacobiPreconditioner3D, Rhs> solve(const Eigen::MatrixBase<Rhs>& b) const {
...
public:
    ...
    BlockJacobiPreconditioner3D() = default;
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)m_dim; i++) {
                static_cast<VectorXr&>(x).block<3, 1>(3 * i, 0) =
                        m_invDiag[i] * static_cast<const VectorXr&>(b).block<3, 1>(3 * i, 0);
            }
        }
    }

    template <typename Rhs>
    inline Eigen::Solve<BlockJacobiPreconditioner3D, Rhs> solve(const Eigen::MatrixBase<Rhs>& b) const {
...
template <typename Rhs>
struct generic_product_impl<MatrixReplacement, Rhs, SparseShape, DenseShape, GemvProduct>  // GEMV stands for generic
                                                                                           // matrix-vector
    ...
    static void scaleAndAddTo(Dest& dst, const MatrixReplacement& lhs, const Rhs& rhs, const Scalar& alpha) {
        ...
        Real* res = &dst(0);
        auto& lhs_ = const_cast<MatrixReplacement&>(lhs);
