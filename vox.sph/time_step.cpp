...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors_avx(const Scalarf8 Vj_avx = convert_zero(fm_neighbor->getVolume(0), count);
...
void TimeStep::reset() { m_iterations = 0; }
...
void TimeStep::approximateNormal(DiscreteGrid *map,
...
void TimeStep::computeVolumeAndBoundaryX(const unsigned int fluidModelIndex, const unsigned int i, const Vector3r &xi) {
    ...
    for (unsigned int pid = 0; pid < nBoundaries; pid++) {
        auto *bm = static_cast<BoundaryModel_Bender2019 *>(sim->getBoundaryModel(pid));
            ...
            if ((dist > 0.0) && (static_cast<Real>(dist) < supportRadius)) {
                ...
                if ((volume > 0.0) && (volume != numeric_limits<double>::max())) {
                    ...
                    if (nl > 1.0e-9) {
                        ...
                        const Real d = max((static_cast<Real>(dist) + static_cast<Real>(0.5) * particleRadius),
                                           static_cast<Real>(2.0) * particleRadius);
        ...
        if (animateParticle) {
            if (dist != numeric_limits<double>::max())  // if dist is numeric_limits<double>::max(), then the particle
                                                        // is not close to the current boundary
            {
                normal = R.cast<double>() * normal;
                const double nl = normal.norm();
...
void TimeStep::computeDensityAndGradient(const unsigned int fluidModelIndex, const unsigned int i, const Vector3r &xi) {
    ...
    for (unsigned int pid = 0; pid < nBoundaries; pid++) {
        auto *bm = static_cast<BoundaryModel_Koschier2017 *>(sim->getBoundaryModel(pid));
        ...
        if ((dist > 0.1 * particleRadius) && (dist < sim->getSupportRadius())) {
            ...
            if (nl > 1.0e-5) {
                ...
                // adapt velocity in normal direction
                sim->getFluidModel(fluidModelIndex)->getVelocity(i) +=
                        (0.05 - sim->getFluidModel(fluidModelIndex)->getVelocity(i).dot(normal.cast<Real>())) *
                        normal.cast<Real>();
            }
            boundaryDensityGradient.setZero();
            boundaryDensity = 0.0;
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nFluids; fluidModelIndex++) {
        ...
        for (int i = 0; i < numParticles; i++) {
            for (unsigned int pid = 0; pid < nFluids; pid++) {
                ...
                // same phase
                if (pid == fluidModelIndex) precomputed_indices_same_phase.push_back(sumNeighborParticles);

                // steps of 8 values due to avx
                sumNeighborParticles += maxN / 8;
                if (maxN % 8 != 0) sumNeighborParticles++;
            }
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)numParticles; i++) {
                ...
                forall_fluid_neighbors_avx(
                        ...
                        precomputed_V_gradW[base + idx] = CubicKernel_AVX::gradW(xi_avx - xj_avx) * Vj_avx; idx++;);
            }
        }
    }
}
