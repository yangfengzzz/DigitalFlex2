...
void DragForce_Gissler2017::step() {
    ...
    const Real c_def =
            ...
            exp(-t_max / td) * (cos(omega * t_max) + static_cast<Real>(1.0) / (omega * td) * sin(omega * t_max));
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            const Real Re_i =
                    static_cast<Real>(2.0) * std::max((rho_a * vi_rel_norm * L) / mu_a, static_cast<Real>(0.1));

            Real C_Di_sphere;
            if (Re_i <= 1000.0)
                C_Di_sphere = static_cast<Real>(24.0) / Re_i *
                              (static_cast<Real>(1.0) +
                               static_cast<Real>(1.0 / 6.0) * pow(Re_i, static_cast<Real>(2.0 / 3.0)));
            else
            ...
            forall_fluid_neighbors_in_same_phase(Vector3r xixj = xi - xj; xixj.normalize();
                                                 const Real x_v = v_i_rel_n.dot(xixj);
                                                 max_v_x = std::max(max_v_x, x_v);)
                    // Equation (15)
                    const Real w_i = std::max(static_cast<Real>(0.0),
                                              std::min(static_cast<Real>(1.0), static_cast<Real>(1.0) - max_v_x));
            ...
            ai += m_dragCoefficient * static_cast<Real>(0.5) / m_model->getMass(i) * rho_a * (v_i_rel * vi_rel_norm) *
                  C_Di * A_i;
        }
    }
}
