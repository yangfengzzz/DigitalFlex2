...
TimeStepPF::TimeStepPF() : TimeStep(), m_stiffness(50000.0), m_counter(0), m_numActiveParticlesTotal(0) {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) {
        ...
        model->addField({"oldPosition", FieldType::Vector3,
                         [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getOldPosition(fluidModelIndex, i)[0];
                         },
                         true});
        model->addField({"S", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getS(fluidModelIndex, i)[0];
                         }});
        model->addField(
                {"numFluidNeighbors", FieldType::UInt, [this, fluidModelIndex](const unsigned int i) -> unsigned int * {
                     return &m_simulationData.getNumFluidNeighbors(fluidModelIndex, i);
                 }});
        model->addField({"diag", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real * {
                             return &m_simulationData.getDiag(fluidModelIndex, i)[0];
                         }});
    }
}
...
#pragma omp parallel for
    for (int i = 0; i < (int)numParticles; i++) {
        ...
        const auto newPos =
                (model->getPosition(i) + h * model->getVelocity(i) + (h * h) * model->getAcceleration(i)).eval();
...
void TimeStepPF::solvePDConstraints() {
    ...
    m_numActiveParticlesTotal =
            m_simulationData.getParticleOffset(nFluids - 1) + sim->getFluidModel(nFluids - 1)->numActiveParticles();
...
#pragma omp parallel for schedule(static)
    ...
    for (m_iterations = 0u; m_iterations < m_maxIterations; m_iterations++) {
        ...
        if (m_solver.iterations() == 0) break;
    }

    updatePositionsAndVelocity(x);
}

void TimeStepPF::updatePositionsAndVelocity(const VectorXr &x) {
    Simulation *sim = Simulation::getCurrent();
    const unsigned int nModels = sim->numberOfFluidModels();
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)numParticles; i++) {
                if (model->getParticleState(i) == ParticleState::Active) {
                    const Vector3r vel =
                            h_inv * (x.Vec3Block(offset + i) - m_simulationData.getOldPosition(fluidModelIndex, i));
...
#ifdef PD_USE_DIAGONAL_PRECONDITIONER
FORCE_INLINE void TimeStepPF::diagonalMatrixElement(const unsigned int row, Vector3r &result, void *userData) {
    auto *timeStepPF = static_cast<TimeStepPF *>(userData);
    Simulation *sim = Simulation::getCurrent();
    const unsigned int nModels = sim->numberOfFluidModels();
...
void TimeStepPF::preparePreconditioner() {
    Simulation *sim = Simulation::getCurrent();
    const unsigned int nModels = sim->numberOfFluidModels();
    const auto h = TimeManager::getCurrent()->getTimeStepSize();
...
#pragma omp parallel for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            for (unsigned int pid = 0; pid < nModels; pid++) {
                density_scale += density0_inv * sim->getFluidModelFromPointSet(pid)->getDensity0() *
                                 sim->numberOfNeighbors(fluidModelIndex, pid, i);
            }
            m_simulationData.setDiag(fluidModelIndex, i,
                                     Vector3r::Constant(system_scale * density_scale + model->getMass(i)));
        }
    }
}
...
void TimeStepPF::addAccellerationToVelocity() {
    Simulation *sim = Simulation::getCurrent();
    const auto h = TimeManager::getCurrent()->getTimeStepSize();
    const unsigned int nModels = sim->numberOfFluidModels();
...
void TimeStepPF::matrixFreeRHS(const VectorXr &x, VectorXr &result) {
    Simulation *sim = Simulation::getCurrent();
    ...
    //////////////////////////////////////////////////////////////////////////
    // constraint value
    const auto calculateC = [&](const unsigned int fluidModelIndex, const unsigned int i,
                                std::vector<Vector3r> &p) -> Real {
        ...
        if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
            ...
            forall_density_maps(density += rho;)
        } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
            forall_volume_maps(density += Vj * sim->W(xi - xj);)
        }
    ...
    // constraint gradient
    const auto calculateNablaC = [&](const unsigned int fluidModelIndex, const unsigned int i,
                                     std::vector<Vector3r> &p) -> std::vector<Vector3r> {
        ...
        if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
            ...
            forall_density_maps(nablaC[0] -= gradRho;)
        } else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019) {
            forall_volume_maps(nablaC[0] += Vj * sim->gradW(xi - xj);)
        }
        return nablaC;
    };
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)numParticles; i++) {
                ...
                for (auto c = 0u; c < 3; c++) addToAtomicReal(accumulator[3 * index_i + c]._a, density0 * p[0][c]);
                unsigned int counter = 1;
                for (unsigned int pid = 0; pid < nFluids; pid++) {
                    const unsigned int neighborOffset = m_simulationData.getParticleOffset(pid);
                    for (unsigned int j = 0; j < sim->numberOfNeighbors(fluidModelIndex, pid, i); j++) {
...
void TimeStepPF::matrixVecProd(const Real *vec, Real *result, void *userData) {
    auto *timeStepPF = static_cast<TimeStepPF *>(userData);
...
void TimeStepPF::resize() { m_simulationData.init(); }
