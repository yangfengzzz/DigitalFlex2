...
DebugTools::DebugTools() : ParameterObject() {
    ...
    for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++) {
        ...
        model->addField({"threadId", FieldType::UInt, [this, fluidModelIndex](const unsigned int i) -> unsigned int* {
                             return &m_threadIds[fluidModelIndex][i];
                         }});
        model->addField(
                {"numNeighbors", FieldType::UInt, [this, fluidModelIndex](const unsigned int i) -> unsigned int* {
                     return &m_numNeighbors[fluidModelIndex][i];
                 }});
        model->addField({"velocityChanges", FieldType::Vector3, [this, fluidModelIndex](const unsigned int i) -> Real* {
                             return &m_velocityChanges[fluidModelIndex][i][0];
                         }});
    }
}
...
void DebugTools::initParameters() {
    ...
    DETERMINE_NUM_NEIGHBORS =
            createBoolParameter("determineNumNeighbors", "Determine # neighbors", &m_determineNumNeighbors);
    ...
    DETERMINE_VELOCITY_CHANGES =
            createBoolParameter("determineVelocityChanges", "Determine velocity changes", &m_determineVelocityChanges);
    setGroup(DETERMINE_VELOCITY_CHANGES, "Debug Tools");
    setDescription(DETERMINE_VELOCITY_CHANGES,
                   "Determine velocity change of each particle and add a corresponding particle field.");
}

void DebugTools::determineThreadIds() {
    Simulation* sim = Simulation::getCurrent();
...
void DebugTools::determineNumNeighbors() {
    Simulation* sim = Simulation::getCurrent();
    const unsigned int nFluids = sim->numberOfFluidModels();
...
void DebugTools::determineVelocityChanges() {
    Simulation* sim = Simulation::getCurrent();
    const unsigned int nFluids = sim->numberOfFluidModels();
...
void DebugTools::step() {
    if (m_determineThreadIds) determineThreadIds();

    if (m_determineNumNeighbors) determineNumNeighbors();

    if (m_determineVelocityChanges) determineVelocityChanges();
}
