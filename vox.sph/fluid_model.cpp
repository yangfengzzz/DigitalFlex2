...
void FluidModel::initParameters() {
    ...
    NUM_REUSED_PARTICLES = createNumericParameter<unsigned int>(
            "numReusedParticles", "# reused particles", [&]() { return m_emitterSystem->numReusedParticles(); });
    ...
    ParameterBase::SetFunc<int> setDragFct =
            std::bind(static_cast<void (FluidModel::*)(const unsigned int)>(&FluidModel::setDragMethod), this,
                      std::placeholders::_1);
    ...
    auto* enumParam = static_cast<EnumParameter*>(getParameter(DRAG_METHOD));
    Simulation* sim = Simulation::getCurrent();
    std::vector<Simulation::NonPressureForceMethod>& dragMethods = sim->getDragMethods();
    for (auto& dragMethod : dragMethods) enumParam->addEnumValue(dragMethod.m_name, dragMethod.m_id);

    ParameterBase::GetFunc<int> getSurfaceTensionFct = std::bind(&FluidModel::getSurfaceTensionMethod, this);
    ParameterBase::SetFunc<int> setSurfaceTensionFct =
            std::bind(static_cast<void (FluidModel::*)(const unsigned int)>(&FluidModel::setSurfaceTensionMethod), this,
                      std::placeholders::_1);
    SURFACE_TENSION_METHOD =
            createEnumParameter("surfaceTensionMethod", "Surface tension", getSurfaceTensionFct, setSurfaceTensionFct);
    ...
    for (auto& surfaceTensionMethod : surfaceTensionMethods)
        enumParam->addEnumValue(surfaceTensionMethod.m_name, surfaceTensionMethod.m_id);

    ParameterBase::GetFunc<int> getViscosityFct = std::bind(&FluidModel::getViscosityMethod, this);
    ParameterBase::SetFunc<int> setViscosityFct =
            std::bind(static_cast<void (FluidModel::*)(const unsigned int)>(&FluidModel::setViscosityMethod), this,
                      std::placeholders::_1);
    ...
    for (auto& viscoMethod : viscoMethods) enumParam->addEnumValue(viscoMethod.m_name, viscoMethod.m_id);

    ParameterBase::GetFunc<int> getVorticityFct = std::bind(&FluidModel::getVorticityMethod, this);
    ParameterBase::SetFunc<int> setVorticityFct =
            std::bind(static_cast<void (FluidModel::*)(const unsigned int)>(&FluidModel::setVorticityMethod), this,
                      std::placeholders::_1);
    ...
    for (auto& vorticityMethod : vorticityMethods)
        enumParam->addEnumValue(vorticityMethod.m_name, vorticityMethod.m_id);

    ParameterBase::GetFunc<int> getElasticityFct = std::bind(&FluidModel::getElasticityMethod, this);
    ParameterBase::SetFunc<int> setElasticityFct =
            std::bind(static_cast<void (FluidModel::*)(const unsigned int)>(&FluidModel::setElasticityMethod), this,
                      std::placeholders::_1);
    ELASTICITY_METHOD =
            createEnumParameter("elasticityMethod", "Elasticity method", getElasticityFct, setElasticityFct);
    ...
    for (auto& elasticityMethod : elasticityMethods)
        enumParam->addEnumValue(elasticityMethod.m_name, elasticityMethod.m_id);
}
...
void FluidModel::reset() {
    ...
    struct Comparator {
        explicit Comparator(FluidModel* _this) : m_this(_this){};
...
#pragma omp for schedule(static)
        for (int i = 0; i < nParticles; i++) {
            setMass(i, m_V * m_density0);  // each particle represents a cube with a side length of r
                                           // mass is slightly reduced to prevent pressure at the beginning of the
                                           // simulation
        }
    }
}
...
void FluidModel::setDensity0(const Real v) {
    m_density0 = v;
    initMasses();
}
...
void FluidModel::setSurfaceTensionMethod(const unsigned int val) {
    ...
    for (auto& stMethod : stMethods) {
        if (stMethod.m_id == method_id) m_surfaceTension = static_cast<SurfaceTensionBase*>(stMethod.m_creator(this));
    }
...
void FluidModel::setViscosityMethod(const unsigned int val) {
    ...
    for (auto& viscoMethod : viscoMethods) {
        if (viscoMethod.m_id == m_viscosityMethod) {
            m_viscosity = static_cast<ViscosityBase*>(viscoMethod.m_creator(this));
            break;
        }
    }
...
void FluidModel::setVorticityMethod(const unsigned int val) {
    ...
    for (auto& vorticityMethod : vorticityMethods) {
        if (vorticityMethod.m_id == method_id)
            m_vorticity = static_cast<VorticityBase*>(vorticityMethod.m_creator(this));
    }
...
void FluidModel::setDragMethod(const unsigned int val) {
    ...
    for (auto& dragMethod : dragMethods) {
        if (dragMethod.m_id == method_id) m_drag = static_cast<DragBase*>(dragMethod.m_creator(this));
    }
...
void FluidModel::setElasticityMethod(const unsigned int val) {
    ...
    for (auto& elasticityMethod : elasticityMethods) {
        if (elasticityMethod.m_id == method_id)
            m_elasticity = static_cast<ElasticityBase*>(elasticityMethod.m_creator(this));
    }
