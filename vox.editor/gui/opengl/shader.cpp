...
Shader::Shader(void) {
    m_initialized = false;
    for (unsigned char i = 0; i < 3; i++) m_shaders[i] = 0;
    m_attributes.clear();
    m_uniforms.clear();
    m_program = 0;
}
...
void Shader::createAndLinkProgram() {
    ...
    for (unsigned char i = 0; i < 3; i++) {
        if (m_shaders[i] != 0) glAttachShader(m_program, m_shaders[i]);
    }
    ...
    for (unsigned char i = 0; i < 3; i++) glDeleteShader(m_shaders[i]);
}
...
bool Shader::isInitialized() { return m_initialized; }
