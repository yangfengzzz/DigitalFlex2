...
#include "mini_gl.h"
#include "vox.editor/gui/opengl/colormaps/colormap_bwr.h"
#include "vox.editor/gui/opengl/colormaps/colormap_coolwarm.h"
#include "vox.editor/gui/opengl/colormaps/colormap_jet.h"
#include "vox.editor/gui/opengl/colormaps/colormap_plasma.h"
#include "vox.editor/gui/opengl/colormaps/colormap_seismic.h"
...
void Simulator_OpenGL::meshShaderEnd() { m_meshShader.end(); }

void Simulator_OpenGL::pointShaderBegin(Shader *shader,
                                        const Real particleRadius,
                                        const float *col,
                                        const Real minVal,
                                        const Real maxVal,
                                        const bool useTexture,
                                        float const *color_map) {
    shader->begin();
...
void Simulator_OpenGL::renderFluid(FluidModel *model,
                                   float *fluidColor,
    ...
    if (MiniGL::checkOpenGLVersion(3, 3)) {
        ...
        if (colorMapType == 0) shader_scalar = &m_shader_scalar;

        if (!useScalarField)
            pointShaderBegin(shader_scalar, particleRadius, &fluidColor[0], renderMinValue, renderMaxValue, false);
        else
            pointShaderBegin(shader_scalar, particleRadius, &fluidColor[0], renderMinValue, renderMaxValue, true,
                             color_map);
...
void Simulator_OpenGL::renderSelectedParticles(FluidModel *model,
    ...
    if (MiniGL::checkOpenGLVersion(3, 3)) {
        if ((selectedParticles.size() > 0) && ((selectedParticles[fluidIndex].size() > 0))) {
            ...
            glDrawElements(GL_POINTS, (GLsizei)selectedParticles[fluidIndex].size(), GL_UNSIGNED_INT,
                           selectedParticles[fluidIndex].data());
            glDisableVertexAttribArray(0);
            glDisableVertexAttribArray(1);
...
void Simulator_OpenGL::renderBoundaryParticles(BoundaryModel_Akinci2012 *model,
                                               const float *col,
                                               const Real renderMinValue,
                                               const Real renderMaxValue) {
    Simulation *sim = Simulation::getCurrent();
