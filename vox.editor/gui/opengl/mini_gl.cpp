...
void MiniGL::setClientSceneFunc(SceneFct func) { scenefunc = func; }
...
void MiniGL::reshape(GLFWwindow *glfw_window, int w, int h) {
    if ((w > 0) && (h > 0)) {
        ...
        for (auto i = 0; i < m_reshapeFct.size(); i++) m_reshapeFct[i](m_width, m_height);
        glViewport(0, 0, m_width, m_height);
    }
}

void MiniGL::setClientIdleFunc(IdleFct func) { idlefunc = func; }

void MiniGL::setClientDestroyFunc(DestroyFct func) { destroyfunc = func; }
...
void MiniGL::keyboard(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    for (auto i = 0; i < m_keyboardFct.size(); i++) {
        if (m_keyboardFct[i](key, scancode, action, mods)) return;
    }
...
void MiniGL::char_callback(GLFWwindow *window, unsigned int codepoint) {
    ...
    for (auto i = 0; i < m_charFct.size(); i++) {
        if (m_charFct[i](codepoint, GLFW_PRESS)) return;
    }
...
void MiniGL::mousePress(GLFWwindow *window, int button, int action, int mods) {
    ...
    for (auto i = 0; i < m_mousePressFct.size(); i++) {
        if (m_mousePressFct[i](button, action, mods)) return;
    }
...
void MiniGL::mouseWheel(GLFWwindow *window, double xoffset, double yoffset) {
    ...
    for (auto i = 0; i < m_mouseWheelFct.size(); i++) {
        if (m_mouseWheelFct[i](static_cast<int>(mouse_wheel_pos), xoffset, yoffset)) return;
    }
...
void MiniGL::mouseMove(GLFWwindow *window, double x, double y) {
    ...
    for (auto i = 0; i < m_mouseMoveFct.size(); i++) {
        if (m_mouseMoveFct[i](static_cast<int>(x), static_cast<int>(y))) return;
    }
...
void MiniGL::error_callback(int error, const char *description) { LOG_ERR << description; }
