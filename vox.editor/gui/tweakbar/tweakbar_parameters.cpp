...
void TweakBarParameters::createParameterObjectGUI(TwBar *tweakBar, ParameterObject *paramObj) {
    ...
    for (unsigned int i = 0; i < numParams; i++) {
        ...
        std::string tmp = "";
        if (paramBase->getHotKey() != "") tmp = "key='" + paramBase->getHotKey() + "' ";
        ...
        if (paramBase->getType() == RealParameterType) {
            ...
            TwAddVarCB(tweakBar, varName, TW_TYPE_REAL, setParameterValue, getParameterValue,
            ...
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_UINT32, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::UINT16) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_UINT16, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::UINT8) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_UINT8, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::INT32) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_INT32, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::INT16) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_INT16, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::INT8) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_INT8, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
            ...
            TwAddVarCB(tweakBar, varName, enumType, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::BOOL) {
            sprintf(str, " label='%s' help='%s' group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_BOOL32, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == RealVectorParameterType) {
            if (static_cast<VectorParameter<Real> *>(paramBase)->getDim() == 3) {
                sprintf(str, " label='%s' help='%s' group='%s' %s ", paramBase->getLabel().c_str(),
                        paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
                TwAddVarCB(tweakBar, varName, TW_TYPE_DIR3R, setParameterValue, getParameterValue,
                           m_params[m_params.size() - 1].get(), str);
            }
        } else if (paramBase->getType() == ParameterBase::STRING) {
            sprintf(str, " label='%s' help='%s' group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(tweakBar, varName, TW_TYPE_STDSTRING, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        }
    }
}
