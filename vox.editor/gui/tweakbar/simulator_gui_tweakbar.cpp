...
#include "tweakbar_parameters.h"
#include "vox.editor/gui/opengl/mini_gl.h"
#include "vox.editor/gui/opengl/selection.h"
...
Simulator_GUI_TweakBar::Simulator_GUI_TweakBar(SimulatorBase *base) : Simulator_GUI_Base(base) {
    m_tweakBar = nullptr;
...
void Simulator_GUI_TweakBar::initTweakBarParameters() {
    TwAddVarCB(getTweakBar(), "Time", TW_TYPE_REAL, setTimeCB, getTimeCB, nullptr,
               " label='Time' precision=5 group='General'");
...
void Simulator_GUI_TweakBar::initSimulationParameterGUI() {
    ...
    if (m_simulatorBase) TweakBarParameters::createParameterObjectGUI(getTweakBar(), m_simulatorBase);
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), sim);
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), (ParameterObject *)sim->getTimeStep());
#ifdef USE_DEBUG_TOOLS
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), (ParameterObject *)sim->getDebugTools());
#endif
    ...
    if (sim->numberOfFluidModels() > 0) {
        ...
        TwAddVarCB(getTweakBar(), "CurrentFluidModel", enumType, setCurrentFluidModel, getCurrentFluidModel, this,
                   enumStr.c_str());
    }
    ...
    std::string str = " label='Color map' enum='0 {None}, 1 {Jet}, 2 {Plasma}' group='" + model->getId() +
                      "' help='Choose a color map.'";
    TwAddVarCB(getTweakBar(), "ColorMapType", enumType2, setColorMapType, getColorMapType, this, str.c_str());
    str = " label='Min. value (shader)' step=0.001 precision=3 group='" + model->getId() +
          "' help='Minimal value used for color-coding the color field in the rendering process.'";
    TwAddVarCB(getTweakBar(), "RenderMinValue", TW_TYPE_REAL, setRenderMinValue, getRenderMinValue, this, str.c_str());
    str = " label='Max. value (shader)' step=0.001 precision=3 group='" + model->getId() +
          "' help='Maximal value used for color-coding the color field in the rendering process.'";
    ...
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), (ParameterObject *)model->getDragBase());
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), (ParameterObject *)model->getSurfaceTensionBase());
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), (ParameterObject *)model->getViscosityBase());
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), (ParameterObject *)model->getVorticityBase());
    TweakBarParameters::createParameterObjectGUI(getTweakBar(), (ParameterObject *)model->getElasticityBase());
...
void Simulator_GUI_TweakBar::initParameterGUI() { TwRemoveAllVars(getTweakBar()); }
...
void Simulator_GUI_TweakBar::render() {
    ...
    for (unsigned int i = 0; i < sim->numberOfFluidModels(); i++) {
        ...
        if ((field == nullptr) || (base->getScalarField(i).size() == 0)) useScalarField = false;
        Simulator_OpenGL::renderFluid(model, fluidColor, base->getColorMapType(i), useScalarField,
                                      base->getScalarField(i), base->getRenderMinValue(i), base->getRenderMaxValue(i));
...
void Simulator_GUI_TweakBar::reset() { m_selectedParticles.clear(); }
    ...
    for (unsigned int i = 0; i < sim->numberOfFluidModels(); i++) {
        ...
        if (nParticles != 0) {
            ...
            Selection::selectRect(start, end, &model->getPosition(0),
                                  &model->getPosition(model->numActiveParticles() - 1), selectedParticles[i]);
            if (selectedParticles[i].size() > 0) selected = true;
        }
    }
    if (selected)
...
void Simulator_GUI_TweakBar::particleInfo() { SimulatorBase::particleInfo(m_selectedParticles); }

void Simulator_GUI_TweakBar::run() { MiniGL::mainLoop(); }

void Simulator_GUI_TweakBar::stop() { MiniGL::leaveMainLoop(); }

void Simulator_GUI_TweakBar::addKeyFunc(char k, std::function<void()> const &func) { MiniGL::addKeyFunc(k, func); }
