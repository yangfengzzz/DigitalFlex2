...
Simulator_GUI_imgui::Simulator_GUI_imgui(SimulatorBase *base) : Simulator_GUI_Base(base) { m_currentFluidModel = 0; }

Simulator_GUI_imgui::~Simulator_GUI_imgui(void) { imguiParameters::cleanup(); }
...
void Simulator_GUI_imgui::init(int argc, char **argv, const char *name) {
    ...
    MiniGL::addKeyboardFunc([](int key, int scancode, int action, int mods) -> bool {
        ImGui_ImplGlfw_KeyCallback(MiniGL::getWindow(), key, scancode, action, mods);
        return ImGui::GetIO().WantCaptureKeyboard;
    });
    MiniGL::addCharFunc([](int key, int action) -> bool {
        ImGui_ImplGlfw_CharCallback(MiniGL::getWindow(), key);
        return ImGui::GetIO().WantCaptureKeyboard;
    });
    MiniGL::addMousePressFunc([](int button, int action, int mods) -> bool {
        ImGui_ImplGlfw_MouseButtonCallback(MiniGL::getWindow(), button, action, mods);
        return ImGui::GetIO().WantCaptureMouse;
    });
    MiniGL::addMouseWheelFunc([](int pos, double xoffset, double yoffset) -> bool {
        ImGui_ImplGlfw_ScrollCallback(MiniGL::getWindow(), xoffset, yoffset);
        return ImGui::GetIO().WantCaptureMouse;
    });
...
void Simulator_GUI_imgui::initImgui() {
    ...
    std::string font =
            utility::FileSystem::normalizePath(m_simulatorBase->getExePath() + "/resources/fonts/Roboto-Medium.ttf");
    // std::string font = utility::FileSystem::normalizePath(m_simulatorBase->getExePath() +
...
void Simulator_GUI_imgui::initImguiParameters() {
    ...
    imguiParameters::imguiNumericParameter<Real> *timeStepSizeParam =
            new imguiParameters::imguiNumericParameter<Real>();
...
void Simulator_GUI_imgui::initSimulationParameterGUI() {
    ...
    imguiParameters::createParameterObjectGUI((ParameterObject *)sim->getTimeStep());
#ifdef USE_DEBUG_TOOLS
    imguiParameters::createParameterObjectGUI((ParameterObject *)sim->getDebugTools());
#endif
    ...
    if (sim->numberOfFluidModels() > 0) {
        ...
        // Select color field
        {
            ...
            param->getFct = [this]() -> int {
                ...
                for (auto i = 0; i < m_colorFieldNames.size(); i++) {
                    if (m_colorFieldNames[i] == fieldName) return i;
                }
                return 0;
            };
            param->setFct = [this](int v) {
        ...
        // Select color min/max value
        {
            ...
            param3->description =
                    "Recompute min and max values for color-coding the color field in the rendering process.";
        ...
        imguiParameters::createParameterObjectGUI((ParameterObject *)model->getDragBase());
        imguiParameters::createParameterObjectGUI((ParameterObject *)model->getSurfaceTensionBase());
        imguiParameters::createParameterObjectGUI((ParameterObject *)model->getViscosityBase());
        imguiParameters::createParameterObjectGUI((ParameterObject *)model->getVorticityBase());
        imguiParameters::createParameterObjectGUI((ParameterObject *)model->getElasticityBase());
    }
}
...
void Simulator_GUI_imgui::cleanup() { MiniGL::getKeyFunc().clear(); }
        ...
        for (int body = sim->numberOfBoundaryModels() - 1; body >= 0; body--) {
            if ((renderWalls == 1) || (!scene.boundaryModels[body]->isWall)) {
                ...
                Simulator_OpenGL::renderBoundaryParticles(bm, scene.boundaryModels[body]->color.data(),
                                                          base->getRenderMinValue(0), base->getRenderMaxValue(0));
            }
        }
...
void Simulator_GUI_imgui::reset() { m_selectedParticles.clear(); }
...
void Simulator_GUI_imgui::particleInfo() { SimulatorBase::particleInfo(m_selectedParticles); }

void Simulator_GUI_imgui::run() { MiniGL::mainLoop(); }

void Simulator_GUI_imgui::stop() { MiniGL::leaveMainLoop(); }

void Simulator_GUI_imgui::addKeyFunc(char k, std::function<void()> const &func) { MiniGL::addKeyFunc(k, func); }
