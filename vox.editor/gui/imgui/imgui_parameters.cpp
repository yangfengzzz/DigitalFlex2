...
#include "imgui_parameters.h"

#include "vox.base/logging.h"
#include "vox.editor/gui/opengl/mini_gl.h"
...
std::vector<std::pair<std::string, std::vector<std::pair<std::string, std::vector<imguiParameters::imguiParameter*>>>>>
        imguiParameters::m_imguiParams;

void imguiParameters::createRealParameter(imguiParameters::imguiParameter* param,
                                          ImGuiInputTextFlags flags,
                                          const std::string& helpText) {
    // Real parameter
    imguiParameters::imguiNumericParameter<Real>* rparam =
            dynamic_cast<imguiParameters::imguiNumericParameter<Real>*>(param);
    if ((rparam != nullptr) && (rparam->getFct != nullptr)) {
        ...
        if (ImGui::IsItemDeactivatedAfterEdit()) {
            if (rparam->minValue != std::numeric_limits<Real>::min()) value = std::max(rparam->minValue, value);
            if (rparam->maxValue != std::numeric_limits<Real>::max()) value = std::min(rparam->maxValue, value);
            if (rparam->setFct) rparam->setFct(value);
        }
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(helpText.c_str());
    }
}

void imguiParameters::createBoolParameter(imguiParameters::imguiParameter* param,
                                          ImGuiInputTextFlags flags,
                                          const std::string& helpText) {
    // bool parameter
    ...
    if ((bparam != nullptr) && (bparam->getFct != nullptr)) {
        ...
        if (ImGui::IsItemDeactivatedAfterEdit()) {
            if (!bparam->readOnly) {
                if (bparam->setFct) bparam->setFct(value);
            }
        }
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(helpText.c_str());
    }
...
void imguiParameters::createStringParameter(imguiParameters::imguiParameter* param,
                                            ImGuiInputTextFlags flags,
                                            const std::string& helpText) {
    // string parameter
    ...
    if ((sparam != nullptr) && (sparam->getFct != nullptr)) {
        ...
        if (ImGui::IsItemDeactivatedAfterEdit()) {
            if (sparam->setFct) sparam->setFct(value);
        }
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(helpText.c_str());
    }
}

void imguiParameters::createVec3rParameter(imguiParameters::imguiParameter* param,
                                           ImGuiInputTextFlags flags,
                                           const std::string& helpText) {
    // vec3 parameter
    ...
    if ((vparam != nullptr) && (vparam->getFct != nullptr)) {
        ...
        ImGui::InputScalarN(vparam->label.c_str(), ImGuiDataType_Real, value.data(), 3, NULL, NULL, m_format.c_str(),
                            flags);

        if (ImGui::IsItemDeactivatedAfterEdit()) {
            if (vparam->setFct) vparam->setFct(value);
        }
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(helpText.c_str());
    }
}

void imguiParameters::createVec3fParameter(imguiParameters::imguiParameter* param,
                                           ImGuiInputTextFlags flags,
                                           const std::string& helpText) {
    // vec3 parameter
...
bool imguiParameters::createEnumParameter(imguiParameters::imguiParameter* param,
                                          ImGuiInputTextFlags flags,
                                          const std::string& helpText) {
    // enum parameter
...
void imguiParameters::createFunctionParameter(imguiParameters::imguiParameter* param,
                                              ImGuiInputTextFlags flags,
                                              const std::string& helpText) {
    // bool parameter
    ...
    if ((fparam != nullptr) && (fparam->function != nullptr)) {
        if (ImGui::Button(fparam->label.c_str())) {
            if (fparam->function) fparam->function();
        }
        if (ImGui::IsItemHovered()) ImGui::SetTooltip(helpText.c_str());
    }
}

void imguiParameters::createSubgroupParameters(
        const std::vector<std::pair<std::string, std::vector<imguiParameter*>>>& params) {
    for (auto subgroup_index = 0; subgroup_index < params.size(); subgroup_index++) {
        ...
        if ((subgroup.first == "") || (ImGui::TreeNodeEx(subgroup.first.c_str(), ImGuiTreeNodeFlags_DefaultOpen))) {
            ...
            for (int i = 0; i < subgroup.second.size(); i++) {
                ...
                if (param->readOnly)
                    helpText += " (read-only)";
                else if ((param->name != "") && (dynamic_cast<imguiParameters::imguiFunctionParameter*>(param) ==
                                                 nullptr))  // no key help text for function parameters
                    helpText += "\n\nkey in scene file:\n" + param->name;
                ...
                if (createEnumParameter(param, flags, helpText)) {
                    // stop the creation of the GUI since the change of an enum value
                    // could cause a change of the GUI
                    if (param->readOnly) ImGui::PopStyleColor(3);
                    if (subgroup.first != "") ImGui::TreePop();
...
void imguiParameters::createParameterGUI() {
    ...
    if (ImGui::BeginTabBar("TabBar", ImGuiTabBarFlags_None)) {
        for (auto group_index = 0; group_index < m_imguiParams.size(); group_index++) {
            auto group = m_imguiParams[group_index];
            if (group.first == "General") continue;
...
void imguiParameters::createParameterObjectGUI(ParameterObject* paramObj) {
    ...
    for (unsigned int i = 0; i < numParams; i++) {
        ...
        std::string subgroup = "";
        if ((group == "WCSPH") || (group == "PCISPH") || (group == "PBF") || (group == "IISPH") || (group == "DFSPH") ||
            (group == "PF") || (group == "ICSPH") || (group == "CFL") || (group == "Kernel")) {
            subgroup = group;
            group = "Simulation";
        }

        if ((group == "FluidModel") || (group == "Elasticity") || (group == "Viscosity") || (group == "Vorticity") ||
            (group == "Drag force") || (group == "Surface tension")) {
            subgroup = group;
            group = "Fluid Model";
        }

        if ((group == "Simulation") && (subgroup == "")) subgroup = group;

        if (group == "General") subgroup = group;

        if (group == "Export") subgroup = "General";
        ...
        if (paramBase->getType() == RealParameterType) {
            ...
            param->getFct = [paramBase]() -> Real {
                return static_cast<NumericParameter<Real>*>(paramBase)->getValue();
            };
            ...
            imguiParameters::imguiNumericParameter<unsigned int>* param =
                    new imguiParameters::imguiNumericParameter<unsigned int>();
            param->name = paramBase->getName();
            param->description = paramBase->getDescription();
            param->label = paramBase->getLabel();
            param->readOnly = paramBase->getReadOnly();
            ...
            param->getFct = [paramBase]() -> unsigned int {
                return static_cast<NumericParameter<unsigned int>*>(paramBase)->getValue();
            };
            param->setFct = [paramBase](unsigned int v) {
                static_cast<NumericParameter<unsigned int>*>(paramBase)->setValue(v);
            };
            imguiParameters::addParam(group, subgroup, param);
        } else if (paramBase->getType() == ParameterBase::UINT16) {
            imguiParameters::imguiNumericParameter<unsigned short>* param =
                    new imguiParameters::imguiNumericParameter<unsigned short>();
            param->name = paramBase->getName();
            param->description = paramBase->getDescription();
            param->label = paramBase->getLabel();
            param->readOnly = paramBase->getReadOnly();
            ...
            param->getFct = [paramBase]() -> unsigned short {
                return static_cast<NumericParameter<unsigned short>*>(paramBase)->getValue();
            };
            param->setFct = [paramBase](unsigned short v) {
                static_cast<NumericParameter<unsigned short>*>(paramBase)->setValue(v);
            };
            imguiParameters::addParam(group, subgroup, param);
        } else if (paramBase->getType() == ParameterBase::UINT8) {
            imguiParameters::imguiNumericParameter<unsigned char>* param =
                    new imguiParameters::imguiNumericParameter<unsigned char>();
            param->name = paramBase->getName();
            param->description = paramBase->getDescription();
            param->label = paramBase->getLabel();
            param->readOnly = paramBase->getReadOnly();
            ...
            param->getFct = [paramBase]() -> unsigned char {
                return static_cast<NumericParameter<unsigned char>*>(paramBase)->getValue();
            };
            param->setFct = [paramBase](unsigned char v) {
                static_cast<NumericParameter<unsigned char>*>(paramBase)->setValue(v);
            };
            imguiParameters::addParam(group, subgroup, param);
            ...
            param->getFct = [paramBase]() -> short {
                return static_cast<NumericParameter<short>*>(paramBase)->getValue();
            };
            ...
            param->getFct = [paramBase]() -> char {
                return static_cast<NumericParameter<char>*>(paramBase)->getValue();
            };
            ...
            for (auto item : eparam->getEnumValues()) param->items.push_back(item.name);
            ...
            param->label = paramBase->getLabel();
            param->readOnly = paramBase->getReadOnly();
            param->getFct = [paramBase]() -> std::string {
                return static_cast<StringParameter*>(paramBase)->getValue();
            };
            param->setFct = [paramBase](const std::string& v) {
                static_cast<StringParameter*>(paramBase)->setValue(v);
            };
            imguiParameters::addParam(group, subgroup, param);
        } else if ((paramBase->getType() == RealVectorParameterType) &&
                   (static_cast<RealVectorParameter*>(paramBase)->getDim() == 3)) {
            imguiParameters::imguiVec3rParameter* param = new imguiParameters::imguiVec3rParameter();
            param->name = paramBase->getName();
            param->description = paramBase->getDescription();
            param->label = paramBase->getLabel();
            param->readOnly = paramBase->getReadOnly();
            param->getFct = [paramBase]() -> Vector3r {
                return Vector3r(static_cast<RealVectorParameter*>(paramBase)->getValue());
            };
            param->setFct = [paramBase](Vector3r& v) {
                static_cast<RealVectorParameter*>(paramBase)->setValue(v.data());
            };
            imguiParameters::addParam(group, subgroup, param);
...
void imguiParameters::addGroup(const std::string& pgroup, const std::string& psubgroup) {
    for (auto group_index = 0; group_index < m_imguiParams.size(); group_index++) {
        ...
        if (group.first == pgroup) {
            for (auto subgroup_index = 0; subgroup_index < group.second.size(); subgroup_index++) {
                auto subgroup = group.second[subgroup_index];
                if (subgroup.first == psubgroup) return;
            }
            // sub group not found
            group.second.push_back({psubgroup, std::vector<imguiParameter*>()});
            group.second.back().second.reserve(20);
