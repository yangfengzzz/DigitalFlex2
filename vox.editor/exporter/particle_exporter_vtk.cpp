...
ParticleExporter_VTK::ParticleExporter_VTK(SimulatorBase* base) : ExporterBase(base) { m_outfile = nullptr; }

ParticleExporter_VTK::~ParticleExporter_VTK() = default;
        ...
        if (!m_base->getValue<bool>(SimulatorBase::EXPORT_OBJECT_SPLITTING)) {
            ...
            for (auto j = 0u; j < m_base->getLastObjectId(); j++) {
                std::string fileName2 =
                        fileName + "_" + model->getId() + "_" + std::to_string(j) + "_" + std::to_string(frame);
...
void ParticleExporter_VTK::createParticleFile(const std::string& fileName, FluidModel* model) {
    ...
    if (!m_outfile->is_open()) {
        LOGW("Cannot open a file to save VTK particles.")
        return;
    }
    ...
    *m_outfile << "DATASET UNSTRUCTURED_GRID\n";
    ...
    m_attributes.erase(std::remove_if(m_attributes.begin(), m_attributes.end(),
                                      [](const std::string& s) { return (s == "position" || s == "id"); }),
                       m_attributes.end());
}
...
void ParticleExporter_VTK::writeParticles(const std::string& fileName, FluidModel* model, const unsigned int objId) {
    ...
    for (unsigned int i = 0; i < numParticles; i++) {
        if ((objId != 0xffffffff) && (model->getObjectId(i) != objId)) continue;
        ...
        // export position attribute as POINTS
        {
            ...
            // swap endianess
            for (unsigned int c = 0; c < 3; c++) swapByteOrder(&positions[positions.size() - 1][c]);
        }
    ...
    if (m_outfile != nullptr) {
        // export to vtk
        const auto nPoints = (unsigned int)positions.size();
        ...
        *m_outfile << "FIELD FieldData " << std::to_string(numFields) << "\n";
        ...
        for (const std::string& a : m_attributes) {
            ...
            if (field.type == FieldType::Scalar) {
                ...
                *m_outfile << attrNameVTK << " 3 " << nPoints << real_str;
                ...
                for (unsigned int i = 0u; i < numParticles; i++) {
                    ...
                    for (unsigned int c = 0; c < 3; c++) swapByteOrder(&val[c]);  // swap endianess
                    attrData.emplace_back(val);
                }
                // export to vtk
            ...
            // TODO support other field types
                ...
                LOGW("Skipping attribute {}, because it is of unsupported type\n", a)
                continue;
            }
            // end of block
            *m_outfile << "\n";
