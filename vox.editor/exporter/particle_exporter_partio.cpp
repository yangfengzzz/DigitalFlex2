...
ParticleExporter_Partio::ParticleExporter_Partio(SimulatorBase* base) : ExporterBase(base) {}

ParticleExporter_Partio::~ParticleExporter_Partio() = default;
...
void ParticleExporter_Partio::writeParticlesPartio(const std::string& fileName,
                                                   FluidModel* model,
    ...
    for (unsigned int i = 0; i < attributes.size(); i++) {
        ...
        if (!found) {
            attrMap[i] = -1;
            LOGW("Unknown field cannot be exported in partio file: {}", attributes[i])
        }
    }
        ...
        for (unsigned int j = 0; j < attributes.size(); j++) {
            ...
            if (fieldIndex != -1) {
                ...
                if (field.type == FieldType::Scalar) {
                    auto* val = particleData.dataWrite<float>(partioAttrMap[j], index);
    ...
    if (async)
        m_handle = std::async(std::launch::async, [&] {
