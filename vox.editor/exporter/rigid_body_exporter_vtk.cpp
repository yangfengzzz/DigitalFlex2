...
RigidBodyExporter_VTK::RigidBodyExporter_VTK(SimulatorBase* base) : ExporterBase(base) { m_isFirstFrame = true; }

RigidBodyExporter_VTK::~RigidBodyExporter_VTK() = default;
...
void RigidBodyExporter_VTK::reset() { m_isFirstFrame = true; }
...
void RigidBodyExporter_VTK::writeRigidBodies(const unsigned int frame) {
    ...
    if (m_isFirstFrame || !isStatic) {
        for (unsigned int i = 0; i < sim->numberOfBoundaryModels(); i++) {
            ...
            if (!outfile) {
                LOGW("Cannot open a file to save VTK mesh.")
                return;
            }
            ...
            // Connectivity
            {
                ...
                outfile.write(reinterpret_cast<char*>(&connectivity_to_write[0]),
                              connectivity_to_write.size() * sizeof(int));
                outfile << "\n";
            }
