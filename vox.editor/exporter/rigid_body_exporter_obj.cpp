...
RigidBodyExporter_OBJ::RigidBodyExporter_OBJ(SimulatorBase* base) : ExporterBase(base) { m_isFirstFrame = true; }

RigidBodyExporter_OBJ::~RigidBodyExporter_OBJ() = default;
...
void RigidBodyExporter_OBJ::reset() { m_isFirstFrame = true; }
        ...
        for (unsigned int i = 0; i < sim->numberOfBoundaryModels(); i++) {
            ...
            // faces
            {
                for (int j = 0; j < n_triangles; j++) {
                    outfile << "f " << faces[3 * j + 0] + 1 << " " << faces[3 * j + 1] + 1 << " "
                            << faces[3 * j + 2] + 1 << "\n";
                }
            }
            outfile.close();
