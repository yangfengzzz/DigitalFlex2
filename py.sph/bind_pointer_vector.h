...
void vector_pointer_modifiers(enable_if_t<is_copy_constructible<typename Vector::value_type>::value, Class_> &cl) {
    ...
    cl.def(
            "clear",
            [](Vector &v) {
                for (const auto &elem : v) delete elem;
                v.clear();
            },
            "Clear the contents");
    ...
    cl.def(
            ...
            [](Vector &v, const Vector &src) {
                for (const auto &elem : src) v.push_back(new TNoPtr(*elem));
            },
    ...
    cl.def(
            ...
            [](Vector &v, const iterable &it) {
                ...
                try {
                    ...
                    for (auto it = v.begin() + static_cast<DiffType>(old_size); it != v.end(); it++) delete *it;
