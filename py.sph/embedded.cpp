...
void Embedded::setCurrent(Embedded* tm) { current = tm; }

bool Embedded::hasCurrent() { return (current != nullptr); }
...
std::string Embedded::import_script(const std::string& fileName) {
    try {
        std::string moduleName = vox::utility::FileSystem::getFileName(fileName);
        ...
        if (m_mainScope.contains(moduleName.c_str())) {
            ...
            sys.attr("path").attr("append")(vox::utility::FileSystem::getFilePath(fileName).c_str());
