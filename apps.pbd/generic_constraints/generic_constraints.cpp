...
#include "vox.pbd/position_based_generic_constraints.h"
...
void GenericDistanceConstraint::constraintFct(const unsigned int numberOfParticles,
                                              const Real mass[],
                                              const Vector3r x[],
                                              void *userData,
...
void GenericDistanceConstraint::gradientFct(const unsigned int i,
...
bool GenericDistanceConstraint::initConstraint(SimulationModel &model,
...
bool GenericDistanceConstraint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    ...
    const bool res = PositionBasedGenericConstraints::solve_GenericConstraint<2, 1>(
            invMass, x, &m_restLength, GenericDistanceConstraint::constraintFct, GenericDistanceConstraint::gradientFct,
            corr);

    if (res) {
        const Real stiffness = m_stiffness;
        if (invMass1 != 0.0) x1 += stiffness * corr[0];
        if (invMass2 != 0.0) x2 += stiffness * corr[1];
    }
    return res;
}
...
void GenericIsometricBendingConstraint::constraintFct(const unsigned int numberOfParticles,
                                                      const Real invMass[],
                                                      const Vector3r x[],
                                                      void *userData,
                                                      Eigen::Matrix<Real, 1, 1> &constraintValue) {
    auto *Q = (Matrix4r *)userData;
...
bool GenericIsometricBendingConstraint::initConstraint(SimulationModel &model,
                                                       const unsigned int particle1,
                                                       const unsigned int particle2,
...
bool GenericIsometricBendingConstraint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    ...
    const bool res = PositionBasedGenericConstraints::solve_GenericConstraint<4, 1>(
            invMass, x, &m_Q, GenericIsometricBendingConstraint::constraintFct,
            // GenericIsometricBendingConstraint::gradientFct,
            corr);

    if (res) {
        if (invMass0 != 0.0) x0 += m_stiffness * corr[0];
        if (invMass1 != 0.0) x1 += m_stiffness * corr[1];
        if (invMass2 != 0.0) x2 += m_stiffness * corr[2];
        if (invMass3 != 0.0) x3 += m_stiffness * corr[3];
    }
    return res;
}
...
void GenericHingeJoint::constraintFct(const unsigned int numberOfRigidBodies,
                                      const Real mass[],
                                      const Vector3r x[],
...
void GenericHingeJoint::gradientFct(const unsigned int i,
...
bool GenericHingeJoint::initConstraint(SimulationModel &model,
    ...
    if (fabs(v.dot(m_jointInfo.col(8))) > 0.99) v = Vector3r(0.0, 1.0, 0.0);
...
bool GenericHingeJoint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    ...
    const bool res = PositionBasedGenericConstraints::solve_GenericConstraint<2, 5>(
            invMass, x, inertiaInverseW, q, &m_jointInfo, GenericHingeJoint::constraintFct,
            GenericHingeJoint::gradientFct, corrX, corrQ);
...
void GenericBallJoint::constraintFct(const unsigned int numberOfRigidBodies,
                                     const Real mass[],
                                     const Vector3r x[],
                                     const Matrix3r inertiaInverseW[],
                                     const Quaternionr q[],
...
bool GenericBallJoint::initConstraint(SimulationModel &model,
                                      const unsigned int rbIndex1,
                                      const unsigned int rbIndex2,
                                      const Vector3r &pos) {
    m_bodies[0] = rbIndex1;
...
bool GenericBallJoint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    ...
    const bool res = PositionBasedGenericConstraints::solve_GenericConstraint<2, 3>(
            invMass, x, inertiaInverseW, q, &m_jointInfo, GenericBallJoint::constraintFct, corrX, corrQ);
...
void GenericSliderJoint::constraintFct(const unsigned int numberOfRigidBodies,
                                       const Real mass[],
                                       const Vector3r x[],
                                       const Matrix3r inertiaInverseW[],
                                       const Quaternionr q[],
...
bool GenericSliderJoint::initConstraint(SimulationModel &model,
                                        const unsigned int rbIndex1,
                                        const unsigned int rbIndex2,
                                        const Vector3r &pos,
                                        const Vector3r &axis) {
    ...
    Vector3r v(1.0, 0.0, 0.0);
    // check if vectors are parallel
    if (fabs(v.dot(A.col(0))) > 0.99) v = Vector3r(0.0, 1.0, 0.0);
...
bool GenericSliderJoint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    ...
    Quaternionr corrQ[2];
    const bool res = PositionBasedGenericConstraints::solve_GenericConstraint<2, 5>(
            invMass, x, inertiaInverseW, q, &m_jointInfo, GenericSliderJoint::constraintFct, corrX, corrQ);
