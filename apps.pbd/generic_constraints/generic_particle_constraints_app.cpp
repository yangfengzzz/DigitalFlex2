...
void createMesh() {
    ...
    model->addRegularTriangleModel(nCols, nRows, Vector3r(0, 1, 0), AngleAxisr(M_PI * 0.5, Vector3r(1, 0, 0)).matrix(),
                                   Vector2r(width, height));
    ...
    for (unsigned int cm = 0; cm < model->getTriangleModels().size(); cm++) {
        ...
        utility::IndexedFaceMesh& mesh = model->getTriangleModels()[cm]->getParticleMesh();
        const unsigned int nEdges = mesh.numEdges();
        const utility::IndexedFaceMesh::Edge* edges = mesh.getEdges().data();
...
void TW_CALL setBendingStiffness(const void* value, void* clientData) {
    ...
    ((SimulationModel*)clientData)
            ->setConstraintValue<GenericIsometricBendingConstraint, Real,
                                 &GenericIsometricBendingConstraint::m_stiffness>(bendingStiffness);
}
...
void TW_CALL setDistanceStiffness(const void* value, void* clientData) {
    ...
    ((SimulationModel*)clientData)
            ->setConstraintValue<GenericDistanceConstraint, Real, &GenericDistanceConstraint::m_stiffness>(
                    distanceStiffness);
}
