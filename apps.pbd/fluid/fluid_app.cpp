...
#include "apps.pbd/fluid/fluid_model_ts.h"
...
int main(int argc, char **argv) {
    ...
    std::string logPath = utility::FileSystem::normalizePath(utility::FileSystem::getProgramPath() + "/log");
    utility::FileSystem::makeDirs(logPath);

    exePath = utility::FileSystem::getProgramPath();
    ...
    TwAddVarCB(MiniGL::getTweakBar(), "TimeStepSize", TW_TYPE_REAL, setTimeStep, getTimeStep, &model,
               " label='Time step size'  min=0.0 max = 0.1 step=0.001 precision=4 group=Simulation ");
    TwType enumType = TwDefineEnum("VelocityUpdateMethodType", nullptr, 0);
    TwAddVarCB(
            MiniGL::getTweakBar(), "VelocityUpdateMethod", enumType, setVelocityUpdateMethod, getVelocityUpdateMethod,
            &simulation,
            " label='Velocity update method' enum='0 {First Order Update}, 1 {Second Order Update}' group=Simulation");
    TwAddVarCB(MiniGL::getTweakBar(), "Viscosity", TW_TYPE_REAL, setViscosity, getViscosity, &model,
               " label='Viscosity'  min=0.0 max = 0.5 step=0.001 precision=4 group=Simulation ");

    buildModel();

    if (context_major_version >= 3) createSphereBuffers((Real)particleRadius, 8);
...
void cleanup() {
    ...
    if (context_major_version >= 3) releaseSphereBuffers();
}
...
void mouseMove(int x, int y, void *clientData) {
    ...
    for (unsigned int selectedParticle : selectedParticles) {
        pd.getVelocity(selectedParticle) += 5.0 * diff / h;
    }
...
void timeStep() {
    ...
    // Simulation code
    for (unsigned int i = 0; i < 8; i++) simulation.step(model);
}
...
void render() {
    ...
    for (unsigned int selectedParticle : selectedParticles) {
        MiniGL::drawSphere(pd.getPosition(selectedParticle), 0.08f, red);
    }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)width; i++) {
            for (unsigned int j = 0; j < height; j++) {
                for (unsigned int k = 0; k < depth; k++) {
                    fluidParticles[i * height * depth + j * depth + k] =
                            diam * Vector3r((Real)i, (Real)j, (Real)k) + Vector3r(startX, startY, startZ);
                }
            }
        }
    }
    ...
    model.initModel((unsigned int)fluidParticles.size(), fluidParticles.data(), (unsigned int)boundaryParticles.size(),
                    boundaryParticles.data());

    LOGI("Number of particles: {}", width * height * depth)
}
...
void addWall(const Vector3r &minX, const Vector3r &maxX, std::vector<Vector3r> &boundaryParticles) {
    ...
    const auto startIndex = (unsigned int)boundaryParticles.size();
            ...
            for (unsigned int k = 0; k < stepsY; k++) {
                for (unsigned int l = 0; l < stepsZ; l++) {
                    const Vector3r currPos =
                            minX + Vector3r(j * particleDistance, k * particleDistance, l * particleDistance);
...
void createSphereBuffers(Real radius, int resolution) {
    ...
    for (int w = 0; w < resolution; w++) {
        for (int h = (-resolution / 2); h < (resolution / 2); h++) {
            ...
            v.emplace_back(radius1 * X1, Z1, radius1 * Y1);
            v.emplace_back(radius1 * X2, Z1, radius1 * Y2);
            v.emplace_back(radius2 * X2, Z2, radius2 * Y2);
            ...
            v.emplace_back(radius1 * X1, Z1, radius1 * Y1);
            v.emplace_back(radius2 * X2, Z2, radius2 * Y2);
            v.emplace_back(radius2 * X1, Z2, radius2 * Y1);
            ...
            n.emplace_back(X1, Z1, Y1);
            n.emplace_back(X2, Z1, Y2);
            n.emplace_back(X2, Z2, Y2);
            n.emplace_back(X1, Z1, Y1);
            n.emplace_back(X2, Z2, Y2);
            n.emplace_back(X1, Z2, Y1);
        }
    }

    for (auto &i : n) i.normalize();
...
void renderSphere(const Vector3r &x, const float color[]) {
    ...
    glDrawElements(GL_TRIANGLES, (GLsizei)vertexBufferSize, GL_UNSIGNED_SHORT, nullptr);
    glPopMatrix();
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
