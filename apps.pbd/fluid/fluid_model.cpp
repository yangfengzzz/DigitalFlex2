...
FluidModel::FluidModel() : m_particles() {
...
FluidModel::~FluidModel() { cleanupModel(); }
...
ParticleData& FluidModel::getParticles() { return m_particles; }
...
#pragma omp for schedule(static)
        for (int i = 0; i < nParticles; i++) {
            m_particles.setMass(i, static_cast<Real>(0.8) * diam * diam * diam *
                                           m_density0);  // each particle represents a cube with a side length of r
                                                         // mass is slightly reduced to prevent pressure at the
...
void FluidModel::initModel(const unsigned int nFluidParticles,
