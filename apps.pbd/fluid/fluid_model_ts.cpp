...
TimeStepFluidModel::TimeStepFluidModel() = default;
...
void TimeStepFluidModel::step(FluidModel &model) {
    ...
    model.getNeighborhoodSearch()->neighborhoodSearch(&model.getParticles().getPosition(0),
                                                      model.numBoundaryParticles(), &model.getBoundaryX(0));
    STOP_TIMING_AVG
    ...
    for (unsigned int i = 0; i < pd.size(); i++) {
        if (m_velocityUpdateMethod == 0)
            ...
            TimeIntegration::velocityUpdateSecondOrder(h, pd.getMass(i), pd.getPosition(i), pd.getOldPosition(i),
                                                       pd.getLastPosition(i), pd.getVelocity(i));
    }
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            Real density_err;
            PositionBasedFluids::computePBFDensity(i, numParticles, &pd.getPosition(0), &pd.getMass(0),
                                                   &model.getBoundaryX(0), &model.getBoundaryPsi(0), numNeighbors[i],
                                                   neighbors[i], model.getDensity0(), true, density_err, density);
        }
    }
}
...
/** Update time step size by CFL condition.
 */
void TimeStepFluidModel::updateTimeStepSizeCFL(FluidModel &model,
                                               const Real minTimeStepSize,
                                               const Real maxTimeStepSize) {
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            for (unsigned int j = 0; j < numNeighbors[i]; j++) {
                ...
                // 				else
                // 				{
                // 					const Vector3r &xj = model.getBoundaryX(neighborIndex -
                // numParticles); 					vi -= viscosity * (model.getBoundaryPsi(neighborIndex
                // - numParticles) / density_i) * (vi)* CubicKernel::W(xi - xj);
                // 				}
            }
        }
    }
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)nParticles; i++) {
                Real density_err;
                PositionBasedFluids::computePBFDensity(i, nParticles, &pd.getPosition(0), &pd.getMass(0),
                                                       &model.getBoundaryX(0), &model.getBoundaryPsi(0),
                                                       numNeighbors[i], neighbors[i], model.getDensity0(), true,
                                                       density_err, model.getDensity(i));
                PositionBasedFluids::computePBFLagrangeMultiplier(i, nParticles, &pd.getPosition(0), &pd.getMass(0),
                                                                  &model.getBoundaryX(0), &model.getBoundaryPsi(0),
                                                                  model.getDensity(i), numNeighbors[i], neighbors[i],
                                                                  model.getDensity0(), true, model.getLambda(i));
            }
        }
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)nParticles; i++) {
                Vector3r corr;
                PositionBasedFluids::solveDensityConstraint(i, nParticles, &pd.getPosition(0), &pd.getMass(0),
                                                            &model.getBoundaryX(0), &model.getBoundaryPsi(0),
                                                            numNeighbors[i], neighbors[i], model.getDensity0(), true,
                                                            &model.getLambda(0), corr);
