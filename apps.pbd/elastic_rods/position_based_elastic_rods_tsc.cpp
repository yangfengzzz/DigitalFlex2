...
PositionBasedElasticRodsTSC::PositionBasedElasticRodsTSC() : TimeStepController() { m_damping = 0.0; }

PositionBasedElasticRodsTSC::~PositionBasedElasticRodsTSC() = default;

void PositionBasedElasticRodsTSC::step(SimulationModel &model) {
    TimeManager *tm = TimeManager::getCurrent();
    const Real hOld = tm->getTimeStepSize();
    auto &ermodel = (PositionBasedElasticRodsModel &)model;
...
#pragma omp for schedule(static)
            ...
            for (int i = 0; i < (int)pg.size(); i++) {
                ...
                TimeIntegration::semiImplicitEuler(h, pg.getMass(i), pg.getPosition(i), pg.getVelocity(i),
                                                   pg.getAcceleration(i));
            }
        }
...
#pragma omp for schedule(static)
            for (int i = 0; i < (int)pg.size(); i++) {
                if (m_velocityUpdateMethod == 0)
                    TimeIntegration::velocityUpdateFirstOrder(h, pg.getMass(i), pg.getPosition(i), pg.getOldPosition(i),
                                                              pg.getVelocity(i));
                else
                    TimeIntegration::velocityUpdateSecondOrder(h, pg.getMass(i), pg.getPosition(i),
                                                               pg.getOldPosition(i), pg.getLastPosition(i),
                                                               pg.getVelocity(i));
            }
        }
    }
...
#pragma omp for schedule(static) nowait
    ...
    if (m_collisionDetection) m_collisionDetection->collisionDetection(model);
