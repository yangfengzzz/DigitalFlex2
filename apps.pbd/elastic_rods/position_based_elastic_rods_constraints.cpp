...
bool PerpendiculaBisectorConstraint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    auto &simModel = static_cast<PositionBasedElasticRodsModel &>(model);
    ...
    const bool res = PositionBasedElasticRods::solve_PerpendiculaBisectorConstraint(
            x1, invMass1, x2, invMass2, x3, invMass3, 1.0, corr[0], corr[1], corr[2]);

    if (res) {
        if (invMass1 != 0.0f) x1 += corr[0];

        if (invMass2 != 0.0f) x2 += corr[1];

        if (invMass2 != 0.0f) x3 += corr[2];
    }
...
//////////////////////////////////////////////////////////////////////////
bool GhostPointEdgeDistanceConstraint::initConstraint(PositionBasedElasticRodsModel &model,
                                                      const unsigned int particle1,
                                                      const unsigned int particle2,
                                                      const unsigned int particle3) {
    m_bodies[0] = particle1;
...
bool GhostPointEdgeDistanceConstraint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    ...
    Vector3r corr[3];
    const bool res = PositionBasedElasticRods::solve_GhostPointEdgeDistanceConstraint(
            x1, invMass1, x2, invMass2, x3, invMass3, 1.0, m_restLength, corr[0], corr[1], corr[2]);
...
//////////////////////////////////////////////////////////////////////////
bool DarbouxVectorConstraint::initConstraint(PositionBasedElasticRodsModel &model,
                                             const unsigned int particle1,
                                             const unsigned int particle2,
                                             const unsigned int particle3,
                                             const unsigned int particle4,
                                             const unsigned int particle5) {
    m_bodies[0] = particle1;
    m_bodies[1] = particle2;
    m_bodies[2] = particle3;
...
bool DarbouxVectorConstraint::solvePositionConstraint(SimulationModel &model, const unsigned int iter) {
    ...
    bool res = PositionBasedElasticRods::solve_DarbouxVectorConstraint(
            ...
            simModel.getBendingAndTwistingStiffness(), 1.0, simModel.getRestDarbouxVector(), corr[0], corr[1], corr[2],
            corr[3], corr[4]);
    ...
    if (res) {
        ...
        if (invMass4 != 0.0f) x4 += corr[3];

        if (invMass5 != 0.0f) x5 += corr[4];
    }
    return res;
}
