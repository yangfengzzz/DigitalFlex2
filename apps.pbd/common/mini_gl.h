...
public:
    ...
    static void drawVector(const Vector3r &a, const Vector3r &b, float w, float *color);
    /** Renders a closed cylinder between two points.
     */
    static void drawCylinder(const Vector3r &a,
                             const Vector3r &b,
                             const float *color,
                             float radius = 0.02,
                             unsigned int subdivisions = 8);
    static void drawSphere(const Vector3r &translation,
                           float radius,
                           float *color,
                           unsigned int subDivision = 16);
    static void drawQuad(const Vector3r &a,
                         const Vector3r &b,
                         const Vector3r &c,
                         const Vector3r &d,
                         const Vector3r &norm,
    ...
    static void drawTetrahedron(
            const Vector3r &a, const Vector3r &b, const Vector3r &c, const Vector3r &d, float *color);
    static void drawTriangle(
            const Vector3r &a, const Vector3r &b, const Vector3r &c, const Vector3r &norm, float *color);
    static void drawGrid_xz(float *color);
    static void drawGrid_xy(float *color);
    static void drawPoint(const Vector3r &translation, float pointSize, const float *color);
    ...
    static void init(int argc, char **argv, int width, int height, const char *name);
    static void destroy();
    ...
    static bool checkOpenGLVersion(int major_version, int minor_version);
    ...
    static void drawTime(Real time);
    ...
    static void unproject(int x, int y, Vector3r &pos);
    ...
    static void addReshapeFunc(const ReshapeFct& func) { m_reshapeFct.push_back(func); }
    static std::vector<ReshapeFct> &getReshapeFunc() { return m_reshapeFct; }
    static void addKeyboardFunc(const KeyboardFct& func) { m_keyboardFct.push_back(func); }
    static std::vector<KeyboardFct> &getKeyboardFunc() { return m_keyboardFct; }
    static void addCharFunc(const CharFct& func) { m_charFct.push_back(func); }
    static std::vector<CharFct> &getCharFunc() { return m_charFct; }
    static void addMousePressFunc(const MousePressFct& func) { m_mousePressFct.push_back(func); }
    static std::vector<MousePressFct> &getMousePressFunc() { return m_mousePressFct; }
    static void addMouseMoveFunc(const MouseMoveFct& func) { m_mouseMoveFct.push_back(func); }
    static std::vector<MouseMoveFct> &getMouseMoveFunc() { return m_mouseMoveFct; }
    static void addMouseWheelFunc(const MouseWheelFct& func) { m_mouseWheelFct.push_back(func); }
    static std::vector<MouseWheelFct> &getMouseWheelFunc() { return m_mouseWheelFct; }

    static void setBreakPointActive(bool active);
    ...
    static void setRotation(Quaternionr val) { m_rotation = std::move(val); }
