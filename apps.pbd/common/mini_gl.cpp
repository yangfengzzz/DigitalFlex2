...
TwBar *MiniGL::m_tweakBar = nullptr;
...
void (*MiniGL::selectionfunc)(const Vector2i &, const Vector2i &, void *) = nullptr;
void *MiniGL::selectionfuncClientData = nullptr;
void (*MiniGL::mousefunc)(int, int, void *) = nullptr;
...
void MiniGL::bindTexture() { glBindTexture(GL_TEXTURE_2D, MiniGL::m_texId); }

void MiniGL::unbindTexture() { glBindTexture(GL_TEXTURE_2D, 0); }
...
void MiniGL::drawCylinder(
        const Vector3r &a, const Vector3r &b, const float *color, const float radius, const unsigned int subdivisions) {
    float speccolor[4] = {1.0, 1.0, 1.0, 1.0};
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, color);
    glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, color);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, speccolor);
...
void MiniGL::drawMesh(const std::vector<Vector3r> &vertices,
...
void MiniGL::drawQuad(const Vector3r &a,
                      const Vector3r &b,
                      const Vector3r &c,
                      const Vector3r &d,
                      const Vector3r &norm,
...
void MiniGL::setClientSceneFunc(SceneFct func) { scenefunc = std::move(func); }
...
void MiniGL::init(int argc, char **argv, const int width, const int height, const char *name) {
    ...
    LOGI("OpenGL version {}.{}", m_context_major_version, m_context_minor_version);
...
void MiniGL::initTexture() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 3, IMAGE_COLS, IMAGE_ROWS, 0, GL_RGB, GL_UNSIGNED_BYTE,
                 texData);  // Create texture from image data
...
void MiniGL::initTweakBar() {
    ...
    // TwDefine(" TweakBar size='300 900' valueswidth=120 position='5 5' color='96 200 224' text=dark "); // change
    // default tweak bar size and color
...
void MiniGL::initTweakBarParameters() {
    ...
    TwAddVarCB(m_tweakBar, "Wireframe", TW_TYPE_BOOL32, setWireframeCB, getWireframeCB, nullptr,
...
void TW_CALL MiniGL::getWireframeCB(void *value, void *clientData) { *(int *)(value) = drawMode == GL_LINE; }

void TW_CALL MiniGL::setRotationCB(const void *value, void *clientData) {
    const auto *val = (const float *)(value);
...
void MiniGL::destroy() { gluDeleteQuadric(m_sphereQuadric); }
...
void MiniGL::reshape(GLFWwindow *glfw_window, int w, int h) {
    if ((w > 0) && (h > 0)) {
        ...
        for (auto &i : m_reshapeFct) i(m_width, m_height);
        glViewport(0, 0, m_width, m_height);
    }
}

void MiniGL::setClientIdleFunc(IdleFct func) { idlefunc = std::move(func); }

void MiniGL::setClientDestroyFunc(DestroyFct func) { destroyfunc = std::move(func); }
...
void MiniGL::keyboard(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    for (auto &i : m_keyboardFct) {
        if (i(key, scancode, action, mods)) return;
    }
...
void MiniGL::char_callback(GLFWwindow *window, unsigned int codepoint) {
    // Check if registered listener wants the event
    for (auto &i : m_charFct) {
        if (i(codepoint, GLFW_PRESS)) return;
    }
...
void MiniGL::mousePress(GLFWwindow *window, int button, int action, int mods) {
    ...
    // Check if registered listener wants the event
    for (auto &i : m_mousePressFct) {
        if (i(button, action, mods)) return;
    }
    ...
    if ((selectionfunc != nullptr) && (modifier_key == 0)) {
...
void MiniGL::mouseWheel(GLFWwindow *window, double xoffset, double yoffset) {
    ...
    // Check if registered listener wants the event
    for (auto &i : m_mouseWheelFct) {
        if (i(static_cast<int>(mouse_wheel_pos), xoffset, yoffset)) return;
    }
...
void MiniGL::mouseMove(GLFWwindow *window, double x, double y) {
    // Check if registered listener wants the event
    for (auto &i : m_mouseMoveFct) {
        if (i(static_cast<int>(x), static_cast<int>(y))) return;
    }
    ...
    if (mouse_button == GLFW_MOUSE_BUTTON_1) {
        ...
        // translate scene in x/y direction
            ...
            move(-static_cast<Real>(d_x) / static_cast<Real>(20.0), -static_cast<Real>(d_y) / static_cast<Real>(20.0),
                 0);
        }
...
float MiniGL::getZNear() { return znear; }

float MiniGL::getZFar() { return zfar; }

TwBar *MiniGL::getTweakBar() { return m_tweakBar; }
...
Shader *MiniGL::createShader(const std::string &vertexShader,
                             const std::string &geometryShader,
                             const std::string &fragmentShader) {
    if (checkOpenGLVersion(3, 3)) {
        auto *shader = new Shader();

        if (!vertexShader.empty()) shader->compileShaderFile(GL_VERTEX_SHADER, vertexShader);
//        if (!geometryShader.empty()) shader->compileShaderFile(GL_GEOMETRY_SHADER, geometryShader);
        if (!fragmentShader.empty()) shader->compileShaderFile(GL_FRAGMENT_SHADER, fragmentShader);
...
void MiniGL::drawElements() {
    for (auto &l : m_drawLines) {
        drawVector(l.a, l.b, l.lineWidth, l.color);
    }
    for (auto &t : m_drawTriangle) {
...
void MiniGL::setBreakPointActive(const bool active) { m_breakPointActive = active; }

void MiniGL::breakPoint() { breakPointMainLoop(); }

void MiniGL::error_callback(int error, const char *description) { LOGE(description); }

void MiniGL::mainLoop() {
    while (!glfwWindowShouldClose(m_glfw_window)) {
        glfwPollEvents();

        if (idlefunc != nullptr) idlefunc();
        ...
        if (scenefunc != nullptr) scenefunc();
...
void MiniGL::leaveMainLoop() { glfwSetWindowShouldClose(m_glfw_window, 1); }
