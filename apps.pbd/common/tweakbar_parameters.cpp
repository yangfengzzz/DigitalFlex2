...
void TweakBarParameters::createParameterGUI() {
    ...
    TwAddVarCB(MiniGL::getTweakBar(), "TimeStepSize", TW_TYPE_DOUBLE, setTimeStepSizeCB, getTimeStepSizeCB, nullptr,
               " label='Time step size' min=0.000001 max=0.1 step=0.0001 precision=5 help='Set time step size' "
               "group=Simulation ");
}
...
void TweakBarParameters::createParameterObjectGUI(ParameterObject *paramObj) {
    ...
    for (auto &m_objectName : m_objectNames) {
        if (objName == m_objectName) {
            index++;
    ...
    for (unsigned int i = 0; i < numParams; i++) {
        ...
        m_params.push_back(std::make_unique<ParameterIndex>(paramObj, i));
        ...
        std::string tmp;
        if (!paramBase->getHotKey().empty()) tmp = "key='" + paramBase->getHotKey() + "' ";
        ...
        if (paramBase->getType() == RealParameterType) {
            sprintf(str, " label='%s' help='%s' precision=5 step=0.01 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_REAL, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::UINT32) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_UINT32, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::UINT16) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_UINT16, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::UINT8) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_UINT8, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::INT32) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_INT32, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::INT16) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_INT16, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::INT8) {
            sprintf(str, " label='%s' help='%s' step=1 group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_INT8, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
            ...
            auto *enumParam = static_cast<EnumParameter *>(paramBase);
            ...
            TwType enumType = TwDefineEnum(enumTypeName, nullptr, 0);
            sprintf(str, " label='%s' help='%s' group='%s' %s %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str(), enumStr.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, enumType, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == ParameterBase::BOOL) {
            sprintf(str, " label='%s' help='%s' group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_BOOL32, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        } else if (paramBase->getType() == RealVectorParameterType) {
            if (static_cast<VectorParameter<Real> *>(paramBase)->getDim() == 3) {
                sprintf(str, " label='%s' help='%s' group='%s' %s ", paramBase->getLabel().c_str(),
                        paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
                TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_DIR3R, setParameterValue, getParameterValue,
                           m_params[m_params.size() - 1].get(), str);
            }
        } else if (paramBase->getType() == ParameterBase::STRING) {
            sprintf(str, " label='%s' help='%s' group='%s' %s ", paramBase->getLabel().c_str(),
                    paramBase->getDescription().c_str(), paramBase->getGroup().c_str(), tmp.c_str());
            TwAddVarCB(MiniGL::getTweakBar(), varName, TW_TYPE_STDSTRING, setParameterValue, getParameterValue,
                       m_params[m_params.size() - 1].get(), str);
        }
    }
}
...
void TW_CALL TweakBarParameters::setParameterValue(const void *value, void *clientData) {
    ...
    if (paramBase->getType() == RealParameterType) {
        ...
        const auto *srcPtr = static_cast<const std::string *>(value);
...
void TW_CALL TweakBarParameters::getParameterValue(void *value, void *clientData) {
    ...
    if (paramBase->getType() == RealParameterType) {
        ...
        *(bool *)(value) = val;
        ...
        auto *destPtr = static_cast<std::string *>(value);
