...
int main(int argc, char **argv) {
    ...
    TwAddVarCB(MiniGL::getTweakBar(), "poissonRatio", TW_TYPE_REAL, setPoissonRatio, getPoissonRatio, model,
               " label='Poisson ratio'  min=0.0 step=0.1 precision=4 group='Solid' ");
    TwAddVarCB(MiniGL::getTweakBar(), "normalizeStretch", TW_TYPE_BOOL32, setNormalizeStretch, getNormalizeStretch,
               model, " label='Normalize stretch' group='Solid' ");
    TwAddVarCB(MiniGL::getTweakBar(), "normalizeShear", TW_TYPE_BOOL32, setNormalizeShear, getNormalizeShear, model,
               " label='Normalize shear' group='Solid' ");
    TwAddVarCB(MiniGL::getTweakBar(), "volumeStiffness", TW_TYPE_REAL, setVolumeStiffness, getVolumeStiffness, model,
               " label='Volume stiffness'  min=0.0 step=0.1 precision=4 group='Solid' ");
...
void buildModel() {
    ...
    // floor
    rb[0] = new RigidBody();
    rb[0]->initBody(1.0, Vector3r(0.0, -5.5, 0.0), Quaternionr(1.0, 0.0, 0.0, 0.0), vd, mesh,
                    Vector3r(100.0, 1.0, 100.0));
    rb[0]->setMass(0.0);

    // torus
    rb[1] = new RigidBody();
    rb[1]->initBody(1.0, Vector3r(5.0, -1.5, 0.0), Quaternionr(1.0, 0.0, 0.0, 0.0), vdTorus, meshTorus,
    ...
    const auto nVert1 = static_cast<unsigned int>(vertices1.size());
    ...
    const auto nVert2 = static_cast<unsigned int>(vertices2.size());
    ...
    for (unsigned int i = 0; i < tm.size(); i++) {
        ...
        cd.addCollisionObjectWithoutGeometry(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
...
void createMesh() {
    SimulationModel *model = Simulation::getCurrent()->getModel();
    model->addRegularTetModel(width, height, depth, Vector3r(4.5, 3, 0), Matrix3r::Identity(), Vector3r(9.0, 1.5, 1.5));
    ...
    for (unsigned int cm = 0; cm < model->getTetModels().size(); cm++) {
        ...
        LOGI("Number of vertices: {}", width * height * depth)
    }
}
...
void TW_CALL setStiffness(const void *value, void *clientData) {
    ...
    ((SimulationModel *)clientData)
            ->setConstraintValue<StrainTetConstraint, Real, &StrainTetConstraint::m_shearStiffness>(stiffness);
    ((SimulationModel *)clientData)
            ->setConstraintValue<DistanceConstraint, Real, &DistanceConstraint::m_stiffness>(stiffness);
    ((SimulationModel *)clientData)
            ->setConstraintValue<DistanceConstraint_XPBD, Real, &DistanceConstraint_XPBD::m_stiffness>(stiffness);
    ((SimulationModel *)clientData)
            ->setConstraintValue<ShapeMatchingConstraint, Real, &ShapeMatchingConstraint::m_stiffness>(stiffness);
}

void TW_CALL getStiffness(void *value, void *clientData) { *(Real *)(value) = stiffness; }
...
void TW_CALL getPoissonRatio(void *value, void *clientData) { *(Real *)(value) = poissonRatio; }
...
void TW_CALL setNormalizeStretch(const void *value, void *clientData) {
    ...
    ((SimulationModel *)clientData)
            ->setConstraintValue<StrainTetConstraint, bool, &StrainTetConstraint::m_normalizeStretch>(normalizeStretch);
}
...
void TW_CALL setNormalizeShear(const void *value, void *clientData) {
    ...
    ((SimulationModel *)clientData)
            ->setConstraintValue<StrainTetConstraint, bool, &StrainTetConstraint::m_normalizeShear>(normalizeShear);
}
