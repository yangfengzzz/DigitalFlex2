...
int main(int argc, char **argv) {
    ...
    TwType enumType = TwDefineEnum("VelocityUpdateMethodType", nullptr, 0);
...
void readScene(const bool readFile) {
    ...
    if (readFile && (data.m_tetModelSimulationMethod != -1)) solidSimulationMethod = data.m_tetModelSimulationMethod;
    if (readFile && (data.m_triangleModelBendingMethod != -1)) bendingMethod = data.m_triangleModelBendingMethod;
    ...
    for (unsigned int i = 0; i < data.m_rigidBodyData.size(); i++) {
        ...
        switch (rbd.m_collisionObjectType) {
            ...
            case SceneLoader::Sphere:
                cd.addCollisionSphere(i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                      vertices.data(), nVert, rbd.m_collisionObjectScale[0], rbd.m_testMesh,
                                      rbd.m_invertSDF);
                break;
            case SceneLoader::Box:
                cd.addCollisionBox(i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                   vertices.data(), nVert, rbd.m_collisionObjectScale, rbd.m_testMesh, rbd.m_invertSDF);
                break;
            case SceneLoader::Cylinder:
                cd.addCollisionCylinder(i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                        vertices.data(), nVert, rbd.m_collisionObjectScale.head<2>(), rbd.m_testMesh,
                                        rbd.m_invertSDF);
                break;
            case SceneLoader::Torus:
                cd.addCollisionTorus(i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                     vertices.data(), nVert, rbd.m_collisionObjectScale.head<2>(), rbd.m_testMesh,
                                     rbd.m_invertSDF);
                break;
            case SceneLoader::HollowSphere:
                cd.addCollisionHollowSphere(i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
            ...
            case SceneLoader::HollowBox:
                cd.addCollisionHollowBox(i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
            ...
            case SceneLoader::SDF: {
                if (rbd.m_collisionObjectFileName.empty()) {
                    ...
                    const string sdfFileName =
                            FileSystem::normalizePath(cachePath + "/" + modelFileName + "_" + resStr + ".csdf");
                    cd.addCubicSDFCollisionObject(i, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
    ...
    for (auto &collisionObject : collisionObjects) {
        if (cd.isDistanceFieldCollisionObject(collisionObject) &&
            ...
            const unsigned int modelIndex = collisionObject->m_bodyIndex;
            TetModel *tm = tetModels[modelIndex];
            const unsigned int offset = tm->getIndexOffset();
            const IndexedTetMesh &mesh = tm->getParticleMesh();

            ((DistanceFieldCollisionDetection::DistanceFieldCollisionObject *)collisionObject)
                    ->initTetBVH(&pd.getPosition(offset), mesh.numVertices(), mesh.getTets().data(), mesh.numTets(),
...
void exportMeshOBJ(const std::string &exportFileName,
                   const unsigned int nVert,
                   const Vector3r *pos,
    ...
    if (!outfile) {
        LOGW("Cannot open a file to save OBJ mesh.")
        return;
    }
...
void exportOBJ() {
    if (!enableExportOBJ) return;

    if (TimeManager::getCurrent()->getTime() < nextFrameTime) return;
