...
#include "stiff_rods_scene_loader.h"
...
int main(int argc, char **argv) {
    ...
    if (!triModels.empty()) {
        TwType enumType2 = TwDefineEnum("ClothSimulationMethodType", nullptr, 0);
        TwAddVarCB(MiniGL::getTweakBar(), "ClothSimulationMethod", enumType2, setClothSimulationMethod,
                   getClothSimulationMethod, &clothSimulationMethod,
                   " label='Cloth sim. method' enum='0 {None}, 1 {Distance constraints}, 2 {FEM based PBD}, 3 {Strain "
    ...
    if (!tetModels.empty()) {
        ...
        TwAddVarCB(MiniGL::getTweakBar(), "poissonRatio", TW_TYPE_REAL, setSolidPoissonRatio, getSolidPoissonRatio,
                   model, " label='Poisson ratio'  min=0.0 step=0.1 precision=4 group='Solid' ");
        TwAddVarCB(MiniGL::getTweakBar(), "normalizeStretch", TW_TYPE_BOOL32, setSolidNormalizeStretch,
                   getSolidNormalizeStretch, model, " label='Normalize stretch' group='Solid' ");
        TwAddVarCB(MiniGL::getTweakBar(), "normalizeShear", TW_TYPE_BOOL32, setSolidNormalizeShear,
                   getSolidNormalizeShear, model, " label='Normalize shear' group='Solid' ");
        TwAddVarCB(MiniGL::getTweakBar(), "volumeStiffness", TW_TYPE_REAL, setVolumeStiffness, getVolumeStiffness,
                   model, " label='Volume stiffness'  min=0.0 step=0.1 precision=4 group='Solid' ");
    }
    if (!triModels.empty()) {
        TwType enumType4 = TwDefineEnum("BendingMethodType", nullptr, 0);
        TwAddVarCB(MiniGL::getTweakBar(), "BendingMethod", enumType4, setBendingMethod, getBendingMethod,
                   &bendingMethod,
                   " label='Bending method' enum='0 {None}, 1 {Dihedral angle}, 2 {Isometric bending}, 3 {XPBD "
                   "isometric bending}' group=Bending");
        TwAddVarCB(MiniGL::getTweakBar(), "BendingStiffness", TW_TYPE_REAL, setBendingStiffness, getBendingStiffness,
                   model, " label='Bending stiffness'  min=0.0 step=0.1 precision=4 group='Bending' ");
    }
...
void initTriangleModelConstraints() {
    ...
    for (unsigned int cm = 0; cm < model->getTriangleModels().size(); cm++) {
        ...
        if (clothSimulationMethod == 4) distanceStiffness = 100000;
        model->addClothConstraints(model->getTriangleModels()[cm], clothSimulationMethod, distanceStiffness,
                                   xxStiffness, yyStiffness, xyStiffness, xyPoissonRatio, yxPoissonRatio,
...
CubicSDFCollisionDetection::GridPtr generateSDF(const std::string &modelFile,
    ...
    if (collisionObjectFileName.empty()) {
        ...
        if (foundCacheFile && md5) {
            ...
            LOGI("Generate SDF for {}", modelFile)
...
void readScene() {
    ...
    auto *loader = static_cast<StiffRodsSceneLoader *>(base->getSceneLoader());
    ...
    if (data.m_triangleModelSimulationMethod != -1) clothSimulationMethod = data.m_triangleModelSimulationMethod;
    if (data.m_tetModelSimulationMethod != -1) solidSimulationMethod = data.m_tetModelSimulationMethod;
    if (data.m_triangleModelBendingMethod != -1) bendingMethod = data.m_triangleModelBendingMethod;
    ...
    for (auto &tmd : data.m_tetModelData) {
        ...
        if (distanceFields.find(sdfKey) == distanceFields.end()) {
            ...
            if (tmd.m_collisionObjectType == SceneLoader::SDF) {
                ...
                CubicSDFCollisionDetection::GridPtr distanceField =
                        generateSDF(tmd.m_modelFileVis, tmd.m_collisionObjectFileName, tmd.m_resolutionSDF, vd, mesh);
    ...
    for (auto &rbd : data.m_rigidBodyData) {
        ...
        rb[rbIndex]->initBody(rbd.m_density, rbd.m_x, rbd.m_q, vd, mesh, rbd.m_scale);
        ...
        switch (rbd.m_collisionObjectType) {
            ...
            case SceneLoader::HollowSphere:
                cd.addCollisionHollowSphere(rbIndex, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                            vertices.data(), nVert, rbd.m_collisionObjectScale[0], rbd.m_thicknessSDF,
                                            rbd.m_testMesh, rbd.m_invertSDF);
                break;
            case SceneLoader::HollowBox:
                cd.addCollisionHollowBox(rbIndex, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                         vertices.data(), nVert, rbd.m_collisionObjectScale, rbd.m_thicknessSDF,
                                         rbd.m_testMesh, rbd.m_invertSDF);
                break;
            case SceneLoader::SDF: {
                if (rbd.m_collisionObjectFileName.empty()) {
                    ...
                    cd.addCubicSDFCollisionObject(rbIndex,
                                                  ...
                                                  rbd.m_collisionObjectScale, rbd.m_testMesh, rbd.m_invertSDF);
                } else
                    cd.addCubicSDFCollisionObject(rbIndex,
                                                  CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
                                                  vertices.data(), nVert, distanceFields[rbd.m_collisionObjectFileName],
                                                  rbd.m_collisionObjectScale, rbd.m_testMesh, rbd.m_invertSDF);
                break;
            }
        }
    ...
    if (!data.m_TreeModels.empty()) LOGI("Loading {} stiff rods", data.m_TreeModels.size())
    ...
    unsigned int lengthIdx(1), widthIdx(0),
            heightIdx(2);  // y-axis is used as the length axis to be compatible to Blender's armatures (Blender 2.78)
    ...
    for (auto &treeModel : data.m_TreeModels) {
        ...
        for (size_t idx(0); idx < data.m_rigidBodyData.size(); ++idx) {
            ...
            if (treeModel.m_SegmentIds.find(segmentData.m_id) == treeModel.m_SegmentIds.end()) continue;

            if (objFiles.find(segmentData.m_modelFile) == objFiles.end()) continue;

            // continue if segment has already been created because it is also a member of another tree
            if (id_index.find(segmentData.m_id) != id_index.end()) continue;
            ...
            const Real Iy = static_cast<Real>(1. / 12.) * mass *
                            (static_cast<Real>(3.) * (static_cast<Real>(0.25) * height * height) + length * length);
            const Real Iz = static_cast<Real>(1. / 12.) * mass *
                            (static_cast<Real>(3.) * (static_cast<Real>(0.25) * width * width) + length * length);
            const Real Ix =
                    static_cast<Real>(0.25) * mass * (static_cast<Real>(0.25) * (height * height + width * width));
            ...
            rb[rbIndex] = new RigidBody();
            rb[rbIndex]->initBody(mass, modelPosition + modelRotationMat * segmentData.m_x, inertiaTensor,
                                  modelRotationQuat * segmentData.m_q, vd, mesh, scale);
        ...
        std::vector<Real> youngsModuli(treeModel.m_StretchBendingTwistingConstraintIds.size(),
                                       treeModel.m_YoungsModulus);
        std::vector<Real> torsionModuli(treeModel.m_StretchBendingTwistingConstraintIds.size(),
                                        treeModel.m_TorsionModulus);
        ...
        for (unsigned int cID : treeModel.m_StretchBendingTwistingConstraintIds) {
            if (cIDTocData.find(cID) == cIDTocData.end()) continue;

            auto &sbtConstraint = *cIDTocData[cID];

            auto itFirstIdx(id_index.find(sbtConstraint.m_SegmentID[0])),
                    itSecondIdx(id_index.find(sbtConstraint.m_SegmentID[1]));

            if (itFirstIdx == id_index.end() || itSecondIdx == id_index.end()) continue;

            constraintSegmentIndices.emplace_back(itFirstIdx->second, itSecondIdx->second);
            ...
            Real avgLength(static_cast<Real>(0.5) * (segmentScale[sbtConstraint.m_SegmentID[0]][lengthIdx] +
                                                     segmentScale[sbtConstraint.m_SegmentID[1]][lengthIdx]));
            ...
            averageRadii.push_back(static_cast<Real>(0.125) * (segmentScale[sbtConstraint.m_SegmentID[0]][widthIdx] +
                                                               ...
                                                               segmentScale[sbtConstraint.m_SegmentID[1]][widthIdx] +
                                                               segmentScale[sbtConstraint.m_SegmentID[1]][heightIdx]));
        }

        model->addDirectPositionBasedSolverForStiffRodsConstraint(constraintSegmentIndices, constraintPositions,
    ...
    for (unsigned int i = 0; i < data.m_triangleModelData.size(); i++) {
        ...
        for (unsigned int m_staticParticle : tmd.m_staticParticles) {
            const unsigned int index = m_staticParticle + offset;
    ...
    for (unsigned int i = 0; i < data.m_tetModelData.size(); i++) {
        ...
        switch (tmd.m_collisionObjectType) {
            case SceneLoader::No_Collision_Object:
                cd.addCollisionObjectWithoutGeometry(i,
                                                     CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                                     &pd.getPosition(offset), nVert, true);
                break;
            case SceneLoader::Sphere:
                cd.addCollisionSphere(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                      &pd.getPosition(offset), nVert, tmd.m_collisionObjectScale[0], tmd.m_testMesh,
                                      tmd.m_invertSDF);
                break;
            case SceneLoader::Box:
                cd.addCollisionBox(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                   &pd.getPosition(offset), nVert, tmd.m_collisionObjectScale, tmd.m_testMesh,
                                   tmd.m_invertSDF);
                break;
            case SceneLoader::Cylinder:
                cd.addCollisionCylinder(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                        &pd.getPosition(offset), nVert, tmd.m_collisionObjectScale.head<2>(),
                                        tmd.m_testMesh, tmd.m_invertSDF);
                break;
            case SceneLoader::Torus:
                cd.addCollisionTorus(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                     &pd.getPosition(offset), nVert, tmd.m_collisionObjectScale.head<2>(),
                                     tmd.m_testMesh, tmd.m_invertSDF);
                break;
            case SceneLoader::HollowSphere:
                cd.addCollisionHollowSphere(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                            &pd.getPosition(offset), nVert, tmd.m_collisionObjectScale[0],
                                            tmd.m_thicknessSDF, tmd.m_testMesh, tmd.m_invertSDF);
                break;
            case SceneLoader::HollowBox:
                cd.addCollisionHollowBox(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                         &pd.getPosition(offset), nVert, tmd.m_collisionObjectScale, tmd.m_thicknessSDF,
                                         tmd.m_testMesh, tmd.m_invertSDF);
                break;
            case SceneLoader::SDF: {
                if (tmd.m_collisionObjectFileName.empty()) {
                    ...
                    cd.addCubicSDFCollisionObject(i, CollisionDetection::CollisionObject::TetModelCollisionObjectType,
                                                  &pd.getPosition(offset), nVert,
                                                  distanceFields[tmd.m_collisionObjectFileName],
                                                  tmd.m_collisionObjectScale, tmd.m_testMesh, tmd.m_invertSDF);
                break;
            }
        }
...
void TW_CALL getClothSimulationMethod(void *value, void *clientData) { *(short *)(value) = *((short *)clientData); }
...
void TW_CALL getSolidSimulationMethod(void *value, void *clientData) { *(short *)(value) = *((short *)clientData); }
...
void TW_CALL setSolidStiffness(const void *value, void *clientData) {
    ...
    ((SimulationModel *)clientData)
            ->setConstraintValue<DistanceConstraint, Real, &DistanceConstraint::m_stiffness>(solidStiffness);
    ((SimulationModel *)clientData)
            ->setConstraintValue<DistanceConstraint_XPBD, Real, &DistanceConstraint_XPBD::m_stiffness>(solidStiffness);
    ((SimulationModel *)clientData)
            ->setConstraintValue<ShapeMatchingConstraint, Real, &ShapeMatchingConstraint::m_stiffness>(solidStiffness);
}

void TW_CALL getSolidStiffness(void *value, void *clientData) { *(Real *)(value) = solidStiffness; }

void TW_CALL setVolumeStiffness(const void *value, void *clientData) {
    volumeStiffness = *(const Real *)(value);
    ((SimulationModel *)clientData)
            ->setConstraintValue<VolumeConstraint, Real, &VolumeConstraint::m_stiffness>(volumeStiffness);
    ((SimulationModel *)clientData)
            ->setConstraintValue<VolumeConstraint_XPBD, Real, &VolumeConstraint_XPBD::m_stiffness>(volumeStiffness);
}

void TW_CALL getVolumeStiffness(void *value, void *clientData) { *(Real *)(value) = volumeStiffness; }

void TW_CALL getSolidPoissonRatio(void *value, void *clientData) { *(Real *)(value) = solidPoissonRatio; }
...
void TW_CALL setSolidNormalizeStretch(const void *value, void *clientData) {
    ...
    ((SimulationModel *)clientData)
            ->setConstraintValue<StrainTetConstraint, bool, &StrainTetConstraint::m_normalizeStretch>(
                    solidNormalizeStretch);
}

void TW_CALL getSolidNormalizeShear(void *value, void *clientData) { *(bool *)(value) = solidNormalizeShear; }

void TW_CALL setSolidNormalizeShear(const void *value, void *clientData) {
    solidNormalizeShear = *(const Real *)(value);
    ((SimulationModel *)clientData)
            ->setConstraintValue<StrainTetConstraint, bool, &StrainTetConstraint::m_normalizeShear>(
                    solidNormalizeShear);
}