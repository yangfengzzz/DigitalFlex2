...
void StiffRodsSceneLoader::parseStiffRodsData(const nlohmann::json &child,
                                              SceneData &sceneData,
                                              const std::string &basePath) {
    if (m_json.find("RodAnimators") != m_json.end()) parseRodAnimators(m_json, "RodAnimators", sceneData);
    ...
    // parse tree data
    if (m_json.find("TreeModels") != m_json.end()) parseTreeModels(m_json, "TreeModels", sceneData);
...
void StiffRodsSceneLoader::parseSkinningMeshes(const nlohmann::json &j,
                                               const std::string &key,
                                               SceneData &sceneData,
                                               const std::string &basePath) {
    const nlohmann::json &child = j[key];
    {
        auto &rodSceneData = static_cast<RodSceneData &>(sceneData);
        ...
        for (auto &sMesh : child) {
            ...
            if (readValue<std::string>(sMesh, "meshFile", meshFileName)) {
                if (utility::FileSystem::isRelativePath(meshFileName)) {
                ...
                axis.setZero();
                Real angle = 0.0;
                if (readVector(sMesh, "rotationAxis", axis) && readValue<Real>(sMesh, "rotationAngle", angle)) {
                    axis.normalize();
...
void StiffRodsSceneLoader::parseTreeModels(const nlohmann::json &j, const std::string &key, SceneData &sceneData) {
    const nlohmann::json &child = j[key];
    {
        ...
        for (auto &tm : child) {
            ...
            axis.setZero();
            Real angle = 0.0;
            if (readVector(tm, "rotationAxis", axis) && readValue<Real>(tm, "rotationAngle", angle)) {
                axis.normalize();
...
void StiffRodsSceneLoader::parseCompactTreeModels(const nlohmann::json &j,
                                                  const std::string &key,
                                                  SceneData &sceneData,
                                                  const std::string &basePath) {
    {
        ...
        for (auto &tm : child) {
            ...
            if (tm.find("segmentPositions") != tm.end()) {
                for (auto &position : tm["segmentPositions"]) {
                    ctmd.m_SegmentPositions.emplace_back(readVectorValues<Real, 3>(position));
                }
            }
            ...
            if (tm.find("segmentOrientationAxes") != tm.end()) {
                for (auto &axis : tm["segmentOrientationAxes"]) {
                    tmpSegmentOrientationAxes.emplace_back(readVectorValues<Real, 3>(axis));
                }
            }

            if (tm.find("segmentOrientationAngles") != tm.end()) {
                for (auto &angle : tm["segmentOrientationAngles"]) {
                    tmpSegmentOrientationAngles.push_back(angle.get<Real>());
                }
            }

            for (size_t idx(0); idx < tmpSegmentOrientationAngles.size() && idx < tmpSegmentOrientationAxes.size();
                 ++idx) {
                Real angle(tmpSegmentOrientationAngles[idx]);
                Vector3r &axis(tmpSegmentOrientationAxes[idx]);
                axis.normalize();
                ctmd.m_SegmentOrientations.emplace_back(AngleAxisr(angle, axis));
            }

            // segment scales
            if (tm.find("segmentScales") != tm.end()) {
                for (auto &scale : tm["segmentScales"]) {
                    ctmd.m_SegmentScales.emplace_back(readVectorValues<Real, 3>(scale));
                }
            }
            ...
            if (tm.find("segmentDensities") != tm.end()) {
                for (auto &density : tm["segmentDensities"]) {
                    ctmd.m_SegmentDensities.push_back(density.get<Real>());
                }
            }
            ...
            if (tm.find("segmentModelFiles") != tm.end()) {
                ...
                for (auto &modelFile : tm["segmentModelFiles"]) {
                    ...
                    if (utility::FileSystem::isRelativePath(geomFileName)) {
            ...
            if (tm.find("segmentFrictionCoefficients") != tm.end()) {
                for (auto &fc : tm["segmentFrictionCoefficients"]) {
                    ctmd.m_SegmentFrictionCoefficients.push_back(fc.get<Real>());
                }
            }
            ...
            if (tm.find("segmentRestitutionCoefficients") != tm.end()) {
                for (auto &rc : tm["segmentRestitutionCoefficients"]) {
                    ctmd.m_SegmentRestitutioCoefficients.push_back(rc.get<Real>());
                }
            }
            ...
            if (tm.find("segmentCollisionCategoryBits") != tm.end()) {
                for (auto &ccb : tm["segmentCollisionCategoryBits"]) {
                    ctmd.m_SegmentCollisionCategoryBits.push_back(ccb.get<short>());
                }
            }
            ...
            if (tm.find("segmentCollisionFilterMaskBits") != tm.end()) {
                for (auto &cfmb : tm["segmentCollisionFilterMaskBits"]) {
                    ctmd.m_SegmentCollisionFilterMaskBits.push_back(cfmb.get<short>());
                }
            }
            ...
            if (tm.find("jointPositions") != tm.end()) {
                for (auto &jp : tm["jointPositions"]) {
                    ctmd.m_SBTConstraintPositions.emplace_back(readVectorValues<Real, 3>(jp));
                }
            }
            ...
            if (tm.find("jointStiffnessModifier") != tm.end()) {
                for (auto &jsm : tm["jointStiffnessModifier"]) {
                    ctmd.m_SBTConstraintStiffnessModifier.emplace_back(readVectorValues<Real, 3>(jsm));
                }
            }
        }
    }
...
void StiffRodsSceneLoader::parseStretchBendingTwistingConstraints(const nlohmann::json &j,
                                                                  const std::string &key,
                                                                  SceneData &sceneData) {
    {
        const nlohmann::json &child = j[key];
        ...
        for (auto &joint : child) {
            rodSceneData.m_StretchBendingTwistingConstraints.resize(
                    rodSceneData.m_StretchBendingTwistingConstraints.size() + 1);
        ...
        for (auto &anim : child) {
            ...
            if (anim.find("data") != anim.end()) {
                for (auto &dataItem : anim["data"]) {
                    ra.m_Data.emplace_back(readVectorValues<Real, 3>(dataItem));
                }
            }
        }
    }
...
void StiffRodsSceneLoader::readStiffRodsScene(const std::string &fileName, SceneData &sceneData) {
    try {
        ...
        std::string basePath = utility::FileSystem::getFilePath(fileName);
        ...
        if (m_json.find("Simulation") != m_json.end()) readSimulation(m_json, "Simulation", sceneData);

        //////////////////////////////////////////////////////////////////////////
        // read rigid bodies
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("RigidBodies") != m_json.end()) readRigidBodies(m_json, "RigidBodies", basePath, sceneData);
        ...
        // read tet models
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("TetModels") != m_json.end()) readTetModels(m_json, "TetModels", basePath, sceneData);

        //////////////////////////////////////////////////////////////////////////
        // read ball joints
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("BallJoints") != m_json.end()) readBallJoints(m_json, "BallJoints", sceneData);
        ...
        // read hinge joints
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("HingeJoints") != m_json.end()) readHingeJoints(m_json, "HingeJoints", sceneData);

        //////////////////////////////////////////////////////////////////////////
        // read universal joints
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("UniversalJoints") != m_json.end()) readUniversalJoints(m_json, "UniversalJoints", sceneData);

        //////////////////////////////////////////////////////////////////////////
        // read slider joints
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("SliderJoints") != m_json.end()) readSliderJoints(m_json, "SliderJoints", sceneData);
        ...
        // read DistanceJoints
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("DistanceJoints") != m_json.end()) readDistanceJoints(m_json, "DistanceJoints", sceneData);
