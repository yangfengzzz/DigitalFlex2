...
public:
    ...
    /** Determine the position corrections for a distance constraint between two
     * particles:\n\n \f$C(\mathbf{p}_0, \mathbf{p}_1) = \| \mathbf{p}_0 -
     * \mathbf{p}_1\| - l_0 = 0\f$\n\n More information can be found in the
     * following papers: \cite Mueller07, \cite BMOT2013, \cite BMOTM2014, \cite
     * BMM2015,
     *
     ...
     * \f$k_{bend}\f$ is a global user parameter defining the bending
     *stiffness.\n\n More information can be found in the following papers: \cite
     *Mueller07, \cite BMOT2013, \cite BMOTM2014, \cite BMM2015,
     *
     * @param p0 position of first particle
     * @param invMass0 inverse mass of first particle
     * @param p1 position of second particle
     ...
     */
    static bool solve_DihedralConstraint(const Vector3r &p0,
                                         Real invMass0,  // angle on (p2, p3) between triangles
                                                         // (p0, p2, p3) and (p1, p3, p2)
                                         const Vector3r &p1,
                                         Real invMass1,
                                         const Vector3r &p2,
                                         Real invMass2,
    ...
    /** Determine the position corrections for a constraint that conserves the
     * volume of single tetrahedron. Such a constraint has the form \f{equation*}{
     * C(\mathbf{p}_1, \mathbf{p}_2, \mathbf{p}_3, \mathbf{p}_4) = \frac{1}{6}
     * \left(\mathbf{p}_{2,1} \times \mathbf{p}_{3,1}\right) \cdot
     * \mathbf{p}_{4,1} - V_0, \f} where \f$\mathbf{p}_1\f$, \f$\mathbf{p}_2\f$,
     * \f$\mathbf{p}_3\f$ and \f$\mathbf{p}_4\f$ are the four corners of the
     * tetrahedron and \f$V_0\f$ is its rest volume.\n\n More information can be
     * found in the following papers: \cite Mueller07, \cite BMOT2013, \cite
     ...
     * @param corr3 position correction of fourth particle
     */
    static bool solve_VolumeConstraint(const Vector3r &p0,
                                       Real invMass0,
                                       const Vector3r &p1,
                                       Real invMass1,
                                       const Vector3r &p2,
     ...
     */
    static bool solve_EdgePointDistanceConstraint(const Vector3r &p,
                                                  Real invMass,
                                                  const Vector3r &p0,
                                                  Real invMass0,
                                                  const Vector3r &p1,
     ...
     */
    static bool solve_TrianglePointDistanceConstraint(const Vector3r &p,
                                                      Real invMass,
                                                      const Vector3r &p0,
                                                      Real invMass0,
                                                      const Vector3r &p1,
     ...
     */
    static bool solve_EdgeEdgeDistanceConstraint(const Vector3r &p0,
                                                 Real invMass0,
                                                 const Vector3r &p1,
                                                 Real invMass1,
                                                 const Vector3r &p2,
     ...
     * @param  Q returns the local stiffness matrix which is required by the
     * solver
     */
     ...
     * @param p3 position of fourth particle
     * @param invMass3 inverse mass of fourth particle
     * @param  Q local stiffness matrix which must be initialized by calling
     ...
     * @param corr3 position correction of fourth particle
     */
    static bool solve_IsometricBendingConstraint(const Vector3r &p0,
                                                 Real invMass0,  // angle on (p2, p3) between triangles
                                                                 // (p0, p2, p3) and (p1, p3, p2)
                                                 const Vector3r &p1,
                                                 Real invMass1,
     ...
     * More information can be found in: \cite BMM2015, \cite BMOT2013, \cite
     * BMOTM2014, \cite Muller2005, \cite Bender2013, \cite Diziol2011
     *
     * @param  x0			rest configuration of all particles in the
     * cluster
     * @param  x			current configuration of all particles in the
     * cluster
     ...
     * @param  corr			position corrections for all particles in
     * the cluster
     ...
     */
    static bool init_StrainTriangleConstraint(const Vector3r &p0,
                                              const Vector3r &p1,
                                              const Vector3r &p2,
                                              Matrix2r &invRestMat);

    /** Solve triangle constraint with strain based dynamics and return position
     * corrections.\n\n More information can be found in: \cite BMM2015, \cite
     * Mueller2014
     *
     * @param p0 position of first particle
     * @param invMass0 inverse mass of first particle
     ...
     * @param p2 position of third particle
     * @param invMass2 inverse mass of third particle
     * @param  invRestMat precomputed matrix determined by
     * init_StrainTriangleConstraint()
     ...
     */
    static bool solve_StrainTriangleConstraint(const Vector3r &p0,
                                               Real invMass0,
                                               const Vector3r &p1,
                                               Real invMass1,
                                               const Vector3r &p2,
    ...
    // has no inversion handling. Possible simple solution: if the volume is
    // negative, scale corrections down and use the volume constraint to fix the
    // volume sign
    static bool solve_StrainTetraConstraint(const Vector3r &p0,
                                            Real invMass0,
                                            const Vector3r &p1,
                                            Real invMass1,
                                            const Vector3r &p2,
...
public:
    ...
    static bool solve_FEMTriangleConstraint(const Vector3r &p0,
                                            Real invMass0,
                                            const Vector3r &p1,
                                            Real invMass1,
                                            const Vector3r &p2,
     ...
     * Recomputation is only necessary when rest shape changes.
     */
    static bool init_FEMTetraConstraint(const Vector3r &p0,
                                        const Vector3r &p1,
                                        const Vector3r &p2,
                                        const Vector3r &p3,
                                        Real &volume,
    ...
    static bool solve_FEMTetraConstraint(const Vector3r &p0,
                                         Real invMass0,
                                         const Vector3r &p1,
                                         Real invMass1,
                                         const Vector3r &p2,
    ...
    /** Perform a solver step for a contact constraint between a particle and a
     * tetrahedron. A contact constraint handles collisions and resting contacts
