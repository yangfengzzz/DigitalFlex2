...
public:
    FileSink(const LogLevel minLevel, const std::string &fileName) : LogSink(minLevel) {
        m_file.open(fileName.c_str(), std::ios::out);
        if (m_file.fail()) std::cerr << "Failed to open file: " << fileName << "\n";
    }

    virtual ~FileSink() { m_file.close(); }
    ...
    virtual void write(const LogLevel level, const std::string &str) {
        ...
        m_file << "[" << (now->tm_year + 1900) << '-' << std::setfill('0') << std::setw(2) << (now->tm_mon + 1) << '-'
...
public:
    ...
    void addSink(std::unique_ptr<LogSink> sink) { m_sinks.push_back(std::move(sink)); }

    void write(const LogLevel level, const std::string &str) {
        for (unsigned int i = 0; i < m_sinks.size(); i++) m_sinks[i]->write(level, str);
    }
};
