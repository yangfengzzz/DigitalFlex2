...
public:
    /** Initialize ball joint and return info which is required by the solver
     * step.
     *
     ...
     * @param jointInfo Stores the local and global positions of the connector
     * points. The first two columns store the local connectors in body 0 and 1,
     * respectively, while the last two columns contain the global connector
     * positions which have to be updated in each simulation step by calling
     ...
     * The ball joint info must be generated in the initialization process of the
     * model by calling the function init_BallJoint(). This method should be
     * called once per simulation step before executing the solver.\n\n
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     * @param x1 center of mass of second body
     ...
     * A ball joint removes three translational degrees of freedom between the
     * bodies. The ball joint info must be generated in the initialization process
     * of the model by calling the function init_BallJoint() and updated each time
     * the bodies change their state by update_BallJoint().\n\n More information
     ...
     * @param x0 center of mass of first body
     * @param inertiaInverseW0 inverse inertia tensor in world coordinates of
     * first body
     * @param q0 rotation of first body
     * @param invMass1 inverse mass of second body
     * @param x1 center of mass of second body
     * @param inertiaInverseW1 inverse inertia tensor in world coordinates of
     * second body
     * @param q1 rotation of second body
     * @param ballJointInfo Ball joint information which is required by the
     * solver. This information must be generated in the beginning by calling
     * init_BallJoint() and updated each time the bodies change their state by
     * update_BallJoint().
    ...
    /** Initialize ball-on-line-joint and return information which is required by
     * the solver step.
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     ...
     * @param jointInfo Stores the local and global positions of the connector
     * points. The first two columns store the local connectors in body 0 and 1,
     * respectively. The next three columns contain a coordinate system for the
     * constraint correction (a full coordinate system where the x-axis is the
     * slider axis) in local coordinates. The last five columns contain the global
     * connector positions and the constraint coordinate system in world space
     * which have to be updated in each simulation step by calling
     * update_BallOnLineJoint().\n The joint info contains the following
    ...
    /** Update ball-on-line-joint information which is required by the solver
     * step. The ball-on-line-joint info must be generated in the initialization
     * process of the model by calling the function init_BallOnLineJoint(). This
     * method should be called once per simulation step before executing the
     * solver.\n\n
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
    ...
    /** Perform a solver step for a ball-on-line-joint which links two rigid
     * bodies. A ball-on-line-joint removes two translational degrees of freedom
     * between the bodies. The joint info must be generated in the initialization
     * process of the model by calling the function init_BallOnLineJoint() and
     * updated each time the bodies change their state by
     ...
     * second body
     * @param q1 rotation of second body
     * @param jointInfo Ball joint information which is required by the solver.
     * This information must be generated in the beginning by calling
     * init_BallJoint() and updated each time the bodies change their state by
     * update_BallJoint().
     * @param corr_x0 position correction of center of mass of first body
     * @param corr_q0 rotation correction of first body
    ...
    /** Initialize hinge joint and return info which is required by the solver
     * step.
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     ...
     * @param hingeJointInfo Stores the local and global positions of the
     * connector points. The joint info contains the following columns:\n 0-1:
     * projection matrix Pr for the rotational part\n 2:	connector in body 0
     ...
     * The joint info stores first the info of the first body (the connector point
     * and a full coordinate system where the x-axis is the hinge axis) and then
     * the info of the second body (the connector point and the hinge axis). The
     * info must be updated in each simulation step by calling
     * update_HingeJoint().
     */
     ...
     * This method should be called once per simulation step before executing the
     * solver.\n\n
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     ...
     * A hinge joint removes three translational and two rotational  degrees of
     * freedom between the bodies. The hinge joint info must be generated in the
     * initialization process of the model by calling the function
     * init_HingeJoint() and updated each time the bodies change their state by
     ...
     * second body
     * @param q1 rotation of second body
     * @param hingeJointInfo Hinge joint information which is required by the
     * solver. This information must be generated in the beginning by calling
     * init_HingeJoint() and updated each time the bodies change their state by
     * update_HingeJoint().
     * @param corr_x0 position correction of center of mass of first body
    ...
    /** Initialize universal joint and return info which is required by the solver
     * step.
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     ...
     * @param jointInfo Stores the local and global positions of the connector
     * points. The joint info contains the following columns:\n
     * 0:	connector in body 0 (local)\n
     * 1:	connector in body 1 (local)\n
     ...
     * The joint info must be generated in the initializatgion process of the
     * model by calling the function init_UniversalJoint(). This method should be
     * called once per simulation step before executing the solver.\n\n
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     ...
     * A universal joint removes three translational and one rotational degree of
     * freedom between the bodies. The universal joint info must be generated in
     * the initialization process of the model by calling the function
     * init_UniversalJoint() and updated each time the bodies change their state
     * by update_UniversalJoint().\n\n More information can be found in: \cite
     * Deul2014
     *
     ...
     * second body
     * @param q1 rotation of second body
     * @param jointInfo Universal joint information which is required by the
     * solver. This information must be generated in the beginning by calling
     * init_UniversalJoint() and updated each time the bodies change their state
     * by update_UniversalJoint().
     * @param corr_x0 position correction of center of mass of first body
    ...
    /** Initialize slider joint and return info which is required by the solver
     * step.
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     ...
     * axis (local)\n 1:   coordinate system in body 0, where the x-axis is the
     * slider axis (global)\n 2:   2D vector d = P * (x0 - x1), where P projects
     * the vector onto a plane perpendicular to the slider axis\n 3-5: projection
     * matrix Pr for the rotational part\n\n The info must be updated in each
     * simulation step by calling update_SliderJoint().
     */
     ...
     * A slider joint removes two translational and three rotational degrees of
     * freedom between the bodies. The slider joint info must be generated in the
     * initialization process of the model by calling the function
     * init_SliderJoint() and updated each time the bodies change their state by
     * update_SliderJoint().\n\n More information can be found in: \cite Deul2014
     *
     ...
     * second body
     * @param q1 rotation of second body
     * @param jointInfo Slider joint information which is required by the solver.
     * This information must be generated in the beginning by calling
     * init_SliderJoint() and updated each time the bodies change their state by
     * update_SliderJoint().
     * @param corr_x0 position correction of center of mass of first body
    ...
    /** Perform a solver step for a motor slider joint which links two rigid
     * bodies. A motor slider joint removes two translational and three rotational
     * degrees of freedom between the bodies. Moreover, a target position can be
     * enforced on the remaining translation axis. The motor slider joint info
     * must be generated in the initialization process of the model by calling the
     * function init_TargetPositionMotorSliderJoint() and updated each time the
     ...
     * @param jointInfo Motor slider joint information which is required by the
     * solver. This information must be generated in the beginning by calling
     * init_TargetPositionMotorSliderJoint() and updated each time the bodies
     ...
     * @param jointInfo Stores the local and global positions of the connector
     * points. The joint info contains the following columns:\n 0:   coordinate
     * system in body 0, where the x-axis is the slider axis (local)\n 1:
     * coordinate system in body 0, where the x-axis is the slider axis (global)\n
     * 2:   2D vector d = P * (x0 - x1), where P projects the vector onto a plane
     * perpendicular to the slider axis\n 3-5: projection matrix Pr for the
     * rotational part\n\n The info must be updated in each simulation step by
     * calling update_TargetVelocityMotorSliderJoint().
     */
    ...
    /** Perform a solver step for a motor slider joint which links two rigid
     * bodies. A motor slider joint removes two translational and three rotational
     * degrees of freedom between the bodies. Moreover, a target velocity can be
     * enforced on the remaining translation axis. The motor slider joint info
     * must be generated in the initialization process of the model by calling the
     * function init_TargetVelocityMotorSliderJoint() and updated each time the
     ...
     * @param jointInfo Motor slider joint information which is required by the
     * solver. This information must be generated in the beginning by calling
     * init_TargetVelocityMotorSliderJoint() and updated each time the bodies
    ...
    /** Perform a velocity solver step for a motor slider joint which links two
     * rigid bodies. A motor slider joint removes two translational and three
     * rotational degrees of freedom between the bodies. Moreover, a target
     * velocity can be enforced on the remaining translation axis. The motor
     * slider joint info must be generated in the initialization process of the
     * model by calling the function init_TargetVelocityMotorSliderJoint() and
     * updated each time the bodies change their state by
    ...
    /** Perform a solver step for a motor hinge joint which links two rigid
     * bodies. A motor hinge joint removes three translational and two rotational
     * degrees of freedom between the bodies. Moreover, a target angle can be
     * enforced on the remaining rotation axis. The motor hinge joint info must be
     * generated in the initialization process of the model by calling the
     * function init_TargetAngleMotorHingeJoint() and updated each time the bodies
     ...
     * @param jointInfo Motor hinge joint information which is required by the
     * solver. This information must be generated in the beginning by calling
     * init_TargetAngleMotorHingeJoint() and updated each time the bodies change
     * their state by update_TargetAngleMotorHingeJoint().
     * @param corr_x0 position correction of center of mass of first body
     * @param corr_q0 rotation correction of first body
     * @param corr_x1 position correction of center of mass of second body
     * @param corr_q1 rotation correction of second body
     ...
     * bodies. A motor hinge joint removes three translational and two rotational
     * degrees of freedom between the bodies. Moreover, a target velocity can be
     * enforced on the remaining rotation axis by calling the function
     * velocitySolve_TargetVelocityMotorHingeJoint(). The motor hinge joint info
     * must be generated in the initialization process of the model by calling the
     * function init_TargetVelocityMotorHingeJoint() and updated each time the
     * bodies change their state by update_TargetVelocityMotorHingeJoint().\n\n
     * More information can be found in: \cite Deul2014
     *
     * \image html motorhingejoint.jpg "motor hinge joint"
     * \image latex motorhingejoint.jpg "motor hinge joint" width=0.5\textwidth
     ...
     * @param jointInfo Motor hinge joint information which is required by the
     * solver. This information must be generated in the beginning by calling
     * init_TargetVelocityMotorHingeJoint() and updated each time the bodies
    ...
    /** Perform a velocity solver step for a motor hinge joint which links two
     * rigid bodies. A motor hinge joint removes three translational and two
     * rotational degrees of freedom between the bodies. Moreover, a target
     * velocity can be enforced on the remaining rotation axis. The motor hinge
     * joint info must be generated in the initialization process of the model by
     * calling the function init_TargetVelocityMotorHingeJoint() and updated each
     * time the bodies change their state by
     ...
     * at the position of the particle and return info which is required by the
     * solver step.
     *
     * @param x0 center of mass of the rigid body
     * @param q0 rotation of the rigid body body
     * @param x1 position of the particle
     * @param jointInfo Stores the local and global position of the connector
     * point in the rigid body. The info must be updated in each simulation step
     * by calling update_RigidBodyParticleBallJoint().\n The joint info contains
    ...
    /** Perform a solver step for a ball joint which links a rigid body and a
     * particle. The joint info must be generated in the initialization process of
     * the model by calling the function init_RigidBodyParticleBallJoint() and
     * updated each time the rigid body changes its state by
     ...
     * init_RigidBodyParticleBallJoint() and updated each time the rigid body
     ...
     * @param constraintInfo Stores the local and global position of the contact
     * points and the contact normal. \n The joint info contains the following
     ...
     * A contact constraint handles collisions and resting contacts between the
     * bodies. The contact info must be generated in each time step.
     *
     * @param invMass0 inverse mass of rigid body
     * @param x0 center of mass of first body
     * @param v0 velocity of body 0
     ...
     * information must be generated in the beginning by calling
     * init_RigidBodyContactConstraint().
     * @param corr_v0 velocity correction of first body
     * @param corr_omega0 angular velocity correction of first body
     * @param corr_v1 velocity correction of second body
     * @param corr_omega1 angular velocity correction of second body
     ...
     * particle. A contact constraint handles collisions and resting contacts
     * between the bodies. The contact info must be generated in each time step.
     *
     * @param invMass0 inverse mass of rigid body
     * @param x0 center of mass of first body
     * @param v0 velocity of body 0
    ...
    /** Initialize distance joint and return info which is required by the solver
     * step.
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     ...
     * The distance joint info must be generated in the initialization process of
     * the model by calling the function init_DistanceJoint(). This method should
     * be called once per simulation step before executing the solver.\n\n
     *
     * @param x0 center of mass of first body
     * @param q0 rotation of first body
     * @param x1 center of mass of second body
     ...
     * A distance joint removes one translational degrees of freedom between the
     * bodies. When setting a stiffness value which is not zero, we get an
     * implicit spring. The distance joint info must be generated in the
     * initialization process of the model by calling the function
     * init_DistanceJoint() and updated each time the bodies change their state by
     * update_DistanceJoint().\n\n More information can be found in: \cite
     * Deul2014
     *
     * @param invMass0 inverse mass of first body
     ...
     * second body
     * @param q1 rotation of second body
     * @param stiffness Stiffness of the constraint. 0 means it is totally stiff
     * and we get a distance joint otherwise we get an implicit spring.
     ...
     * init_DistanceJoint() and updated each time the bodies change their state by
     * update_DistanceJoint().
     * @param lambda Lagrange multiplier (required for XPBD). Must be 0 in the
     * first iteration.
     * @param corr_x0 position correction of center of mass of first body
     * @param corr_q0 rotation correction of first body
     * @param corr_x1 position correction of center of mass of second body
     ...
     * @param jointInfo Stores the local and global positions of the connector
     * points. The joint info contains the following columns:\n
     * 0:	coordinate system in body 0, where the x-axis is the slider axis
     * (local)\n
     * 1:	coordinate system in body 0, where the x-axis is the slider axis
     * (global)\n 2:    3D vector d = R^T * (x0 - x1), where R is a rotation
     * matrix with the slider axis as first column\n 3-5:	projection matrix Pr for
     * the rotational part\n\n The info must be updated in each simulation step by
     * calling update_TargetPositionMotorSliderJoint().
     */
