...
void TimeStepController::initParameters() {
    ...
    // 	SOLVER_ITERATIONS = createNumericParameter("iterations", "Iterations",
    // &m_iterations); 	setGroup(SOLVER_ITERATIONS, "PBD");
    ...
    // 	SOLVER_ITERATIONS_V = createNumericParameter("iterationsV", "Velocity
    // iterations", &m_iterationsV); 	setGroup(SOLVER_ITERATIONS_V, "PBD");
    // 	setDescription(SOLVER_ITERATIONS_V, "Iterations required by the velocity
    // solver."); 	getParameter(SOLVER_ITERATIONS_V)->setReadOnly(true);
