...
void KDTree<HullType>::construct() {
    ...
    for (auto i = 0u; i < m_lst.size(); ++i) box.extend(entity_position(i));
...
void KDTree<HullType>::construct(unsigned int node, AlignedBox3r const &box, unsigned int b, unsigned int n) {
    ...
    std::sort(m_lst.begin() + b, m_lst.begin() + b + n, [&](unsigned int a, unsigned int b) {
    ...
    auto c = static_cast<Real>(0.5) *
             (entity_position(m_lst[b + hal - 1])(max_dir) + entity_position(m_lst[b + hal])(max_dir));
    auto l_box = box;
    l_box.max()(max_dir) = c;
    auto r_box = box;
    r_box.min()(max_dir) = c;
...
template <typename HullType>
void KDTree<HullType>::traverse_depth_first(TraversalPredicate pred,
                                            TraversalCallback cb,
                                            TraversalPriorityLess const &pless) const {
    if (m_nodes.empty()) return;

    if (pred(0, 0)) traverse_depth_first(0, 0, pred, cb, pless);
}
...
template <typename HullType>
void KDTree<HullType>::traverse_breadth_first_parallel(TraversalPredicate pred, TraversalCallback cb) const {
    ...
    traverse_breadth_first(
            ...
            [&](unsigned int node_index, unsigned int depth) {
                if ((depth == targetDepth) || (node(node_index).is_leaf())) start_nodes.push_back({node_index, depth});
            });
...
void KDTree<HullType>::update() {
    traverse_depth_first([&](unsigned int, unsigned int) { return true; },
