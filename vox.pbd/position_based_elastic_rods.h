...
private:
     ...
     */
    static Real factor(int intervalIndex,
...
public:
     ...
     * and torsion constraints of the rod. The set of constraints must be acyclic.
     ...
     * @param RHS vector with entries for each constraint.
     * In concatenation these entries represent the right hand side of the system of equations to be solved. (eq. 22 in
     * the paper)
     ...
     * vector outside the solve-method avoids repeated reallocation between iterations of the solver
     ...
     *
     * @param rodConstraints contains the combined zero-stretch, bending and torsion constraints of the rod.
     * @param inverseTimeStepSize inverse of the current time step size used to compute compliance (see computation of
     * alpha-tilde in eq. 17)
     * @param lambdaSums contains entries of the sum of all lambda updates for
     * each constraint in the rod during one time step which is needed by the solver to handle
     * compliance in the correct way (cf. the right hand side of eq. 22 in the paper).
     */
    static bool initBeforeProjection_DirectPositionBasedSolverForStiffRodsConstraint(
            const std::vector<RodConstraint *> &rodConstraints,
            Real inverseTimeStepSize,
            std::vector<Vector6r> &lambdaSums);

    /** Determine the position and orientation corrections for all combined zero-stretch,
     * bending and twisting constraints of the rod (eq. 22 in the paper). \n\n
     *
     * @param rodConstraints contains the combined zero-stretch, bending and
     * torsion constraints of the rod. The set of constraints must be acyclic.
     * @param rodSegments contains the segments of the rod
     ...
     * zero-stretch, bending and torsion constraints. The reverse of the forward list.
     * @param RHS vector with entries for each constraint. In concatenation these
     * entries represent the right hand side of the system of equations to be solved. (eq. 22 in the paper)
     * @param lambdaSums contains entries of the sum of all lambda updates for
     * each constraint in the rod during one time step which is needed by the solver to handle
     ...
     * @param averageSegmentLength the average length of the two rod segments connected by the constraint. Value in
     * Meters (m)
     * @param youngsModulus the Young's modulus of the rod material measures stiffness against bending. Value in Pascal
     * (Pa)
     * @param torsionModulus the torsion modulus (also called shear modulus) of the rod material measures stiffness
     ...
     * @param stiffnessCoefficientK diagonal matrix with material parameters for bending and torsion stiffness (eq. 5 in
     * the paper)
     * @param restDarbouxVector the rest Darboux vector computed in this method with the initial constraint
     * configuration
     */
     ...
     * @param inverseTimeStepSize inverse of the current time step size used to compute compliance (see computation of
     * alpha-tilde in eq. 17)
     * @param bendingAndTorsionCompliance the compliance of the bending and torsion constraint part (eq. 24 in the
     * paper)
     ...
     */
    static bool solve_StretchBendingTwistingConstraint(Real invMass0,
                                                       const Vector3r &x0,
