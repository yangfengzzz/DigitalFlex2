...
private:
    static void initLists(int numberOfIntervals,
    ...
    /** This method recursively visits all segment nodes and inserts constraint
     * nodes between them.
     */
    ...
    /** Sorts matrix H.
     */
    static void orderMatrix(Node *n, int intervalIndex, std::list<Node *> *forward, std::list<Node *> *backward);
     ...
     */
    static void initNodes(int intervalIndex,
                          std::vector<RodSegment *> &rodSegments,
                          Node *&root,
                          Interval *intervals,
                          std::vector<RodConstraint *> &rodConstraints,
    ...
    static void initTree(std::vector<RodConstraint *> &rodConstraints,
                         std::vector<RodSegment *> &rodSegments,
                         Interval *&intervals,
     ...
     */
    static Real factor(const int intervalIndex,
     ...
     */
    static bool solve(int intervalIndex,
                      std::list<Node *> *forward,
                      std::list<Node *> *backward,
                      std::vector<Vector6r> &RHS,
                      std::vector<Vector6r> &lambdaSums,
...
public:
    ///** Initialize the zero-stretch, bending, and torsion constraints of the
    /// rod.
    //* Computes constraint connectors in segment space, computes the diagonal
    // stiffness matrices
    //* and the Darboux vectors of the initial state. Initializes the forward and
    // backward lists
    //* of nodes  for the direct solver\n\n
    //*
    //* @param rodConstraints contains the combined zero-stretch, bending
    //* and torsion constraints of the rod. The set of constraints must by
    // acyclic.
    //* @param rodSegments contains the segments of the rod
    //* @param forward list of nodes in the acyclic tree of rod segments and
    // zero-stretch,
    //* bending and torsion constraints so that parent nodes occur later in the
    // list than their children
    //* @param backward list of nodes in the acyclic tree of rod segments and
    // zero-stretch,
    //* bending and torsion constraints. The reverse of the forward list.
    //* @param constraintPositions positions of the rod's constraints in world
    // coordinates
    //* @param averageRadii the average radii at the constraint positions of the
    // rod. Value in Meters (m)
    //* @param averageSegmentLengths vector of the average lengths of the two rod
    // segments
    //* connected by a constraint of the rod. Value in Meters (m)
    //* @param youngsModuli vector of the Young's modulus of every constraint of
    // the rod.
    //* The Young's modulus of the rod material measures
    //* stiffness against bending. Value in Pascal (Pa)
    //* @param torsionModuli vector of the torsion modulus of every constraint of
    // the rod.
    //* The torsion modulus (also called shear modulus)
    //* of the rod material measures stiffness against torsion. Value in Pascal
    //(Pa)
    //* @param RHS vector with entries for each constraint. In concatenation these
    // entries represent the right hand side of the system of equations to be
    // solved. (eq. 22 in the paper)
    //* @param lambdaSums contains entries of the sum of all lambda updates for
    //* each constraint in the rod during one time step which is needed by the
    // solver to handle
    //* compliance in the correct way (cf. the right hand side of eq. 22 in the
    // paper).
    ...
    //* vector outside of the solve-method avoids repeated reallocation between
    // iterations of the solver
    //* @param corr_x vector of position corrections for every segment of the rod
    //(part of delta-x in eq. 22 in the paper)
    //* @param corr_q vector of rotation corrections for every segment of the rod
    //(part of delta-x in eq. 22 in the paper)
    //*/
    ...
    //*
    //* @param rodConstraints contains the combined zero-stretch, bending and
    // torsion constraints of the rod.
    //* @param rodSegments contains the segments of the rod
    //*/
    ...
    ///** Initialize the constraint before the projection iterations in each time
    /// step. \n\n
    //*
    //* @param rodConstraints contains the combined zero-stretch, bending and
    // torsion constraints of the rod.
    //* @param inverseTimeStepSize inverse of the current time step size used to
    // compute compliance (see computation of alpha-tilde in eq. 17)
    //* @param lambdaSums contains entries of the sum of all lambda updates for
    //* each constraint in the rod during one time step which is needed by the
    // solver to handle
    //* compliance in the correct way (cf. the right hand side of eq. 22 in the
    ...
    ///** Determine the position and orientation corrections for all combined
    /// zero-stretch, bending and twisting constraints of the rod (eq. 22 in the
    /// paper). \n\n
    //*
    //* @param rodConstraints contains the combined zero-stretch, bending and
    // torsion constraints of the rod. The set of constraints must by acyclic.
    //* @param rodSegments contains the segments of the rod
    //* @param forward list of nodes in the acyclic tree of rod segments and
    // zero-stretch, bending and torsion constraints so that parent nodes occur
    // later in the list than their children
    //* @param backward list of nodes in the acyclic tree of rod segments and
    // zero-stretch, bending and torsion constraints. The reverse of the forward
    // list.
    //* @param RHS vector with entries for each constraint. In concatenation these
    // entries represent the right hand side of the system of equations to be
    // solved. (eq. 22 in the paper)
    ...
    //* @param averageRadius the average radius of the two rod segments connected
    // by the constraint. Value in Meters (m)
    //* @param averageSegmentLength the average length of the two rod segments
    // connected by the constraint. Value in Meters (m)
    //* @param youngsModulus the Young's modulus of the rod material measures
    // stiffness against bending. Value in Pascal (Pa)
    //* @param torsionModulus the torsion modulus (also called shear modulus) of
    // the rod material measures stiffness against torsion. Value in Pascal (Pa)
    //* @param jointInfo joint information which is required by the solver.This
    //* information is generated in this method
    //* and updated each time the bodies change their state by
    // update_StretchBendingTwistingConstraint().
    //* @param stiffnessCoefficientK diagonal matrix with material parameters for
    // bending and torsion stiffness (eq. 5 in the paper)
    //* @param restDarbouxVector the rest Darboux vector computed in this method
    // with the initial constraint configuration
    //*/
    static bool init_StretchBendingTwistingConstraint(const Vector3r &x0,
    ...
    //* @param inverseTimeStepSize inverse of the current time step size used to
    // compute compliance (see computation of alpha-tilde in eq. 17)
    //* @param bendingAndTorsionCompliance the compliance of the bending and
    // torsion constraint part (eq. 24 in the paper)
    //* @param stretchCompliance the compliance of the stretch constraint part
    //(eq. 24 in the paper)
    //* @param lambdaSum the sum of all lambda updates of
    //* this constraint during one time step which is needed by the solver to
    // handle
    //* compliance in the correct way. Is set to zero. (see eq. 19 in the paper)
    //*/
    static bool initBeforeProjection_StretchBendingTwistingConstraint(const Vector3r &stiffnessCoefficientK,
                                                                      const Real inverseTimeStepSize,
                                                                      const Real averageSegmentLength,
    ...
    //*/
    static bool update_StretchBendingTwistingConstraint(const Vector3r &x0,
                                                        const Quaternionr &q0,
                                                        const Vector3r &x1,
                                                        const Quaternionr &q1,
                                                        Eigen::Matrix<Real, 3, 4, Eigen::DontAlign> &jointInfo);

    ///** Determine the position and orientation corrections for the combined
    /// zero-stretch, bending and twisting constraint (eq. 23 in the paper). \n\n
    //*
    //* @param invMass0 inverse mass of first body; inverse mass is zero if body
    // is static
    //* @param x0 center of mass of body 0
    //* @param inertiaInverseW0 inverse inertia tensor (world space) of body 0
    //* @param q0 rotation of body 0
    //* @param invMass1 inverse mass of second body; inverse mass is zero if body
    // is static
    //* @param x1 center of mass of body 1
    //* @param inertiaInverseW1 inverse inertia tensor (world space) of body 1
    //* @param q1 rotation of body 1
    //* @param restDarbouxVector the rest Darboux vector of the initial constraint
    // configuration
    //* @param averageSegmentLength the average length of the two rod segments
    // connected by the constraint
    //* @param stretchCompliance the compliance of the stretch constraint part
    //(eq. 24 in the paper)
    //* @param bendingAndTorsionCompliance the compliance of the bending and
    // torsion constraint part (eq. 24 in the paper)
    ...
    //* initBeforeProjection_StretchBendingTwistingConstraint(). (see eq. 19 in
    // the paper)
    //*/
    static bool solve_StretchBendingTwistingConstraint(const Real invMass0,
                                                       const Vector3r &x0,
...
public:
    /** Determine the position and orientation corrections for the stretch and
     * shear constraint constraint (eq. 37 in the paper). \n\n
     *
     ...
     * @param stretchingAndShearingKs stiffness coefficients for stretching and
     * shearing
     ...
     */
    static bool solve_StretchShearConstraint(const Vector3r &p0,
                                             Real invMass0,
                                             const Vector3r &p1,
                                             Real invMass1,
                                             const Quaternionr &q0,
    ...
    /** Determine the position corrections for the bending and torsion constraint
     * constraint (eq. 40 in the paper). \n\n
     *
     ...
     * @param bendingAndTwistingKs stiffness coefficients for stretching and
     * shearing
     ...
     */
    static bool solve_BendTwistConstraint(const Quaternionr &q0,
                                          Real invMassq0,
                                          const Quaternionr &q1,
                                          Real invMassq1,
...
public:
     ...
     * \mathbf{p}_1 - 0.5 (\mathbf{p}_0 + \mathbf{p}_1))^T (\mathbf{p}_1 -
     * \mathbf{p}_0) = 0\f$\n\n
     *
     * @param p0 position of first particle
     * @param invMass0 inverse mass of first particle
     ...
     */
    static bool solve_PerpendiculaBisectorConstraint(const Vector3r &p0,
                                                     Real invMass0,
                                                     const Vector3r &p1,
                                                     Real invMass1,
                                                     const Vector3r &p2,
    ...
    /** Determine the position corrections for a constraint that enforces a rest
     * length between an edge and a ghost point:\n\n \f$C(\mathbf{p}_0,
     * \mathbf{p}_1, \mathbf{p}_2) = \| ( 0.5 (\mathbf{p}_0 + \mathbf{p}_1) -
     * \mathbf{p}_2 \| - L_0 = 0\f$\n\n
     *
     * @param p0 position of first particle
     * @param invMass0 inverse mass of first particle
     ...
     * @param corr2 position correction of third particle
     */
    static bool solve_GhostPointEdgeDistanceConstraint(const Vector3r &p0,
                                                       Real invMass0,
                                                       const Vector3r &p1,
                                                       Real invMass1,
                                                       const Vector3r &p2,
    ...
    /** Determine the position corrections for the Darboux vector constraint (eq.
     * 21 in the paper). See the paper appendix for derivation details\n\n
     *
     * @param p0 position of first particle
     * @param invMass0 inverse mass of first particle
     * @param p1 position of second particle
     ...
     */
    static bool solve_DarbouxVectorConstraint(const Vector3r &p0,
                                              Real invMass0,
                                              const Vector3r &p1,
                                              Real invMass1,
                                              const Vector3r &p2,
    ...
    /** Computes the material frame derivatives (eq. 43, 44 and 45 in the
     * appendix)
     */
    ...
    /** Compute the Darboux gradient in respect to each point (eq. 49-53 in the
     * appendix)
     */
    static bool computeDarbouxGradient(const Vector3r &darboux_vector,  // Darboux vector
                                       ...
                                       // const Vector3r& bendAndTwistKs, //bending (x,y) and twisting (z)
                                       // stiffness
                                       Matrix3r &omega_pa,
                                       Matrix3r &omega_pb,
                                       Matrix3r &omega_pc,
