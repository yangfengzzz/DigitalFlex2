...
public:
    ...
    static std::string normalizePath(const std::string &path) {
        ...
        for (unsigned int i = 1; i < tokens.size(); i++) result = result + "/" + tokens[i];
     ...
     */
    static const std::string fileDialog(int dialogType, const std::string &initialDir, const std::string &filter) {
        ...
        if (dialogType == 0) {
            if (GetOpenFileName(&ofn)) return std::string(fileNameBuffer);
        } else {
