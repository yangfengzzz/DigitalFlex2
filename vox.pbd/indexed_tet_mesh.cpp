...
void IndexedTetMesh::initMesh(const unsigned int nPoints,
...
void IndexedTetMesh::addTet(const unsigned int *const indices) {
    for (unsigned int i = 0u; i < 4; i++) m_tetIndices.push_back(indices[i]);
}

/** Add a new tet. Indices must be an array of size 4.
 */
void IndexedTetMesh::addTet(const int *const indices) {
    for (unsigned int i = 0u; i < 4; i++) m_tetIndices.push_back((unsigned int)indices[i]);
}
...
void IndexedTetMesh::buildNeighbors() {
    ...
    for (unsigned int i = 0; i < numTets(); i++) {
        ...
        const unsigned int edges[12] = {m_tetIndices[4 * i],     m_tetIndices[4 * i + 1], m_tetIndices[4 * i],
        ...
        /*const unsigned int faces[12] = {	m_tetIndices[4*i], m_tetIndices[4*i+1],
           m_tetIndices[4*i+2], m_tetIndices[4*i+1], m_tetIndices[4*i+3],
           m_tetIndices[4*i+2], m_tetIndices[4*i+3], m_tetIndices[4*i],
           m_tetIndices[4*i+2], m_tetIndices[4*i+1], m_tetIndices[4*i],
           m_tetIndices[4*i+3]};*/
        ...
        for (unsigned int j = 0u; j < 4; j++) {
            ...
            for (unsigned int k = 0; k < m_verticesFaces[a].size(); k++) {
                ...
                if (((m_faceIndices[3 * faceIndex] == a) || (m_faceIndices[3 * faceIndex] == b) ||
                     (m_faceIndices[3 * faceIndex] == c)) &&
                    ((m_faceIndices[3 * faceIndex + 1] == a) || (m_faceIndices[3 * faceIndex + 1] == b) ||
                     (m_faceIndices[3 * faceIndex + 1] == c)) &&
                    ((m_faceIndices[3 * faceIndex + 2] == a) || (m_faceIndices[3 * faceIndex + 2] == b) ||
        ...
        for (unsigned int j = 0u; j < 6; j++) {
            ...
            for (unsigned int k = 0; k < m_verticesEdges[a].size(); k++) {
                ...
                if (((e.m_vert[0] == a) || (e.m_vert[0] == b)) && ((e.m_vert[1] == a) || (e.m_vert[1] == b))) {
