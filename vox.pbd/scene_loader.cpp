...
void SceneLoader::readScene(const std::string &fileName, SceneData &sceneData) {
    ...
    try {
        ...
        // read DamperJoints
        //////////////////////////////////////////////////////////////////////////
        if (m_json.find("DamperJoints") != m_json.end()) readDamperJoints(m_json, "DamperJoints", sceneData);
...
void SceneLoader::readRigidBodies(const nlohmann::json &j,
    ...
    for (auto &rigidBody : child) {
        ...
        if (readValue<std::string>(rigidBody, "geometryFile", geomFileName)) {
            ...
            axis.setZero();
            Real angle = 0.0;
            if (readVector(rigidBody, "rotationAxis", axis) && readValue<Real>(rigidBody, "rotationAngle", angle)) {
                axis.normalize();
...
void SceneLoader::readTriangleModels(const nlohmann::json &j,
    ...
    for (auto &triModel : child) {
        ...
        if (readValue<std::string>(triModel, "geometryFile", geomFileName)) {
            ...
            axis.setZero();
            Real angle = 0.0;
            if (readVector(triModel, "rotationAxis", axis) && readValue<Real>(triModel, "rotationAngle", angle)) {
                axis.normalize();
...
void SceneLoader::readTetModels(const nlohmann::json &j,
    ...
    for (auto &tetModel : child) {
        ...
        if (readValue<std::string>(tetModel, "nodeFile", nodeFileName) &&
            ...
            axis.setZero();
            Real angle = 0.0;
            if (readVector(tetModel, "rotationAxis", axis) && readValue<Real>(tetModel, "rotationAngle", angle)) {
                axis.normalize();
                data.m_q = Quaternionr(AngleAxisr(angle, axis));
            } else
                data.m_q = Quaternionr(1.0, 0.0, 0.0, 0.0);
...
void SceneLoader::readBallJoints(const nlohmann::json &j, const std::string &key, SceneData &sceneData) {
    ...
    for (auto &joint : child) {
        BallJointData jd;
        if (readValue(joint, "bodyID1", jd.m_bodyID[0]) && readValue(joint, "bodyID2", jd.m_bodyID[1]) &&
...
void SceneLoader::readBallOnLineJoints(const nlohmann::json &j, const std::string &key, SceneData &sceneData) {
    ...
    for (auto &joint : child) {
        ...
        if (readValue(joint, "bodyID1", jd.m_bodyID[0]) && readValue(joint, "bodyID2", jd.m_bodyID[1]) &&
            readVector(joint, "position", jd.m_position) && readVector(joint, "axis", jd.m_axis)) {
...
void SceneLoader::readUniversalJoints(const nlohmann::json &j, const std::string &key, SceneData &sceneData) {
    ...
    for (auto &joint : child) {
        ...
        if (readValue(joint, "bodyID1", jd.m_bodyID[0]) && readValue(joint, "bodyID2", jd.m_bodyID[1]) &&
            readVector(joint, "position", jd.m_position) && readVector(joint, "axis1", jd.m_axis[0]) &&
        ...
        if (readValue(joint, "bodyID1", jd.m_bodyID[0]) && readValue(joint, "bodyID2", jd.m_bodyID[1]) &&
            ...
            if (!readValue(joint, "target", jd.m_target)) {
                ...
                if (joint.find("targetSequence") != joint.end()) {
                    ...
                    if (jd.m_targetSequence.size() % 2 == 1) jd.m_targetSequence.pop_back();
...
void SceneLoader::readRigidBodySprings(const nlohmann::json &j, const std::string &key, SceneData &sceneData) {
    ...
    for (auto &joint : child) {
        ...
        if (readValue(joint, "bodyID1", jd.m_bodyID[0]) && readValue(joint, "bodyID2", jd.m_bodyID[1]) &&
            readVector(joint, "position1", jd.m_position1) && readVector(joint, "position2", jd.m_position2) &&
...
void SceneLoader::readDamperJoints(const nlohmann::json &j, const std::string &key, SceneData &sceneData) {
    ...
    for (auto &joint : child) {
        DamperJointData jd;
        if (readValue(joint, "bodyID1", jd.m_bodyID[0]) && readValue(joint, "bodyID2", jd.m_bodyID[1]) &&
            readVector(joint, "axis", jd.m_axis) && readValue(joint, "stiffness", jd.m_stiffness)) {
