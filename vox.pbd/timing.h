...
/** \brief Struct to store the total time and the number of steps in order to
...
public:
    ...
    FORCE_INLINE static double stopTiming(bool print = true) {
        if (!Timing::m_timingStack.empty()) {
            ...
            if (print) LOG_INFO << "time " << h.name.c_str() << ": " << t << " ms";
            return t;
        }
        return 0;
    }
    ...
    FORCE_INLINE static double stopTiming(bool print, int &id) {
        ...
        if (!Timing::m_timingStack.empty()) {
            ...
            if (print && !Timing::m_dontPrintTimes) LOG_INFO << "time " << h.name.c_str() << ": " << t << " ms";
    ...
    FORCE_INLINE static void printAverageTimes() {
        ...
        if (Timing::m_startCounter != Timing::m_stopCounter)
            LOG_INFO << "Problem: " << Timing::m_startCounter << " calls of startTiming and " << Timing::m_stopCounter
                     << " calls of stopTiming. ";
        LOG_INFO << "--------------------------------------------------------------"
                    "-------------\n";
    }
