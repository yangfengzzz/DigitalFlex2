...
IndexedFaceMesh &IndexedFaceMesh::operator=(IndexedFaceMesh const &other) {
    ...
    for (size_t i(0u); i < m_verticesEdges.size(); ++i) m_verticesEdges[i] = other.m_verticesEdges[i];

    m_verticesFaces.resize(other.m_verticesFaces.size());
    for (size_t i(0u); i < m_verticesFaces.size(); ++i) m_verticesFaces[i] = other.m_verticesFaces[i];
...
bool IndexedFaceMesh::isClosed() const { return m_closed; }
...
void IndexedFaceMesh::addFace(const unsigned int *const indices) {
    for (unsigned int i = 0u; i < m_verticesPerFace; i++) m_indices.push_back(indices[i]);
}

/** Add a new face. Indices must be an array of size m_verticesPerFace.
 */
void IndexedFaceMesh::addFace(const int *const indices) {
    for (unsigned int i = 0u; i < m_verticesPerFace; i++) m_indices.push_back((unsigned int)indices[i]);
}
...
void IndexedFaceMesh::addUVIndex(const unsigned int index) { m_uvIndices.push_back(index); }
...
unsigned int IndexedFaceMesh::getVerticesPerFace() const { return m_verticesPerFace; }
