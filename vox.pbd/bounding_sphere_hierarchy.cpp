...
void PointCloudBSH::compute_hull_approx(unsigned int b, unsigned int n, BoundingSphere &hull) const {
    ...
    Vector3r x;
    x.setZero();
    for (unsigned int i = b; i < b + n; i++) x += m_vertices[m_lst[i]];
    x /= (Real)n;
...
void TetMeshBSH::init(const Vector3r *vertices,
    ...
    for (unsigned int i = 0; i < numTets; i++) {
        m_com[i] = 0.25 * (m_vertices[m_indices[4 * i]] + m_vertices[m_indices[4 * i + 1]] +
                           m_vertices[m_indices[4 * i + 2]] + m_vertices[m_indices[4 * i + 3]]);
    }
}
...
void BVHTest::traverse(PointCloudBSH const &b1,
