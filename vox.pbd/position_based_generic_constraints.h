...
// ------------------------------------------------------------------------------------
namespace vox {
/** Generic constraints provide the possibility to experiment with new
 * constraint functions very easily. Only a callback for the constraint function
 * and optionally for the gradient of the constraint must be provided.\n\n
 *
...
public:
    ...
    static bool solve_GenericConstraint(
            const Real invMass[numberOfParticles],  // inverse mass is zero if particle
                                                    // is static
            const Vector3r x[numberOfParticles],    // positions of particles
            void *userData,

            void (*constraintFct)(const unsigned int numParticles,
            ...
            void (*gradientFct)(const unsigned int i,  // compute dC / dxi
     ...
     * @param  inertiaInverseW inverse inertia tensor in world coordinates of the
     * rigid bodies
    ...
    static bool solve_GenericConstraint(
            const Real invMass[numberOfRigidBodies],              // inverse mass is zero if body
                                                                  // is static
            const Vector3r x[numberOfRigidBodies],                // positions of bodies
            const Matrix3r inertiaInverseW[numberOfRigidBodies],  // inverse inertia tensor (world
                                                                  // space) of bodies
            const Quaternionr q[numberOfRigidBodies],             // rotation of bodies
            void *userData,

            void (*constraintFct)(const unsigned int numRigidBodies,
                                  const Real invMass[numberOfRigidBodies],              // inverse mass is zero if
                                                                                        // body is static
                                  const Vector3r x[numberOfRigidBodies],                // positions of bodies
                                  const Matrix3r inertiaInverseW[numberOfRigidBodies],  // inverse inertia tensor
                                                                                        // (world space) of bodies
                                  const Quaternionr q[numberOfRigidBodies],             // rotation of bodies
                                  void *userData,
                                  Eigen::Matrix<Real, dim, 1> &constraintValue),
...
bool PositionBasedGenericConstraints::solve_GenericConstraint(
    ...
    // compute Kinv
    if (K.determinant() < 1.0e-6) return false;
...
bool PositionBasedGenericConstraints::solve_GenericConstraint(
    ...
    for (unsigned int j = 0; j < numberOfParticles; j++) xTemp[j] = x[j].template cast<Real>();
...
bool PositionBasedGenericConstraints::solve_GenericConstraint(
    ...
    for (unsigned int i = 0u; i < numberOfRigidBodies; i++) {
        if (invMass[i] != 0.0) {
            // compute gradient
            approximateGradient<numberOfRigidBodies, dim>(i, invMass, x, inertiaInverseW, q, userData, constraintFct,
                                                          gradients[i]);
